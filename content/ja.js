...
window.LIE_CONTENT.ja = {
  ...
  cards: [
   {t:'この旅', b:'<p>五回分の議論に相当する導出を、一つの空間に収めました。この旅は<em>合流点</em>です: 平坦な最適化と曲がった <span class="m">SO(3)</span> がここで出会います — 拘束から射影と <span class="m">exp</span> を経てトポロジーまで、八つのステーション。</p><p>平坦な土台を築くのは <em>Geometry · ℝⁿ</em> と <em>Optimization · 勾配降下法</em> の旅です。ここでは思い出すだけ。菫色の糸が思考の筋道で、ステーション間を飛ぶときに見えます。</p><p>操作: ← → のボタンかキー、マウスのドラッグで見回し、スクロールでズーム。</p>'},
   {t:'平坦な世界', b:'<p>おなじみのレシピを一段落で: パラメータ空間は <span class="m">ℝⁿ</span>、コストは谷、そして <span class="m">w − α∇L</span> のステップはいつでも有効なままです — 外に出て行く先がそもそもありません。</p><p>ここを丁寧に築くのは <em>Geometry · ℝⁿ</em> と <em>Optimization · 勾配降下法</em> の旅です。ここで要るのは一つだけ: <strong>勾配は点と同じ空間に住んでいる</strong>ということ。次のステーションで壊れるのが、まさにこれです。</p>'},
   ...
   {t:'SE(3) — 時間の中の位置姿勢', b:'<p>回転 + 並進 = 位置姿勢: 6 自由度、経路に沿って動く座標系です。<span class="m">⊞</span> のレシピは同じで、接空間が <span class="m">ℝ⁶</span> になるだけ: 三つの角度と三つの並進。</p><p>位置姿勢には独立した章があります: <em>Geometry · SE(3)</em> の旅が十のステーションで扱います — 連鎖、R と t の順序、ねじ運動、左ヤコビアン、随伴、そして補間。</p><p>ここから SLAM が始まります。こうした位置姿勢が多数、因子で結ばれ、その上で前のステーションの反復が走ります。</p>'}
  ]
};
