...
window.LIE_CONTENT.ja = {
  ...
  hub: {
    ...
    hintSystem: ['ドラッグ: 回転', 'スクロール: ズーム', '← → : 惑星', '↑ 入る · ↓ 選択解除'],
    hintPlanet: ['ドラッグ: 回転', 'スクロール: ズーム', '← → : 衛星リング', '↑ 入る · ↓ 出る'],
    moonWord: '目的地',
    branches: {
      geometry: { title: '幾何', summary: '<p>ものが住む空間 — 回転、位置姿勢、相似 — と、その動き方。平坦な ℝⁿ から Sim(3) まで六つの旅: 接空間、exp/log、そして（SO(3) では）トポロジー。コスト関数はどこにも出てきません。</p>',
        moons: {
          ...
          so3: { title: 'SO(3) — 三次元の回転', summary: '<p>理論全体を正当化する空間。非可換で、コンパクトで、本当のかたちは球面ではなく ℝP³ です。</p><p>不可能性の定理もここにあります: 大域的で特異点のない三数の座標は存在しません — ジンバルロックは厄介事ではなく、帰結です。</p>' },
      ...
      optimization: { title: '最適化', summary: '<p>コストを最小化するやり方: 勾配降下法、次に Gauss–Newton、そして実務的な変種たち。すべては平坦な ℝⁿ に収まります — 多様体は要りません。</p>',
        moons: {
          gd: { title: '勾配降下法', summary: '<p>谷、勾配、そして反復 — すべて平坦な ℝ² で、閉じた形まで計算し切ります。</p><p>これがのちにリーマン勾配で曲がった SO(3) へ持ち上げるエンジンです。まずは何も複雑にしない場所で動くのを見ましょう。</p>' },
