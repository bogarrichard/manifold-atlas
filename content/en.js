...
window.LIE_CONTENT.en = {
  ...
  ui: {
    ...
    speech: {
      ...
      off: 'off',
      auto: 'each station',
      playAria: 'Read this station aloud',
      stopAria: 'Stop reading',
      voiceLabel: 'Voice',
      network: 'network',
      supWord: 'to the power of',
      subWord: 'sub',
      matrix: 'a {r} by {c} matrix',
      vector: 'a column vector of {r} entries',
      symbols: {
        ℝ: 'R',
        ℤ: 'Z',
        ẏ: 'y dot',
    ...
    noscript:
      'This interactive visualization requires JavaScript and WebGL. Please enable them in your browser.',
  },
  hub: {
    eyebrow: 'star chart',
    branchWord: 'sector',
    ...
    branches: {
      geometry: {
        ...
        moons: {
          flat: {
            ...
            summary:
              '<p>The flat base case: the space you already know. The tangent space <em>is</em> the space, the step is plain addition, and there is nowhere to step off to.</p><p>This is the yardstick every curved case gets measured against — which is why it is worth starting here.</p>',
          },
          so2: {
            title: 'SO(2) — rotation in the plane',
            summary:
              '<p>The first curved space, and the gentlest: it is <em>commutative</em>. There is no body/world distinction here yet.</p><p>That is exactly its use — when the distinction shows up in SO(3), you will know it came from non-commutativity, not from curvature.</p>',
          },
          se2: {
            title: 'SE(2) — motion in the plane',
            summary:
              '<p>Rotation and translation together, in two dimensions. The first case where the two components talk to each other.</p><p>This is where order starts to matter: rotate-then-translate is not translate-then-rotate.</p>',
          },
          so3: {
          ...
          se3: {
            title: 'SE(3) — the pose',
            summary:
              '<p>Rotation + translation: 6 degrees of freedom, and the geometric skeleton of SLAM.</p><p>Ten stations: the pose/transformation duality, chaining reference frames, the order of R and t, the screw motion, the left Jacobian, the adjoint and interpolation.</p>',
          },
          sim3: {
            title: 'Sim(3) — similarity',
            summary:
      ...
      optimization: {
        title: 'Optimization',
        summary:
          '<p>How we minimize a cost: gradient descent, then Gauss–Newton, then the practical variants. All of it lives happily in flat ℝ<sup>n</sup> — no manifold required.</p>',
        moons: {
          gd: {
            title: 'Gradient descent',
            summary:
              '<p>The valley, the gradient and the iteration — all in flat ℝ<sup>2</sup>, worked through in closed form.</p><p>This is the engine the Riemannian gradient later lifts onto curved SO(3). First watch it run where nothing complicates it.</p>',
          },
          gn: {
            title: 'Gauss–Newton',
            summary:
              '<p>Linearize the residual, and see what falls out: <span class="m">H δ = −g</span>.</p><p>Not a recipe — the bottom of the parabola. And because it uses the curvature and not just the direction, it is far faster than gradient descent.</p>',
          },
          lm: {
            title: 'LM · robust',
            summary:
              '<p>When the Gauss–Newton step is too bold: damping by a λ that tunes between the two methods.</p><p>And robust kernels — because if the noise is not Gaussian, the sum of squares is not imprecise, it is the wrong objective.</p>',
          },
        },
      },
      slam: {
        title: 'SLAM',
        summary:
          '<p>Where geometry and optimization meet. Riemannian GD and factor graphs — the residuals and weights a SLAM problem is built from — converge here into full SLAM.</p>',
        moons: {
          riemann: {
            title: 'Riemannian GD',
            summary:
              '<p>The first real collision: flat gradient descent meets curved SO(3).</p><p>The constraint, the projection of the raw gradient, exp as the way back onto the surface, and the iteration running on the manifold — the whole chain, in one arc.</p>',
          },
          fg: {
            title: 'Factor graphs',
            summary:
              '<p>The graph of the measurements: variable nodes and factor nodes. Not a separate formalism — <em>it is the picture of MAP estimation</em>.</p><p>And here is where it becomes clear why a large problem is solvable at all: the graph is sparse.</p>',
          },
          slam: {
            title: 'SLAM',
            summary:
              '<p>The whole chain at once: poses, landmarks, loop closure and scale.</p><p>Everything that ran on separate branches meets here — and the difficulties turn out not to add up, but to be consequences of the same few claims.</p>',
          },
        },
      },
    },
