    ...
    branches: {
      geometry: { title: 'Geometry', summary: '<p>The spaces things live in — rotations, poses, similarities — and how they move. Six journeys from flat ℝⁿ up to Sim(3): tangent spaces, exp/log, and (for SO(3)) topology. No cost function in sight.</p>',
        moons: {
          ...
          so3: { title: 'SO(3) — rotations in 3D', summary: '<p>The space that justifies the whole theory. Non-commutative, compact, and its true shape is not a sphere but ℝP³.</p><p>The impossibility theorem lives here too: there is no global, singularity-free three-number coordinate — gimbal lock is a consequence, not an annoyance.</p>' },
      ...
      optimization: { title: 'Optimization', summary: '<p>How we minimize a cost: gradient descent, then Gauss–Newton, then the practical variants. All of it lives happily in flat ℝⁿ — no manifold required.</p>',
        moons: {
          gd: { title: 'Gradient descent', summary: '<p>The valley, the gradient and the iteration — all in flat ℝ², worked through in closed form.</p><p>This is the engine the Riemannian gradient later lifts onto curved SO(3). First watch it run where nothing complicates it.</p>' },
