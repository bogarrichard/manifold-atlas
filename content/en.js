...
window.LIE_CONTENT.en = {
  ...
  cards: [
   ...
   {t:'The iteration', b:'<p>The machine, driven beat by beat. The colors are the cost (warm = large error), the green dot is the minimum. The button walks through one iteration in three beats: tangent plane and projected gradient → exp arc → displacement and a new anchor.</p><p>What is <span class="m">δ</span>? The step on the tangent plane, <em>three ordinary numbers</em>: <span class="m">δ = (δ<sub>x</sub>, δ<sub>y</sub>, δ<sub>z</sub>)</span> — how much to turn about each axis. The “hat” <span class="m">δ<sup>∧</sup></span> arranges these three numbers into a skew-symmetric matrix so that exp can take them in:</p><p style="text-align:center;font-family:Georgia,serif;font-style:italic;color:var(--ink);font-size:13px">δ<sup>∧</sup> = [ 0, −δ<sub>z</sub>, δ<sub>y</sub> ; δ<sub>z</sub>, 0, −δ<sub>x</sub> ; −δ<sub>y</sub>, δ<sub>x</sub>, 0 ]</p><p>So <span class="m">⊞</span> is entirely concrete: <span class="m">x ⊞ δ := x · exp(δ<sup>∧</sup>)</span> — a generalization of ordinary <span class="m">+</span>. In flat space point + vector = point; here manifold point + tangent vector = manifold point. At the end of every round <span class="m">δ</span> resets to zero: we spread the map out afresh at the new estimate.</p><div class="acts"><button class="act" id="s5step">1/3 · tangent plane</button><button class="act" id="s5reset">Reset</button></div><div class="ro" id="s5ph">anchor at the estimate — δ = 0</div><div class="ro">iteration: <b id="s5it">0</b> &nbsp;·&nbsp; L = <b id="s5L">—</b></div>'},
