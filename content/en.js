...
window.LIE_CONTENT.en = {
  ...
  cards: [
   {t:'The Journey', b:'<p>Five conversations’ worth of derivation, in a single space. This journey is the <em>convergence</em>: flat optimization and curved <span class="m">SO(3)</span> meet here — eight stations, from the constraint through the projection and <span class="m">exp</span> to the topology.</p><p>The flat groundwork is built up by the <em>Geometry · ℝⁿ</em> and <em>Optimization · gradient descent</em> journeys; here we only recall it. The violet thread is the line of thought — you’ll see it as you fly between the stations.</p><p>Navigation: ← → buttons or keys, drag with the mouse to look around, scroll to zoom.</p>'},
   {t:'The Flat World', b:'<p>The familiar recipe, in one paragraph: the parameter space is <span class="m">ℝⁿ</span>, the cost is a valley, and the <span class="m">w − α∇L</span> step always stays valid — there is nowhere to step off to.</p><p>The <em>Geometry · ℝⁿ</em> and <em>Optimization · gradient descent</em> journeys build this up properly. Only one thing from it matters here: <strong>the gradient lives in the same space as the point.</strong> That is exactly what breaks at the next station.</p>'},
   ...
   {t:'SE(3) — A Pose in Time', b:'<p>Rotation + translation = pose: 6 degrees of freedom, a moving reference frame along the path. <span class="m">⊞</span> is the same recipe, only the tangent space is <span class="m">ℝ⁶</span>: three angles, three translations.</p><p>Poses have a chapter of their own: the <em>Geometry · SE(3)</em> journey takes ten stations over it — chaining, the order of R and t, the screw motion, the left Jacobian, the adjoint and interpolation.</p><p>This is where SLAM begins: many such poses linked by factors, with the previous station’s iteration running on them.</p>'}
  ]
};
