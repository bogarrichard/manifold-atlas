...
LIE.hub = (function () {
  ...
  function run(ctx) {
    ...
    const DEF = {
      eb: HUB.eyebrow || '',
      ti: HUB.title || (C.meta && C.meta.title) || '',
      bo: HUB.intro || '',
    };
    ...
    function setCard(br, moonUD) {
      const key = moonUD
        ? 'm:' + moonUD.branchId + ':' + moonUD.journey.k
        : br
          ? 'b:' + br.id
          : null;
      ...
      if (moonUD) {
        ...
        eb.textContent = HUB.branchWord || DEF.eb;
        ti.textContent = info.title || br.title;
        bo.innerHTML = info.summary || '';
      } else {
        eb.textContent = DEF.eb;
        ti.textContent = DEF.ti;
        bo.innerHTML = DEF.bo;
      }
    ...
    function firstLive(br) {
      for (const j of br.journeys) {
        if (j.journey) return byKey[br.id + ':' + j.k];
      }
      return null;
    }
    ...
    function build() {
      ...
      [
        [V3(2.4, 0, 0), PAL.coral],
        [V3(0, 2.4, 0), PAL.teal],
        [V3(0, 0, 2.4), PAL.violet],
      ].forEach(([v, c]) => {
        const a = fatArrow(c, 0.065);
      ...
      ecliptic.rotation.set(0.2, 0, 0.05);
      world.add(ecliptic);
      byKey = {};
      ...
      BRANCHES.forEach(br => {
        ...
        const core = new THREE.Mesh(
          new THREE.SphereGeometry(coreR, 26, 20),
          new THREE.MeshBasicMaterial({color: col, transparent: true, opacity: 0.82})
        );
        core.userData = {isPlanet: true, branch: br, branchId: br.id, planet, h: 0};
        planet.add(core);
        cores.push(core);
        pickables.push(core);
        planet.add(
          new THREE.Mesh(
            new THREE.SphereGeometry(coreR * 1.35, 20, 16),
        ...
        const blabel = makeLabel(brInfo(br.id).title || br.title, hexStr(PAL.ink), 9.5, {
          halo: true,
        ...
        ringG.rotation.set(br.tilt[0], br.tilt[1], 0);
        planet.add(ringG);
        planet.userData.ringG = ringG;
        ringG.add(
          new THREE.Line(
            new THREE.BufferGeometry().setFromPoints(ringPts(ringR, 80)),
        ...
        br.journeys.forEach((j, idx) => {
          ...
          const sphere = new THREE.Mesh(
            new THREE.SphereGeometry(0.5, 20, 15),
            new THREE.MeshBasicMaterial({
              color: col,
              transparent: true,
              opacity: live ? 0.97 : 0.42,
            })
          );
          anchor.add(sphere);
          anchor.add(
          ...
          const labelBase = String(idx + 1),
            labelFull = idx + 1 + ': ' + j.title;
          ...
          title.userData.labelBase = labelBase;
          title.userData.labelFull = labelFull;
          title.userData.labelHaloColor = haloColor;
          title.userData.labelState = 'base';
          anchor.add(title);
    ...
    function enterPlanet(planet, idx) {
      ...
      noteActivity();
      mode = 'planet';
      focus = {planet, br, idx: idx || 0};
      inFocus = new Set();
      planet.traverse(o => inFocus.add(o));
    ...
    function exitPlanet() {
      ...
      inFocus = new Set();
      aimAtSel();
      setCard(selPlanet.userData.branch, null);
      syncFocus();
    }
    ...
    function step(d) {
      if (mode !== 'planet') return;
      const n = moonsOf(focus.br).length;
      if (!n) return;
    ...
    const planetAzimuth = p => {
      p.getWorldPosition(_az);
    ...
    function enterTarget() {
      if (mode === 'planet') {
        const m = selMoon();
        return m && m.userData.live ? m : null;
      }
    ...
    function syncFocus() {
      ...
      enterBtn.onclick = () => {
        const t = enterTarget();
        if (!t) return;
        if (mode === 'planet') enter(t);
        else enterPlanet(t, 0);
      };
      ...
      if (mode === 'planet') {
        ...
        ms.forEach((sp, i) => {
          ...
          d.type = 'button';
          d.title = sp.userData.journey.title;
          d.className =
            'dot' + (i === focus.idx ? ' on' : '') + (sp.userData.live ? '' : ' locked');
          d.setAttribute(
            'aria-label',
            (HUB.moonWord || '') +
              ' ' +
        ...
        planets.forEach((p, i) => {
          const d = document.createElement('button');
          d.type = 'button';
          d.title = p.userData.branch.title;
    ...
    canvas.addEventListener('pointermove', e => {
      ...
      if (dragging) {
        ...
        if (moved) {
          theta -= dx * 0.006;
          phi = clamp(phi - dy * 0.006, 0.22, Math.PI - 0.22);
        }
        lx = e.clientX;
        ly = e.clientY;
      }
    ...
    navWrap.querySelector('#prev').onclick = () => {
      if (mode === 'planet') step(-1);
      else stepPlanet(-1);
    };
    navWrap.querySelector('#next').onclick = () => {
      if (mode === 'planet') step(1);
      else stepPlanet(1);
    };
    ...
    function pick() {
      ...
      const pool =
        mode === 'planet'
          ? pickables.filter(o => inFocus.has(o) || o.userData.isPlanet)
          : pickables;
    ...
    (function restoreFocus() {
      ...
      const brId = qs.get('planet');
      if (!brId) return;
      const br = BRANCHES.find(b => b.id === brId);
      if (!br) return;
      const planet = planets.find(p => p.userData.branch === br);
      if (!planet) return;
      const moonKey = qs.get('moon');
      let idx = 0;
      if (moonKey) {
        const i = br.journeys.findIndex(j => j.k === moonKey);
        if (i >= 0) idx = i;
      }
      mode = 'planet';
      focus = {planet, br, idx};
    ...
    function loop() {
      ...
      planets.forEach(p => {
        ...
        p.position.copy(orbitPos(o, o.ph + orbitTime * o.w));
        const rg = p.userData.ringG;
        if (!rg) return;
        if (focus && p === focus.planet) {
          ...
          rg.rotation.set(
            lerp(p.userData.tilt[0], PRESENT_TILT, focusT),
            lerp(p.userData.tilt[1], 0, focusT),
            spinCur
          );
        } else {
      ...
      threads.forEach(th => {
        th.sa.getWorldPosition(_a);
        th.sb.getWorldPosition(_b);
      ...
      spheres.forEach(sp => {
        ...
        if (u.title.userData.labelState !== wantState) {
          const text =
            wantState === 'full' ? u.title.userData.labelFull : u.title.userData.labelBase;
          updateLabel(u.title, text, hexStr(PAL.ink), {
            halo: true,
            haloColor: u.title.userData.labelHaloColor,
          });
    ...
    function dimPass(dt) {
      ...
      world.traverse(o => {
        ...
        const factor = mine ? 1 : o.userData && o.userData.isPlanetLabel ? 1 - k : 1 - k * 0.94;
  ...
  function order() {
    const out = [];
    BRANCHES.forEach(br =>
      br.journeys.forEach(j => {
        if (j.journey) out.push(j.journey);
      })
    );
    return out;
  }
  ...
  function nextMoon(journeyId) {
    for (const br of BRANCHES) {
      ...
      if (idx < 0) continue;
      for (let i = idx + 1; i < br.journeys.length; i++) {
        if (br.journeys[i].journey) return br.journeys[i].journey;
      }
      return null;
    }
    return null;
