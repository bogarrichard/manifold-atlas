...
LIE.journeys['geometry-se3'] = (function(){
  ...
  function build(C, PAL){
    ...
    let jacApi = null, interpApi = null;
    ...
    const stations = [
      ...
      /* 1 · pose or transformation: the columns of T are where the world basis vectors
             land. One basis vector at a time flies from the world frame onto the body
             axis it becomes, so "the matrix" and "where the frame stands" become one thing. */
      g=>{
        grid(g);
        const T = pose(V3(2.0,0.8,-0.7), V3(0.18,0.95,0.28));
        g.add(gizmo(1.35, 0.04, 0.3));                                   // W, with its basis
        // B is drawn faint: the bright flying arrow has to be seen landing *onto* its axis
        const body = gizmo(1.25, 0.05, 0.45); setPose(body, T); g.add(body);
        const lw = makeLabel('W', HX.ink, 1.2); lw.position.set(0.55,1.5,0); g.add(lw);
        const lbdy = makeLabel('B', HX.ink, 1.2);
        lbdy.position.copy(T.p).add(V3(0,1.7,0)); g.add(lbdy);
        const tArr = dimArrow(COL.amber, 0.032, 0.65); setArrow(tArr, V3(0,0,0), T.p); g.add(tArr);
        const lt = makeLabel('t', HX.amber, 1.0);
        lt.position.copy(T.p).multiplyScalar(0.5).add(V3(0,0.34,0)); g.add(lt);
        const AX = [V3(1,0,0), V3(0,1,0), V3(0,0,1)], CO = [COL.coral, COL.teal, COL.violet];
        const TX = ['R e1', 'R e2', 'R e3'];
        const fly = fatArrow(COL.coral, 0.06); g.add(fly);
        const lbl = makeLabel(TX[0], HX.coral, 2.0); g.add(lbl);
        const PER = 2.8;
        let cur = -1;
        return {tick(t){
          const k = Math.floor((t/PER)%3), u = ease(clamp(((t%PER)-0.35)/1.9, 0, 1));
          if(k !== cur){ cur = k;
            updateLabel(lbl, TX[k], hexStr(CO[k]));
            fly.userData.cyl.material.color.setHex(CO[k]);                // cyl and cone share it
          }
          // the basis vector is carried from the world origin onto the body axis it becomes
          const base = V3(0,0,0).lerp(T.p, u);
          const vec = AX[k].clone().multiplyScalar(1.35)
                        .lerp(AX[k].clone().applyQuaternion(T.q).multiplyScalar(1.35), u);
          setArrow(fly, base, vec);
          lbl.position.copy(base).add(vec).add(V3(0,0.34,0));
        }};
      },

      /* 2 · chaining as re-reading one point: the landmark never moves, only the frame we
             read it in. The staircase draws the three components along that frame's axes,
             and the numbers on the label are the coordinates it is spelling out. */
      g=>{
        grid(g);
        // the frames sit left of the world origin, which the HUD card covers, so the whole
        // scene is shifted right. A uniform shift leaves every read-out coordinate unchanged.
        const S = new THREE.Group(); S.position.set(1.7,-0.3,0); g.add(S); g = S;
        ...
        [[W,'W'],[TWB,'B'],[TWC,'C']].forEach(([T,name])=>{
          const gz = gizmo(0.95, 0.038, 0.45); setPose(gz, T); g.add(gz);
          const l = makeLabel(name, HX.ink, 0.9); l.position.copy(T.p).add(V3(-0.35,1.25,0)); g.add(l);
        });
        // one 2-point line per axis component, redrawn as the staircase grows
        const legs = [polyline(2, COL.coral, 0.95), polyline(2, COL.teal, 0.95),
                      polyline(2, COL.violet, 0.95)];
        legs.forEach(l=>g.add(l));
        const read = makeLabel('p(W)', HX.green, 3.6); g.add(read);
        const step = makeLabel(' ', HX.amber, 2.2); g.add(step);
        const PH = [{T:TWC, n:'C', s:''}, {T:TWB, n:'B', s:'B ← C'}, {T:W, n:'W', s:'W ← B'}];
        const PER = 3.4;
        let cur = -1;
        function reveal(T, w){
          const c = P.clone().sub(T.p).applyQuaternion(T.q.clone().conjugate());   // local coords
          const dirs = [V3(1,0,0),V3(0,1,0),V3(0,0,1)].map(e=>e.applyQuaternion(T.q));
          const comp = [c.x, c.y, c.z];
          let from = T.p.clone();
          for(let i=0;i<3;i++){
            const f = clamp(w*3 - i, 0, 1);
            const to = from.clone().addScaledVector(dirs[i], comp[i]*f);
            legs[i].userData.set([from, to]);
            from = from.clone().addScaledVector(dirs[i], comp[i]);
          }
          return c;
        }
        return {tick(t){
          const k = Math.floor((t/PER)%3), tc = t%PER;
          const w = ease(clamp((tc-0.4)/1.9, 0, 1));
          const c = reveal(PH[k].T, w);
          if(k !== cur){
            cur = k;
            updateLabel(read, 'p('+PH[k].n+') = ('+c.x.toFixed(1)+', '+c.y.toFixed(1)+', '+c.z.toFixed(1)+')', HX.green);
            updateLabel(step, PH[k].s || ' ', HX.amber);
            step.visible = !!PH[k].s;
          }
          read.position.copy(P).add(V3(0,0.75,0));
          step.position.copy(PH[k].T.p).add(V3(0,1.75,0));
        }};
      },
  ...
  return {
    ...
    cards: {
      hu: [
        {t:'A póz', b:'<p>Egy <em>póz</em> arra válaszol, hogy hol áll és merre néz egy keret egy másikhoz képest. Két adat írja le: egy <span class="m">R ∈ SO(3)</span> forgatás és egy <span class="m">t ∈ ℝ³</span> eltolás. Egy pont átszámolása a keretek között ennyi:</p><p class="matline"><span class="m">p<sub>W</sub> = R p<sub>B</sub> + t</span></p><p>A kettőt egyetlen <button class="termbtn" id="se3HomInfo" type="button" aria-expanded="false" aria-controls="se3HomNote">4×4-es</button> mátrixba rendezzük:</p><p class="matline"><span class="m">T =</span><span class="matrix"><span class="mbracket"></span><span class="mgrid" style="grid-template-columns:repeat(2,auto)"><span>R</span><span>t</span><span>0<sup>⊤</sup></span><span>1</span></span><span class="mbracket right"></span></span><span class="m">∈ SE(3)</span></p><p>Hat szabadsági fok — három szög, három eltolás —, de tizenkét tárolt szám. A különbséget az <span class="m">R<sup>⊤</sup>R = I</span> kényszer nyeli el, és pontosan ez teszi görbültté a teret: két pózt <em>nem</em> lehet elemenként átlagolni, mert az eredmény kiesne <span class="m">SE(3)</span>-ból.</p><p>SLAM-szótárban minden kulcskép egy póz, a térkép pontjai a világ keretben élnek, a kamera–IMU <em>extrinszik</em> szintén egy póz. A SLAM persze ennél jóval több — becslés, adattársítás, hurokzárás, optimalizálás —, de a geometriai váza végig ez az átszámolás, és minden más erre épül.</p><div class="bubble" id="se3HomNote" role="dialog" aria-label="Miert 4x4" hidden><p>A trükk a <em>homogén koordináta</em>: a három szám mellé írunk egy negyediket, és ez a negyedik mondja meg, hogy a hármas mit jelent.</p><p><strong>1</strong> a végén: <em>pont</em>. Egy hely a térben, van origótól mért helye.</p><p><strong>0</strong> a végén: <em>irányvektor</em>. Nem hely, hanem egy nyíl, amiből csak az irány és a hossz számít, a kezdőpontja nem. Ilyen egy látósugár iránya, egy felületi normális vagy egy sebességvektor. Fontos: itt nem forgatásról van szó, az az <span class="m">R</span> dolga, hanem egy közönséges háromelemű vektorról.</p><p class="matline"><span class="m">T</span><span class="matrix"><span class="mbracket"></span><span class="mgrid" style="grid-template-columns:auto"><span>p</span><span>1</span></span><span class="mbracket right"></span></span><span class="m">=</span><span class="matrix"><span class="mbracket"></span><span class="mgrid" style="grid-template-columns:auto"><span>Rp + t</span><span>1</span></span><span class="mbracket right"></span></span><span class="m">,&nbsp;&nbsp; T</span><span class="matrix"><span class="mbracket"></span><span class="mgrid" style="grid-template-columns:auto"><span>v</span><span>0</span></span><span class="mbracket right"></span></span><span class="m">=</span><span class="matrix"><span class="mbracket"></span><span class="mgrid" style="grid-template-columns:auto"><span>Rv</span><span>0</span></span><span class="mbracket right"></span></span></p><p>Az irányvektorból tehát kiesik az eltolás, és ez így helyes. Ha a kamerát elforgatjuk, a látósugár iránya elfordul. Ha viszont csak arrébb visszük két méterrel, a nyíl iránya változatlan marad. Egy pontnak van „hol”-ja, egy iránynak nincs.</p><p>A ráragasztott <span class="m">[0 0 0 1]</span> sor pedig nem dísz: ettől lesz két póz szorzata is póz. Így a láncolás közönséges mátrixszorzás, van egységelem és van inverz, vagyis a pózok <em>csoportot</em> alkotnak. Ez az <span class="m">SE(3)</span>.</p></div>'},

        {t:'Póz vagy transzformáció?', b:'<p>Ugyanaz a 4×4-es mátrix két dolgot jelent egyszerre, és érdemes tisztázni, mikor melyiket mondjuk.</p><p><strong>Keretváltás.</strong> A pont mozdulatlan, csak más keret koordinátáiban írjuk le. Ez a <span class="m">p<sub>W</sub> = R p<sub>B</sub> + t</span> képlet, semmi nem mozdul el, csak a leolvasás módja változik.</p><p><strong>Póz.</strong> Hol áll a <span class="m">B</span> keret <span class="m">W</span>-ben. És most jön a lényeg: ez a kettő <em>ugyanaz a mátrix</em>, nem véletlen egybeesés. Nézzük meg, mit mond a keretváltás a <span class="m">B</span>-beli bázisvektorokról:</p><p class="matline"><span class="m">e<sub>1</sub> ↦ R e<sub>1</sub> + t,&nbsp;&nbsp; e<sub>2</sub> ↦ R e<sub>2</sub> + t,&nbsp;&nbsp; e<sub>3</sub> ↦ R e<sub>3</sub> + t</span></p><p><span class="m">R</span> <button class="termbtn" id="se3ColInfo" type="button" aria-expanded="false" aria-controls="se3ColNote">oszlopai</button> tehát pontosan <span class="m">B</span> tengelyei, <span class="m">W</span> koordinátáiban kifejezve, <span class="m">t</span> pedig <span class="m">B</span> origója <span class="m">W</span>-ben. A mátrix négy oszlopa szó szerint lerajzolja a keretet, és a jelenetben pont ezt látod: a bázisvektorok átrepülnek oda, ahol a testhez rögzített tengelyek állnak.</p><p><strong>Mozgás.</strong> Van egy harmadik olvasat is: fogj egy testet, és told-forgasd el. Ilyenkor <span class="m">T</span> nem egy keret <em>helyzete</em>, hanem egy <em>elmozdulás</em>. A számolás ugyanaz, a jelentés más, ezért érdemes külön nevet adni neki — relatív póz, elmozdulás, <span class="m">ΔT</span>.</p><p>A jelölés ezt viszi végig. A <span class="m">T<sub>WB</sub></span> olvasható úgy, hogy „<span class="m">B</span> póza <span class="m">W</span>-ben”, és úgy is, hogy „<span class="m">B</span>-koordinátákat <span class="m">W</span>-koordinátákká alakít”. A két mondat ugyanazt jelenti. Ha viszont <span class="m">ΔT</span>-t írunk, akkor mozgásról beszélünk, és ott mindig meg kell mondani, melyik kerethez képest — erről szól majd az adjungált.</p><div class="bubble" id="se3ColNote" role="dialog" aria-label="Miert az oszlopok" hidden><p>Egy mátrix <span class="m">i</span>-edik oszlopa definíció szerint az, amit az <span class="m">i</span>-edik bázisvektoron kapunk:</p><p class="matline"><span class="m">R e<sub>1</sub> = az R első oszlopa</span></p><p>Ez azért van így, mert <span class="m">e<sub>1</sub> = (1, 0, 0)</span>, a mátrixszorzás pedig az oszlopok súlyozott összege. Itt a súlyok <span class="m">1, 0, 0</span>, vagyis pontosan az első oszlop marad.</p><p>Szemléletesen: ha a <span class="m">B</span> keret <span class="m">x</span> tengelye mentén lépünk egy egységnyit, a világban az <span class="m">R</span> első oszlopával lépünk. A keret három tengelye és az origója együtt éppen a <span class="m">T</span> négy oszlopa — ezért mondjuk, hogy a póz „benne van” a mátrixban.</p></div>'},

        {t:'Keretek láncolása', b:'<p>Ha kiírjuk, melyik keretből melyikbe visz az átszámolás, a láncolás magától olvasható, mert a belső indexek kiejtik egymást:</p><p class="matline"><span class="m">T<sub>WC</sub> = T<sub>WB</sub> · T<sub>BC</sub></span></p><p>Jobbról balra kell olvasni: előbb a kamera keretéről a testére, majd a testéről a világéra váltunk.</p><p>A jelenetben a zöld pont végig ugyanott áll, meg sem rezdül. Csak azt cseréljük, melyik keret tengelyei mentén olvassuk le, és a lépcső ezt a leolvasást rajzolja meg, komponensenként. A három számhármas mind más, a pont mégis ugyanaz. Ez a keretváltás teljes tartalma.</p><p>A gyakorlatban <span class="m">T<sub>BC</sub></span> az extrinszik, merev és nem változik, <span class="m">T<sub>WB</sub></span> pedig a mozgó test póza. A kettő szorzata a kamera póza a világban, és ugyanez a mátrix számolja át a kamera koordinátáit a világéra.</p><p>A <button class="termbtn" id="se3InvInfo" type="button" aria-expanded="false" aria-controls="se3InvNote">megfordítás</button> pedig nem egyszerű transzponálás:</p><p class="matline"><span class="m">T<sup>−1</sup> =</span><span class="matrix"><span class="mbracket"></span><span class="mgrid" style="grid-template-columns:repeat(2,auto)"><span>R<sup>⊤</sup></span><span>−R<sup>⊤</sup>t</span><span>0<sup>⊤</sup></span><span>1</span></span><span class="mbracket right"></span></span></p><p>A forgatásnál elég a transzponálás, mert ortogonális mátrix. Az eltolást viszont vissza is kell forgatni, különben a régi keret tengelyei szerint tolnánk. SLAM-ben ez a mindennapi művelet, az odometria láncokat fűz, a relatív póz pedig <span class="m">T<sub>ab</sub> = T<sub>a</sub><sup>−1</sup>T<sub>b</sub></span>.</p><p>Egy dolgot viszont nem szabad elfelejteni: a szorzás sorrendje nem cserélhető fel. Erről szól a következő állomás.</p><div class="bubble" id="se3InvNote" role="dialog" aria-label="Az inverz levezetese" hidden><p>Két sor az egész. Induljunk a definícióból:</p><p class="matline"><span class="m">q = R p + t</span></p><p>Fejezzük ki <span class="m">p</span>-t. Vonjuk ki <span class="m">t</span>-t, majd szorozzunk balról <span class="m">R<sup>−1</sup> = R<sup>⊤</sup></span>-tal:</p><p class="matline"><span class="m">p = R<sup>⊤</sup>(q − t) = R<sup>⊤</sup>q − R<sup>⊤</sup>t</span></p><p>Ez pontosan egy póz alakja, forgatása <span class="m">R<sup>⊤</sup></span>, eltolása <span class="m">−R<sup>⊤</sup>t</span>. Szemléletesen: <span class="m">−R<sup>⊤</sup>t</span> nem más, mint a <em>régi origó helye az új keretben</em>.</p></div>'},

        {t:'A sorrend', b:'<p>A blokkalak maga is sorrendet kódol. A <span class="m">p ↦ Rp + t</span> képlet azt mondja, hogy <em>előbb forgatunk, aztán tolunk</em>. Ha fordítva csinálnánk, előbb tolnánk és utána forgatnánk, akkor <span class="m">R(p + t) = Rp + Rt</span> jönne ki, aminek az eltolása már <span class="m">Rt</span>, nem <span class="m">t</span>.</p><p>Ez nem szőrszálhasogatás, hanem magának a nem kommutativitásnak a lényege:</p><p class="matline"><span class="m">T(t) T(R) ≠ T(R) T(t)</span></p><p>A jelenetben mindkét test ugyanazt a két mozgást végzi el, csak más sorrendben. A teál előbb megfordul a helyén, aztán elindul. A lila előbb elindul, és csak utána fordul — csakhogy a forgatás a <em>világ</em> origója körül forgat, ezért a már eltolt testet körbelendíti. A két végpont láthatóan nem ugyanaz, és nemcsak máshol van, hanem merőleges irányban.</p><p>Miért a világ origója körül? Mert a bal oldali szorzás a világ keretében ható mozgást jelent. Ha a testhez rögzített tengely körül akarnánk forgatni, azt jobbról kell szorozni. <button class="termbtn" id="se3OrdInfo" type="button" aria-expanded="false" aria-controls="se3OrdNote">Számokkal</button> mindkettő két sor.</p><p>Ez a kettősség végigkíséri a SLAM-et. A becslés perturbációja lehet baloldali (világ szerinti, más néven globális) vagy jobboldali (test szerinti, más néven lokális), és a Jacobi-mátrixok alakja is emiatt tér el könyvről könyvre. Nem másról van szó, csak arról, melyik oldalról szorzunk.</p><div class="bubble" id="se3OrdNote" role="dialog" aria-label="A sorrend szamokkal" hidden><p>Legyen <span class="m">T(t)</span> tiszta eltolás <span class="m">t = (2, 0, 0)</span>-val, <span class="m">T(R)</span> pedig 90°-os forgatás a <span class="m">z</span> tengely körül:</p><p class="matline"><span class="m">R =</span><span class="matrix"><span class="mbracket"></span><span class="mgrid"><span>0</span><span>−1</span><span>0</span><span>1</span><span>0</span><span>0</span><span>0</span><span>0</span><span>1</span></span><span class="mbracket right"></span></span></p><p>A 4×4-es szorzás blokkokban egyetlen szabály:</p><p class="matline"><span class="m">[R<sub>1</sub>, t<sub>1</sub>] · [R<sub>2</sub>, t<sub>2</sub>] = [R<sub>1</sub>R<sub>2</sub>,&nbsp; R<sub>1</sub>t<sub>2</sub> + t<sub>1</sub>]</span></p><p>vagyis a jobb oldali eltolást mindig megforgatja a bal oldali forgatás. Behelyettesítve a két sorrendet:</p><p class="matline"><span class="m">T(t)T(R) = [R,&nbsp; t],&nbsp;&nbsp;&nbsp; T(R)T(t) = [R,&nbsp; R t]</span></p><p>A forgatásrész mindkettőben ugyanaz. Az eltolás viszont nem:</p><p class="matline"><span class="m">t = (2, 0, 0),&nbsp;&nbsp; R t = (0, 2, 0)</span></p><p>Ugyanaz a két mozgás, más sorrend, és a végpont nemcsak arrébb került, hanem merőleges irányba.</p></div>'},

        {t:'A csavarsebesség', b:'<p>Hogyan <em>mozog</em> egy merev test? Vegyük a sebességét minden pontjában. A merevség kényszere brutálisan leszűkíti a lehetőségeket, az egész mezőt két vektor megadja:</p><p class="matline"><span class="m">v(x) = ω × x + u</span></p><p>egy <span class="m">ω</span> szögsebesség (a korall nyíl) és egy <span class="m">u</span> haladó sebesség (teál). A képen minden apró nyíl ebből a két vektorból számolt, hat szám és semmi több. A gúlának azért van csúcsa, hogy lássuk, merre néz a test.</p><p>A <button class="termbtn" id="se3SkewInfo" type="button" aria-expanded="false" aria-controls="se3SkewNote">vektoriális szorzat</button> lineáris <span class="m">x</span>-ben, tehát mátrixként is írható, és ez a mátrix ferdén szimmetrikus. Ez a „kalap” művelet:</p><p class="matline"><span class="m">φ<sup>∧</sup> =</span><span class="matrix"><span class="mbracket"></span><span class="mgrid"><span>0</span><span>−φ<sub>3</sub></span><span>φ<sub>2</sub></span><span>φ<sub>3</sub></span><span>0</span><span>−φ<sub>1</sub></span><span>−φ<sub>2</sub></span><span>φ<sub>1</sub></span><span>0</span></span><span class="mbracket right"></span></span></p><p>A hat számot egyetlen vektorba fogjuk, és a bevett elnevezés ez: a haladó rész <span class="m">ρ := u</span>, a forgó rész <span class="m">φ := ω</span>.</p><p class="matline"><span class="m">ξ = (ρ, φ) ∈ ℝ<sup>6</sup></span></p><p>A kalap 4×4-es alakja ugyanaz a doboz, amiben a póz is él:</p><p class="matline"><span class="m">ξ<sup>∧</sup> =</span><span class="matrix"><span class="mbracket"></span><span class="mgrid" style="grid-template-columns:repeat(2,auto)"><span>φ<sup>∧</sup></span><span>ρ</span><span>0<sup>⊤</sup></span><span>0</span></span><span class="mbracket right"></span></span></p><p>Ez az érintőtér az egységelemnél, a neve <span class="m">se(3)</span>. Itt már <em>szabad összeadni és skálázni</em>, pózokat nem lehet, sebességeket igen. Ezért él minden becslés, gradiens és kovariancia ebben a hatdimenziós lapos térben, nem a görbült sokaságon.</p><div class="bubble" id="se3SkewNote" role="dialog" aria-label="Ferde szimmetria es ortogonalitas" hidden><p>Deriváljuk a forgatások kényszerét egy <span class="m">R(t)</span> mozgás mentén:</p><p class="matline"><span class="m">R<sup>⊤</sup>R = I  ⟹  Ṙ<sup>⊤</sup>R + R<sup>⊤</sup>Ṙ = 0</span></p><p>Vagyis az <span class="m">Ω = R<sup>⊤</sup>Ṙ</span> mátrixra <span class="m">Ω<sup>⊤</sup> = −Ω</span>, ferdén szimmetrikus. Egy ilyen mátrixnak három szabad eleme van, épp a szögsebesség három komponense, és a hatása pontosan a vektoriális szorzat: <span class="m">Ω x = ω × x</span>.</p><p><strong>És mi köti össze a ferde szimmetriát az ortogonalitással?</strong> Az exponenciális, méghozzá két sorban. Ha <span class="m">Ω</span> ferdén szimmetrikus, akkor</p><p class="matline"><span class="m">(exp Ω)<sup>⊤</sup> = exp(Ω<sup>⊤</sup>) = exp(−Ω) = (exp Ω)<sup>−1</sup></span></p><p>ami pontosan az ortogonalitás feltétele. A determináns pedig</p><p class="matline"><span class="m">det(exp Ω) = e<sup>tr Ω</sup> = e<sup>0</sup> = 1</span></p><p>mert egy ferdén szimmetrikus mátrix átlója csupa nulla. Tehát nem tükrözést kapunk, hanem valódi forgatást.</p><p>Röviden: a ferdén szimmetrikus mátrixok a <em>végtelenül kicsi</em> forgatások, az ortogonálisak a <em>végesek</em>, és az <span class="m">exp</span> pontosan az egyikből a másikba visz. A <span class="m">log</span> visszafelé ugyanezt teszi.</p></div>'},

        {t:'exp — a csavarmozgás', b:'<p>Tartsuk a <span class="m">ξ</span> sebességet állandónak, és hagyjuk működni <em>egységnyi</em> ideig. Az eredmény az exponenciális leképezés:</p><p class="matline"><span class="m">T = exp(ξ<sup>∧</sup>)</span></p><p>Érdemes megállni annál, mi is történik itt. A <span class="m">ξ<sup>∧</sup></span> elsőre egy absztrakt 4×4-es mátrix, csupa nulla meg kereszttag, semmit nem mond. Valójában viszont nagyon is kézzelfogható: <em>ez maga a mozgás</em>, egyenletes sebességgel leírva. Az <span class="m">exp</span> pedig nem csinál mást, mint lejátssza ezt a mozgást az elejétől a végéig. Az „absztrakt Lie-algebra-elem” tehát egy egyenletes mozgás, a hozzá tartozó csoportelem pedig az, ahova ezzel megérkezel.</p><p>A pálya, amit a keret bejár, mindig ugyanolyan alakú: forgás egy rögzített tengely körül, közben csúszás <em>ugyanazon</em> tengely mentén. Ez egy csavar, a bejárt pálya hélix, és ez <button class="termbtn" id="se3ChaInfo" type="button" aria-expanded="false" aria-controls="se3ChaNote">Chasles tétele</button>: minden merev mozgás csavar.</p><p>A tengely iránya <span class="m">â = φ/θ</span> (ahol <span class="m">θ = |φ|</span>), a menetemelkedés pedig a tengelyre eső eltolás, <span class="m">d = â · ρ</span>. Két elfajult eset van, és mindkettő ismerős:</p><p class="matline"><span class="m">θ = 0 → egyenes ·  d = 0 → tiszta forgás</span></p><p>Ez az <span class="m">SE(3)</span> geodetikusa, a „legegyenesebb” út két póz között, állandó sebességgel bejárva. Ugyanaz a szerep, mint a lapos térben a szakasznak, vagy a gömbön a főkör-ívnek.</p><p>A „legrövidebb” szóval viszont óvatosan. Hosszat mérni csak akkor tudunk, ha megmondjuk, hány méter ér fel egy radiánnal, és <span class="m">SE(3)</span>-on nincs olyan természetes mérték, ami minden keretváltásra változatlan maradna. A csavar tehát az <em>egyenletes</em> mozgás, ez a természetes választás, de hogy egyben a legrövidebb-e, az a mértéken múlik. Tiszta forgatásokra, <span class="m">SO(3)</span>-on, ez a kérdés fel sem merül: ott van természetes mérték, és az ív valóban a legrövidebb út.</p><div class="bubble" id="se3ChaNote" role="dialog" aria-label="Chasles tetele" hidden><p>Miért kerül elő mindig egy tengely? Mert a forgatásrész <span class="m">R = exp(φ<sup>∧</sup>)</span> már kijelöl egyet, a saját forgástengelyét, <span class="m">â</span>-t. Az eltolást pedig két részre bonthatjuk: a tengellyel <em>párhuzamos</em> és arra <em>merőleges</em> részre.</p><p>A párhuzamos rész, <span class="m">d = â·t</span>, nem forgatható el sehova, ez marad a csúszás. A merőleges részt viszont a forgatás elintézi, ha a tengelyt eltoljuk a megfelelő <span class="m">c</span> pontba. Keressük azt a <span class="m">c</span>-t, amit a mozgás csak a tengely mentén visz el:</p><p class="matline"><span class="m">R c + t = c + d â</span></p><p>Ennek mindig van megoldása, ha <span class="m">θ ≠ 0</span>. A képen az így kapott sárga szaggatott vonal a csavartengely, a mozgás „gerince”.</p></div>'},

        {t:'Ívhossz és húr', b:'<p>Itt van a hat szám csapdája. A <span class="m">ξ = (ρ, φ)</span> második fele ártatlan: <span class="m">φ</span> a forgásvektor, hossza a szög. Az első fele viszont <strong>nem</strong> a végpont eltolása.</p><p>Miért? Mert <span class="m">ρ</span> a <em>mozgó</em> keretben mért állandó sebesség, és miközben haladsz, a forgatás folyamatosan elfordítja az orrodat. A végpont ezeknek a folyamatosan elfordított sebességeknek az <button class="termbtn" id="se3IntInfo" type="button" aria-expanded="false" aria-controls="se3IntNote">összege</button>:</p><p class="matline"><span class="m">t = ( ∫<sub>0</sub><sup>1</sup> R(s) ds ) ρ = V(φ) ρ</span></p><p>A <span class="m">V</span> tehát nem más, mint a mozgás közben bejárt forgatások <em>átlaga</em>. Zárt alakja ugyanolyan cos/sin-mintát követ, mint a Rodrigues-formula:</p><p class="matline"><span class="m">V = I + <span class="frac"><span>1 − cos θ</span><span>θ<sup>2</sup></span></span> φ<sup>∧</sup> + <span class="frac"><span>θ − sin θ</span><span>θ<sup>3</sup></span></span> (φ<sup>∧</sup>)<sup>2</sup></span></p><p>A képen az apró lila nyilak ezek a fej-farok láncba fűzött <span class="m">R(s)ρ</span> sebességek, együtt épp a pályát rajzolják ki. A borostyán nyíl a <span class="m">t</span> húr. Mivel a sebesség hossza végig ugyanaz, a lényeg egy mondatban:</p><p class="matline"><span class="m">|ρ| = ívhossz&nbsp;&nbsp;·&nbsp;&nbsp;|t| = húr</span></p><p>Told fel a szöget. Nulla körül a kettő egybeesik (<span class="m">V → I</span>), nagy szögnél a húr összemegy, teljes körnél pedig nullára. Ezért a megtett utat, és a belőle számolt sebességet, a <span class="m">|ρ|</span> adja, nem a <span class="m">|t|</span>.</p><div class="slrow"><label>θ = <b id="se3jth">0°</b></label><input type="range" id="se3jsl" min="0" max="200" value="0" step="1"></div><div class="ro">|ρ| = <b id="se3jrho">—</b> &nbsp;·&nbsp; |t| = <b id="se3jt">—</b></div><div class="bubble" id="se3IntNote" role="dialog" aria-label="Az integral" hidden><p>A test origója <span class="m">ρ</span> sebességgel halad a <em>saját</em> keretében. A világban ezt a pillanatnyi forgatás fordítja el, tehát a világbeli sebesség <span class="m">R(s) ρ</span>, ahol <span class="m">R(s) = exp(s φ<sup>∧</sup>)</span>.</p><p>Az elmozdulás ezek összege egységnyi idő alatt:</p><p class="matline"><span class="m">t = ∫<sub>0</sub><sup>1</sup> R(s) ρ ds</span></p><p>Mivel <span class="m">ρ</span> állandó, kiemelhető, és marad a forgatások integrálja, ez a <span class="m">V(φ)</span>. Ha nincs forgás, <span class="m">R(s) = I</span> és <span class="m">V = I</span>, vagyis <span class="m">t = ρ</span>. Ez a lapos eset.</p></div>'},
      ...
      en: [
        ...
        {t:'The Adjoint — Another Observer', b:'<p>The six numbers are always relative to some frame. Move to a different frame and the motion stays physically the same, but its coordinates change. The change is linear, and its matrix is called the <button class="termbtn" id="se3AdjInfo" type="button" aria-expanded="false" aria-controls="se3AdjNote">adjoint</button>:</p><p class="matline"><span class="m">ξ<sub>A</sub> = Ad<sub>T<sub>AB</sub></sub> ξ<sub>B</sub>,&nbsp;&nbsp; Ad<sub>T</sub> =</span><span class="matrix"><span class="mbracket"></span><span class="mgrid" style="grid-template-columns:repeat(2,auto)"><span>R</span><span>t<sup>∧</sup>R</span><span>0</span><span>R</span></span><span class="mbracket right"></span></span></p><p>The rotation part merely turns. The translation part picks up the offset <span class="m">t</span> between the frames, because seen from a point further out the same rotation also looks like sideways motion. Exactly what you feel sitting at the rim of a carousel.</p><p>There is one <button class="termbtn" id="se3InvarInfo" type="button" aria-expanded="false" aria-controls="se3InvarNote">exception</button>, and it is what makes all of this usable: a screw is invariant under <em>its own</em> motion.</p><p class="matline"><span class="m">Ad<sub>exp(ξ)</sub> ξ = ξ</span></p><p>Geometrically: the screw axis travels along with the body, so the motion has the same six coordinates seen from the frame at either end. That is why the coral and teal arrow pair glued to the frame looks identical at both endpoints in the scene.</p><p>This has a very concrete payoff. If you derive a speed from the relative pose of two keyframes, <span class="m">|ρ| / Δt</span> does not depend on which endpoint frame you worked in, the same number comes out either way. Likewise <span class="m">θ / Δt</span> is the angular rate. That is what makes these comparable against IMU or wheel odometry, and usable for rejecting outlier motions.</p><div class="bubble" id="se3AdjNote" role="dialog" aria-label="What the adjoint is for" hidden><p>The adjoint is the motion converter. Its definition is one line:</p><p class="matline"><span class="m">T exp(ξ<sup>∧</sup>) T<sup>−1</sup> = exp( (Ad<sub>T</sub> ξ)<sup>∧</sup> )</span></p><p>In words: carrying a motion into another frame, performing it there, and coming back is the same as moving with the <em>converted</em> velocity in the first place.</p><p>That is why it shows up wherever frames change: switching between left and right perturbations, chaining Jacobians, and converting covariances, <span class="m">Σ<sub>A</sub> = Ad Σ<sub>B</sub> Ad<sup>⊤</sup></span>.</p></div><div class="bubble" id="se3InvarNote" role="dialog" aria-label="Why invariant" hidden><p>Put <span class="m">T = exp(ξ<sup>∧</sup>)</span> into the definition:</p><p class="matline"><span class="m">exp(ξ<sup>∧</sup>) exp(ξ<sup>∧</sup>) exp(ξ<sup>∧</sup>)<sup>−1</sup> = exp(ξ<sup>∧</sup>)</span></p><p>The left side is obviously <span class="m">exp(ξ<sup>∧</sup>)</span>, hence <span class="m">Ad<sub>exp(ξ)</sub> ξ = ξ</span>. An element always commutes with itself, that is the whole argument.</p><p>The consequence is not trivial though: the log of a relative pose gives the same coordinates in the frame at the start of the motion and in the frame at its end. A local six-vector that needs no label saying which endpoint it belongs to.</p></div>'},

        {t:'A Pose at Any Instant', b:'<p>A practical question every SLAM system runs into: poses arrive on one time grid (say 100 Hz odometry), while the query sits somewhere else, at the timestamp of an image, a lidar scan, or a label. You need a pose <em>between</em> two neighbours.</p><p>The established recipe treats the two parts separately. For rotation, <strong>SLERP</strong>, the spherical geodesic travelled at constant angular speed. (Averaging the matrices entry by entry is not allowed, the result would leave <span class="m">SO(3)</span>.) For translation, plain component-wise <strong>linear interpolation</strong>.</p><p>It is worth seeing how close this stays to the screw. SLERP <em>is</em> exactly the rotation part of the screw, the same constant-angular-velocity arc. The difference lives only in the translation:</p><p class="matline"><span class="m">lerp → chord&nbsp;&nbsp;·&nbsp;&nbsp;screw → arc</span></p><p>In the scene the teal path is the split recipe, straight segments joining the keyframes. The violet one is the true <span class="m">SE(3)</span> geodesic, <span class="m">T(u) = T<sub>a</sub> exp(u · log(T<sub>a</sub><sup>−1</sup>T<sub>b</sub>))</span>. The red arrow is the gap between them. Drag the slider: the gap grows with the <em>rotation</em> inside the segment and is second order for small angles, so it is negligible for a dense, high-rate trajectory, but not for a segment that jumps half a turn.</p><div class="slrow"><label>t = <b id="se3iq">0.00</b></label><input type="range" id="se3isl" min="0" max="1000" value="0" step="1"></div><div class="ro">gap: <b id="se3igap">—</b> &nbsp;·&nbsp; <button class="act" id="se3iauto" type="button" style="padding:2px 10px;font-size:12px">auto</button></div><p style="margin-top:12px">Two things are worth keeping in mind. The split recipe is invariant to the choice of <em>world</em> frame, multiply from the left and you get the same curve. It is not invariant to a redefinition of the <em>body</em> frame, whereas the screw version is invariant to both. And both are only <em>interpolation</em>: a strictly increasing time grid and a query inside the covered range are not formalities, because extrapolating poses is a bad idea.</p>'}
      ],
      ja: [
        {t:'位置姿勢', b:'<p><em>位置姿勢（ポーズ）</em>は、ある座標系が別の座標系に対してどこに立ち、どちらを向いているかに答えます。記述するのは二つの量: 回転 <span class="m">R ∈ SO(3)</span> と並進 <span class="m">t ∈ ℝ³</span> です。点を座標系のあいだで移すのはこれだけ:</p><p class="matline"><span class="m">p<sub>W</sub> = R p<sub>B</sub> + t</span></p><p>この二つは一つの <button class="termbtn" id="se3HomInfo" type="button" aria-expanded="false" aria-controls="se3HomNote">4×4</button> 行列に詰め込まれます:</p><p class="matline"><span class="m">T =</span><span class="matrix"><span class="mbracket"></span><span class="mgrid" style="grid-template-columns:repeat(2,auto)"><span>R</span><span>t</span><span>0<sup>⊤</sup></span><span>1</span></span><span class="mbracket right"></span></span><span class="m">∈ SE(3)</span></p><p>自由度は 6 — 三つの角度と三つのずれ — なのに、蓄えている数は 12 個。その差を吸収するのが拘束 <span class="m">R<sup>⊤</sup>R = I</span> であり、まさにそれがこの空間を曲げています: 二つの位置姿勢を成分ごとに平均してはいけません。結果が <span class="m">SE(3)</span> から外れてしまうからです。</p><p>SLAM の言葉でいえば、キーフレームはどれも位置姿勢であり、地図点は世界座標系に住み、カメラ–IMU の<em>外部パラメータ</em>もまた位置姿勢です。もちろん SLAM はそれ以上のもの — 推定、データ対応付け、ループ閉じ込み、最適化 — ですが、この変換こそがその幾何的な骨格で、他はすべてその上に建っています。</p><div class="bubble" id="se3HomNote" role="dialog" aria-label="なぜ 4x4 か" hidden><p>仕掛けは<em>同次座標</em>です: 三つの数の隣に四つ目の数を書き、その四つ目に「この三つ組が何を意味するか」を言わせます。</p><p>末尾が <strong>1</strong>: <em>点</em>。原点から測った位置を持つ、空間の場所です。</p><p>末尾が <strong>0</strong>: <em>方向ベクトル</em>。場所ではなく矢印で、意味を持つのは向きと長さだけ、基点は関係ありません。視線の向き、面の法線、速度ベクトルを思い浮かべてください。これは回転ではない — 回転は <span class="m">R</span> の仕事です — ふつうの 3 成分ベクトルです。</p><p class="matline"><span class="m">T</span><span class="matrix"><span class="mbracket"></span><span class="mgrid" style="grid-template-columns:auto"><span>p</span><span>1</span></span><span class="mbracket right"></span></span><span class="m">=</span><span class="matrix"><span class="mbracket"></span><span class="mgrid" style="grid-template-columns:auto"><span>Rp + t</span><span>1</span></span><span class="mbracket right"></span></span><span class="m">,&nbsp;&nbsp; T</span><span class="matrix"><span class="mbracket"></span><span class="mgrid" style="grid-template-columns:auto"><span>v</span><span>0</span></span><span class="mbracket right"></span></span><span class="m">=</span><span class="matrix"><span class="mbracket"></span><span class="mgrid" style="grid-template-columns:auto"><span>Rv</span><span>0</span></span><span class="mbracket right"></span></span></p><p>つまり方向からは並進が落ちる — そしてそれで正しいのです。カメラを回せば視線の向きも一緒に回りますが、カメラを 2 メートル左へ運んでも矢印は同じ向きのままです。点には「どこ」があり、方向にはありません。</p><p>付け足された <span class="m">[0 0 0 1]</span> の行も飾りではありません: 二つの位置姿勢の積を再び位置姿勢にするのがこの行です。連鎖はふつうの行列積になり、単位元があり、逆元もある。だから位置姿勢は<em>群</em>をなします。その群が <span class="m">SE(3)</span> です。</p></div>'},

        {t:'位置姿勢か、変換か', b:'<p>同じ 4×4 行列が二つのことを同時に意味します。どちらを言っているのかを、はっきりさせておく価値があります。</p><p><strong>座標系の取り替え。</strong> 点は動かず、ただ別の座標系の成分で記述されるだけ。これが <span class="m">p<sub>W</sub> = R p<sub>B</sub> + t</span> という式で、何も変位せず、読み方が変わるだけです。</p><p><strong>位置姿勢。</strong> 座標系 <span class="m">B</span> が <span class="m">W</span> の中でどこに立っているか。そしてここが要点です: この二つは<em>同じ行列</em>であり、偶然ではありません。座標系の取り替えが <span class="m">B</span> の基底ベクトルについて何と言っているかを見てください:</p><p class="matline"><span class="m">e<sub>1</sub> ↦ R e<sub>1</sub> + t,&nbsp;&nbsp; e<sub>2</sub> ↦ R e<sub>2</sub> + t,&nbsp;&nbsp; e<sub>3</sub> ↦ R e<sub>3</sub> + t</span></p><p>つまり <span class="m">R</span> の<button class="termbtn" id="se3ColInfo" type="button" aria-expanded="false" aria-controls="se3ColNote">列</button>はちょうど、<span class="m">W</span> で表した <span class="m">B</span> の軸であり、<span class="m">t</span> は <span class="m">W</span> における <span class="m">B</span> の原点です。行列の四本の列が文字どおり座標系を描いており、シーンが見せているのもそれです: 基底ベクトルが、body の軸の立っている場所へ飛んでいきます。</p><p><strong>運動。</strong> 三つ目の読み方もあります: 物体を取って変位させる。そのとき <span class="m">T</span> は座標系の<em>位置</em>ではなく<em>変位</em>です。計算はまったく同じ、意味は違う。だから別の名前に値します — 相対位置姿勢、変位、<span class="m">ΔT</span>。</p><p>記法はこれを引き受けます。<span class="m">T<sub>WB</sub></span> は「<span class="m">W</span> における <span class="m">B</span> の位置姿勢」とも、「<span class="m">B</span> 座標を <span class="m">W</span> 座標へ変換する」とも読めます。この二つの文は同じことを言っています。代わりに <span class="m">ΔT</span> と書けば運動の話であり、その場合はつねに「どの座標系に対してか」を言わねばなりません — それが随伴 Ad の主題になります。</p><div class="bubble" id="se3ColNote" role="dialog" aria-label="なぜ列なのか" hidden><p>行列の <span class="m">i</span> 番目の列とは、定義からして <span class="m">i</span> 番目の基底ベクトルに作用させた結果です:</p><p class="matline"><span class="m">R e<sub>1</sub> = R の第 1 列</span></p><p>そうなるのは <span class="m">e<sub>1</sub> = (1, 0, 0)</span> であり、行列の積が列の重み付き和だからです。ここでの重みは <span class="m">1, 0, 0</span> なので、ちょうど第 1 列だけが生き残ります。</p><p>幾何的には: 座標系 <span class="m">B</span> の <span class="m">x</span> 軸に沿って 1 だけ進むと、世界の中では <span class="m">R</span> の第 1 列だけ進んだことになる。座標系の三本の軸とその原点が、ちょうど <span class="m">T</span> の四本の列です — だから位置姿勢は行列の「中に入っている」と言うのです。</p></div>'},

        {t:'座標系の連鎖', b:'<p>変換がどの座標系からどの座標系へ行くのかを書き下せば、内側の添字が消し合うので、連鎖はひとりでに読めます:</p><p class="matline"><span class="m">T<sub>WC</sub> = T<sub>WB</sub> · T<sub>BC</sub></span></p><p>右から左へ読みます: まずカメラ座標系から body 座標系へ、次に body から世界へ。</p><p>シーンの緑の点はぴくりとも動かず、まさにその場所にとどまります。変えているのは、どの座標軸に沿って読むかだけで、階段状の線がその読み取りを成分ごとに描き出します。三つの数の組はどれも違うのに、点は同じ。座標系の取り替えとは、まるごとそれだけのことです。</p><p>実務では <span class="m">T<sub>BC</sub></span> が外部パラメータ（剛体で不変）、<span class="m">T<sub>WB</sub></span> が動く body の位置姿勢です。その積が世界におけるカメラの位置姿勢であり、同じ行列がカメラ座標を世界座標へ変換します。</p><p><button class="termbtn" id="se3InvInfo" type="button" aria-expanded="false" aria-controls="se3InvNote">逆行列</button>は単なる転置ではありません:</p><p class="matline"><span class="m">T<sup>−1</sup> =</span><span class="matrix"><span class="mbracket"></span><span class="mgrid" style="grid-template-columns:repeat(2,auto)"><span>R<sup>⊤</sup></span><span>−R<sup>⊤</sup>t</span><span>0<sup>⊤</sup></span><span>1</span></span><span class="mbracket right"></span></span></p><p>回転は直交行列なので転置で足ります。しかし並進のほうも回し戻さねばなりません。さもないと古い座標系の軸に沿って適用されてしまいます。SLAM ではこれが日常の操作で、オドメトリは連鎖を作り、相対位置姿勢は <span class="m">T<sub>ab</sub> = T<sub>a</sub><sup>−1</sup>T<sub>b</sub></span> です。</p><p>一つ忘れてはいけないこと: 積の順序は入れ替えられません。それが次のステーションの主題です。</p><div class="bubble" id="se3InvNote" role="dialog" aria-label="逆行列の導出" hidden><p>二行で済みます。定義から出発:</p><p class="matline"><span class="m">q = R p + t</span></p><p><span class="m">p</span> について解く: <span class="m">t</span> を引き、左から <span class="m">R<sup>−1</sup> = R<sup>⊤</sup></span> を掛けます:</p><p class="matline"><span class="m">p = R<sup>⊤</sup>(q − t) = R<sup>⊤</sup>q − R<sup>⊤</sup>t</span></p><p>これはちょうど位置姿勢の形で、回転は <span class="m">R<sup>⊤</sup></span>、並進は <span class="m">−R<sup>⊤</sup>t</span>。幾何的に読めば <span class="m">−R<sup>⊤</sup>t</span> は単に<em>古い原点が新しい座標系のどこにあるか</em>です。</p></div>'},

        {t:'順序', b:'<p>ブロック形そのものが順序を符号化しています。<span class="m">p ↦ Rp + t</span> という式は<em>まず回して、それから動かす</em>と言っています。逆にやって、まず動かしてから回すと <span class="m">R(p + t) = Rp + Rt</span> になり、その並進は <span class="m">t</span> ではなく <span class="m">Rt</span> です。</p><p>これは揚げ足取りではなく、非可換性の本質です:</p><p class="matline"><span class="m">T(t) T(R) ≠ T(R) T(t)</span></p><p>シーンでは二つの物体が同じ二つの動きを、順序だけ逆にして行います。青緑のほうはその場で回ってから出発。菫のほうは先に出発し、あとから回る — ただしその回転は<em>世界</em>の原点まわりなので、すでに並進した物体を振り回します。二つの終点は目に見えて違い、しかも単にずれているのではなく、垂直な方向にあります。</p><p>なぜ世界の原点まわりなのか。左から掛けることは、世界座標系で作用する運動を意味するからです。物体に固定された軸のまわりに回したければ、右から掛けます。<button class="termbtn" id="se3OrdInfo" type="button" aria-expanded="false" aria-controls="se3OrdNote">数で見れば</button>どちらも二行です。</p><p>この二面性は SLAM のいたるところに付いてきます。推定値の摂動は左（世界、global とも）にも右（body、local とも）にも置けて、ヤコビアンの形が本ごとに違って見える理由は、ただそれだけです。他には何も起きていません、掛ける側が違うだけです。</p><div class="bubble" id="se3OrdNote" role="dialog" aria-label="順序を数で" hidden><p><span class="m">T(t)</span> を <span class="m">t = (2, 0, 0)</span> だけの純並進、<span class="m">T(R)</span> を <span class="m">z</span> 軸まわり 90° の回転としましょう:</p><p class="matline"><span class="m">R =</span><span class="matrix"><span class="mbracket"></span><span class="mgrid"><span>0</span><span>−1</span><span>0</span><span>1</span><span>0</span><span>0</span><span>0</span><span>0</span><span>1</span></span><span class="mbracket right"></span></span></p><p>4×4 の積はブロック形の一つの規則です:</p><p class="matline"><span class="m">[R<sub>1</sub>, t<sub>1</sub>] · [R<sub>2</sub>, t<sub>2</sub>] = [R<sub>1</sub>R<sub>2</sub>,&nbsp; R<sub>1</sub>t<sub>2</sub> + t<sub>1</sub>]</span></p><p>つまり右側の並進はつねに左側の回転で回されます。二つの順序を代入すると:</p><p class="matline"><span class="m">T(t)T(R) = [R,&nbsp; t],&nbsp;&nbsp;&nbsp; T(R)T(t) = [R,&nbsp; R t]</span></p><p>回転部分はどちらも同じ。並進は違います:</p><p class="matline"><span class="m">t = (2, 0, 0),&nbsp;&nbsp; R t = (0, 2, 0)</span></p><p>同じ二つの運動、違う順序、そして終点は単に動いたのではなく、垂直に動いています。</p></div>'},

        {t:'ツイスト', b:'<p>剛体はどう<em>動く</em>のか。各点の速度を見てみましょう。剛体性が可能性を容赦なく狭め、たった二つのベクトルが場全体を決めてしまいます:</p><p class="matline"><span class="m">v(x) = ω × x + u</span></p><p>角速度 <span class="m">ω</span>（珊瑚色の矢印）と並進速度 <span class="m">u</span>（青緑）です。シーンの小さな矢印はどれもこの二つだけから計算されており、六つの数がすべてです。角錐に鼻先が付いているのは、回りが読み取れるようにするためです。</p><p><button class="termbtn" id="se3SkewInfo" type="button" aria-expanded="false" aria-controls="se3SkewNote">外積</button>は <span class="m">x</span> について線形なので行列で書け、その行列は歪対称です。これがハット演算子です:</p><p class="matline"><span class="m">φ<sup>∧</sup> =</span><span class="matrix"><span class="mbracket"></span><span class="mgrid"><span>0</span><span>−φ<sub>3</sub></span><span>φ<sub>2</sub></span><span>φ<sub>3</sub></span><span>0</span><span>−φ<sub>1</sub></span><span>−φ<sub>2</sub></span><span>φ<sub>1</sub></span><span>0</span></span><span class="mbracket right"></span></span></p><p>六つの数は一本のベクトルに収められ、定着した名前づけはこうです: 並進部分が <span class="m">ρ := u</span>、回転部分が <span class="m">φ := ω</span>。</p><p class="matline"><span class="m">ξ = (ρ, φ) ∈ ℝ<sup>6</sup></span></p><p>ハットを付けた 4×4 の形は、位置姿勢が住むのと同じ箱です:</p><p class="matline"><span class="m">ξ<sup>∧</sup> =</span><span class="matrix"><span class="mbracket"></span><span class="mgrid" style="grid-template-columns:repeat(2,auto)"><span>φ<sup>∧</sup></span><span>ρ</span><span>0<sup>⊤</sup></span><span>0</span></span><span class="mbracket right"></span></span></p><p>これが単位元における接空間で、その名は <span class="m">se(3)</span>。ここでは足し算もスカラー倍も<em>してよい</em> — 位置姿勢は足せませんが、速度は足せます。だからこそ、あらゆる推定値、勾配、共分散は、曲がった多様体の上ではなく、この平坦な 6 次元空間に住むのです。</p><div class="bubble" id="se3SkewNote" role="dialog" aria-label="歪対称性と直交性" hidden><p>回転の拘束を、運動 <span class="m">R(t)</span> に沿って微分します:</p><p class="matline"><span class="m">R<sup>⊤</sup>R = I  ⟹  Ṙ<sup>⊤</sup>R + R<sup>⊤</sup>Ṙ = 0</span></p><p>つまり行列 <span class="m">Ω = R<sup>⊤</sup>Ṙ</span> は <span class="m">Ω<sup>⊤</sup> = −Ω</span> を満たす、歪対称行列です。この形の行列は自由な成分を三つ持ち、それがちょうど角速度の三成分で、その作用はまさに外積です: <span class="m">Ω x = ω × x</span>。</p><p><strong>では歪対称性と直交性を結ぶものは？</strong> 指数関数です、二行で。<span class="m">Ω</span> が歪対称なら</p><p class="matline"><span class="m">(exp Ω)<sup>⊤</sup> = exp(Ω<sup>⊤</sup>) = exp(−Ω) = (exp Ω)<sup>−1</sup></span></p><p>これはまさに直交性の条件です。行列式については</p><p class="matline"><span class="m">det(exp Ω) = e<sup>tr Ω</sup> = e<sup>0</sup> = 1</span></p><p>歪対称行列の対角成分はすべて 0 だからです。だから出てくるのは鏡映ではなく、正真正銘の回転です。</p><p>短く言えば: 歪対称行列は<em>無限小の</em>回転、直交行列は<em>有限の</em>回転で、<span class="m">exp</span> が前者を後者へ運びます。<span class="m">log</span> は同じことを逆向きにします。</p></div>'},

        {t:'exp — ねじ運動', b:'<p>速度 <span class="m">ξ</span> を一定に保ち、<em>単位</em>時間だけ働かせましょう。その結果が指数写像です:</p><p class="matline"><span class="m">T = exp(ξ<sup>∧</sup>)</span></p><p>ここで何が起きているのか、少し立ち止まる価値があります。一見すると <span class="m">ξ<sup>∧</sup></span> は抽象的な 4×4 行列で、0 と交差項ばかり、何も語らないように見えます。ところが実際はまったく具体的です: <em>それは運動そのもの</em>を、一定速度として記述したものです。そして <span class="m">exp</span> はその運動を最初から最後まで再生するだけ。「抽象的なリー環の元」とは等速運動のことであり、それに対応する群の元とは、その運動が連れて行った先のことです。</p><p>座標系が掃く経路の形はいつも同じです: 固定された軸のまわりに回りながら、<em>その同じ</em>軸に沿って滑る。これがねじであり、経路は螺旋、そしてこれが<button class="termbtn" id="se3ChaInfo" type="button" aria-expanded="false" aria-controls="se3ChaNote">シャールの定理</button>です: すべての剛体運動はねじ運動である。</p><p>軸の向きは <span class="m">â = φ/θ</span>（<span class="m">θ = |φ|</span>）、ピッチはそれに沿った並進成分 <span class="m">d = â · ρ</span> です。退化する場合が二つあり、どちらもおなじみです:</p><p class="matline"><span class="m">θ = 0 → 直線 ·  d = 0 → 純回転</span></p><p>これが <span class="m">SE(3)</span> の測地線、二つの位置姿勢を結ぶ最もまっすぐな道を、一定の速さでたどったものです。平坦な空間での線分、球面上での大円の弧と同じ役どころです。</p><p>ただし「最短」という言葉には注意が要ります。長さを測るには 1 ラジアンが何メートル分かを決めねばならず、<span class="m">SE(3)</span> には、あらゆる座標系の取り替えのもとで変わらない自然な測り方がありません。ですからねじ運動は<em>等速</em>の運動であり自然な選択ですが、それが最短でもあるかは測り方に依ります。純回転、すなわち <span class="m">SO(3)</span> の上ではこの問題は起きません: そこには自然な測り方があり、弧は本当に最短経路です。</p><div class="bubble" id="se3ChaNote" role="dialog" aria-label="シャールの定理" hidden><p>なぜいつも軸が現れるのか。回転部分 <span class="m">R = exp(φ<sup>∧</sup>)</span> がすでに一つ選んでいるからです — それ自身の回転軸 <span class="m">â</span> です。そして並進はその軸に<em>平行</em>な成分と<em>垂直</em>な成分に分かれます。</p><p>平行成分 <span class="m">d = â·t</span> は回して消すことができないので、滑りとして残ります。垂直成分は、軸を適切な点 <span class="m">c</span> へずらせば回転が引き受けます。運動が軸に沿って運ぶだけになるような <span class="m">c</span> を探すのです:</p><p class="matline"><span class="m">R c + t = c + d â</span></p><p><span class="m">θ ≠ 0</span> のときこれはつねに解を持ちます。シーンの琥珀色の破線がそのねじ軸、運動の背骨です。</p></div>'},

        {t:'弧と弦', b:'<p>六つの数に隠された罠がここにあります。<span class="m">ξ = (ρ, φ)</span> の後半は無害です: <span class="m">φ</span> は回転ベクトルで、その長さが角度。ところが前半は、終点の並進では<strong>ありません</strong>。</p><p>なぜか。<span class="m">ρ</span> は<em>動いている</em>座標系で測った一定速度であり、進んでいるあいだも回転が続いて、鼻先の向きを変え続けるからです。終点はそうして向きを変え続けた速度の<button class="termbtn" id="se3IntInfo" type="button" aria-expanded="false" aria-controls="se3IntNote">総和</button>です:</p><p class="matline"><span class="m">t = ( ∫<sub>0</sub><sup>1</sup> R(s) ds ) ρ = V(φ) ρ</span></p><p>つまり <span class="m">V</span> は、道すがら通り過ぎた回転たちの<em>平均</em>にほかなりません。その閉じた形は、ロドリゲスの公式と同じ cos/sin の型に従います:</p><p class="matline"><span class="m">V = I + <span class="frac"><span>1 − cos θ</span><span>θ<sup>2</sup></span></span> φ<sup>∧</sup> + <span class="frac"><span>θ − sin θ</span><span>θ<sup>3</sup></span></span> (φ<sup>∧</sup>)<sup>2</sup></span></p><p>シーンの小さな菫色の矢印がその <span class="m">R(s)ρ</span> という速度で、頭と尾をつないで並べると経路そのものを描きます。琥珀色の矢印が弦 <span class="m">t</span> です。速度は道中ずっと長さを保つので、要点は一行に収まります:</p><p class="matline"><span class="m">|ρ| = 弧長&nbsp;&nbsp;·&nbsp;&nbsp;|t| = 弦</span></p><p>角度を上げてみてください。0 の近くでは二つは一致し（<span class="m">V → I</span>）、角度が大きくなると弦は縮み、一回転で完全に潰れます。ですから進んだ距離、そしてそこから導かれる速さは、<span class="m">|t|</span> ではなく <span class="m">|ρ|</span> から来ます。</p><div class="slrow"><label>θ = <b id="se3jth">0°</b></label><input type="range" id="se3jsl" min="0" max="200" value="0" step="1"></div><div class="ro">|ρ| = <b id="se3jrho">—</b> &nbsp;·&nbsp; |t| = <b id="se3jt">—</b></div><div class="bubble" id="se3IntNote" role="dialog" aria-label="積分" hidden><p>body の原点は<em>自分自身の</em>座標系で速度 <span class="m">ρ</span> で動きます。世界から見ればその速度は現在の回転で回されるので、世界での速度は <span class="m">R(s) ρ</span>、ここで <span class="m">R(s) = exp(s φ<sup>∧</sup>)</span> です。</p><p>変位は単位時間にわたるその総和です:</p><p class="matline"><span class="m">t = ∫<sub>0</sub><sup>1</sup> R(s) ρ ds</span></p><p><span class="m">ρ</span> は定数なので外へ出せて、残るのは回転の積分、すなわち <span class="m">V(φ)</span> です。回転がなければ <span class="m">R(s) = I</span>、<span class="m">V = I</span> なので <span class="m">t = ρ</span>。それが平坦な場合です。</p></div>'},

        {t:'log — 運動を読み戻す', b:'<p>逆向きの問い: 位置姿勢が与えられたとき、そこへ連れて行った六つの数は何だったのか。二段階です。まず回転から軸・角度ベクトル <span class="m">φ = log(R)</span> を取り、次に並進を、まさに同じ <span class="m">V</span> でほどきます:</p><p class="matline"><span class="m">ρ = V(φ)<sup>−1</sup> t</span></p><p><span class="m">V<sup>−1</sup></span> の閉じた形は驚くほど親切で、項は有限個です。<span class="m">(φ<sup>∧</sup>)<sup>3</sup></span> が低い冪に落ちるからです:</p><p class="matline"><span class="m">V<sup>−1</sup> = I − <span class="frac"><span>1</span><span>2</span></span> φ<sup>∧</sup> + <span class="frac"><span>1</span><span>θ<sup>2</sup></span></span>(1 − <span class="frac"><span>θ sin θ</span><span>2(1 − cos θ)</span></span>) (φ<sup>∧</sup>)<sup>2</sup></span></p><p><span class="m">θ = 0</span> では第三の<button class="termbtn" id="se3SerInfo" type="button" aria-expanded="false" aria-controls="se3SerNote">係数</button>が <span class="m">0/0</span> の形になりますが、極限は有限で <span class="m">1/12</span> です。除去可能な特異点であり、数学的には無害。ところが数値的には無害ではありません: 小さな角度では閉じた形が、屑のような量から大きな数を組み立てようとするので、そこでは級数展開が正しい道です。</p><p>これで <span class="m">exp</span> と <span class="m">log</span> は、<span class="m">|θ| &lt; π</span> であるかぎり厳密に互いの逆になります。その境界を越えると回転は巻き戻り、log はつねに短いほうの道を返します。<span class="m">SO(3)</span> の球体の対蹠点が語っていたのと同じトポロジーです。</p><p>SLAM ではこれが働き者です。二つのキーフレームの相対位置姿勢は <span class="m">T<sub>ab</sub> = T<sub>a</sub><sup>−1</sup>T<sub>b</sub></span> で、その log は六つの数 — 二つのフレームのあいだで body がどれだけ回り、どれだけ進んだか。因子グラフの残差は文字どおりこれです:</p><p class="matline"><span class="m">r = log(T<sub>meas</sub><sup>−1</sup> T<sub>est</sub>)<sup>∨</sup> ∈ ℝ<sup>6</sup></span></p><p>これによって位置姿勢の誤差が、最小二乗の機械が噛み砕けるふつうのベクトルになります。</p><div class="bubble" id="se3SerNote" role="dialog" aria-label="小さな角度の場合" hidden><p>分子と分母を <span class="m">θ = 0</span> のまわりで展開します:</p><p class="matline"><span class="m">1 − cos θ ≈ <span class="frac"><span>θ<sup>2</sup></span><span>2</span></span>,&nbsp;&nbsp; θ sin θ ≈ θ<sup>2</sup> − <span class="frac"><span>θ<sup>4</sup></span><span>6</span></span></span></p><p>代入すると、係数はおなじみの <span class="m">1/12</span> に近づき、次の項は <span class="m">θ<sup>2</sup></span> に比例します。</p><p>実務上の教訓: 閉じた形の <span class="m">1 − cos θ</span> は、<span class="m">θ</span> が小さいときほぼ等しい二数の差であり、まさにそこで浮動小数点は有効数字を失います。そこでは極限の定数のほうが、厳密な式<em>より</em>正確です。</p></div>'},

        {t:'随伴 Ad — 別の観測者', b:'<p>六つの数は、つねに何らかの座標系に対して言われています。別の座標系へ移ると、運動は物理的には同じままですが、その座標は変わります。この変化は線形で、その行列を<button class="termbtn" id="se3AdjInfo" type="button" aria-expanded="false" aria-controls="se3AdjNote">随伴（アジョイント）</button>と呼びます:</p><p class="matline"><span class="m">ξ<sub>A</sub> = Ad<sub>T<sub>AB</sub></sub> ξ<sub>B</sub>,&nbsp;&nbsp; Ad<sub>T</sub> =</span><span class="matrix"><span class="mbracket"></span><span class="mgrid" style="grid-template-columns:repeat(2,auto)"><span>R</span><span>t<sup>∧</sup>R</span><span>0</span><span>R</span></span><span class="mbracket right"></span></span></p><p>回転部分はただ回るだけ。並進部分は座標系のあいだのずれ <span class="m">t</span> を拾います。遠くの点から見れば、同じ回転が横方向の運動としても見えるからです。回転木馬の縁に座っているときに感じるのが、まさにそれです。</p><p>ひとつだけ<button class="termbtn" id="se3InvarInfo" type="button" aria-expanded="false" aria-controls="se3InvarNote">例外</button>があり、それがこの全体を使いものにしています: ねじ運動は<em>それ自身の</em>運動のもとで不変です。</p><p class="matline"><span class="m">Ad<sub>exp(ξ)</sub> ξ = ξ</span></p><p>幾何的には: ねじ軸は物体と一緒に運ばれるので、始点側の座標系から見ても終点側から見ても、その運動は同じ六つの座標を持ちます。だからシーンで座標系に貼り付いた珊瑚色と青緑の矢印の組は、両端でまったく同じに見えるのです。</p><p>これには非常に具体的な見返りがあります。二つのキーフレームの相対位置姿勢から速さを求めるとき、<span class="m">|ρ| / Δt</span> はどちら端の座標系で計算したかに依らず、同じ数が出ます。同様に <span class="m">θ / Δt</span> が角速度です。だからこそこれらは IMU や車輪オドメトリと比較でき、外れ値の運動を弾くのに使えます。</p><div class="bubble" id="se3AdjNote" role="dialog" aria-label="随伴は何のためか" hidden><p>随伴は運動の変換器です。その定義は一行:</p><p class="matline"><span class="m">T exp(ξ<sup>∧</sup>) T<sup>−1</sup> = exp( (Ad<sub>T</sub> ξ)<sup>∧</sup> )</span></p><p>言葉にすれば: 運動を別の座標系へ運び、そこで実行し、戻ってくることは、最初から<em>変換済みの</em>速度で動くことと同じである。</p><p>だから座標系が変わるところには必ず顔を出します: 左右の摂動の切り替え、ヤコビアンの連鎖、そして共分散の変換 <span class="m">Σ<sub>A</sub> = Ad Σ<sub>B</sub> Ad<sup>⊤</sup></span>。</p></div><div class="bubble" id="se3InvarNote" role="dialog" aria-label="なぜ不変か" hidden><p>定義に <span class="m">T = exp(ξ<sup>∧</sup>)</span> を入れます:</p><p class="matline"><span class="m">exp(ξ<sup>∧</sup>) exp(ξ<sup>∧</sup>) exp(ξ<sup>∧</sup>)<sup>−1</sup> = exp(ξ<sup>∧</sup>)</span></p><p>左辺は明らかに <span class="m">exp(ξ<sup>∧</sup>)</span> なので <span class="m">Ad<sub>exp(ξ)</sub> ξ = ξ</span>。元はつねに自分自身と可換である、議論はそれだけです。</p><p>とはいえ帰結は自明ではありません: 相対位置姿勢の log は、運動の始まりの座標系でも終わりの座標系でも同じ座標を与えます。どちら端に属するかの札を必要としない、局所的な 6 次元ベクトルです。</p></div>'},

        {t:'任意の時刻の位置姿勢', b:'<p>どの SLAM システムでもぶつかる実務的な問い: 位置姿勢はある時間格子の上に来る（たとえば 100 Hz のオドメトリ）のに、問い合わせはそれとは別の場所 — 画像やライダースキャン、ラベルのタイムスタンプ — にあります。隣り合う二つの<em>あいだ</em>の位置姿勢が要るのです。</p><p>定着したレシピは二つの部分を別々に扱います。回転には <strong>SLERP</strong>、一定の角速度でたどる球面測地線。（行列を成分ごとに平均するのは禁止です、結果が <span class="m">SO(3)</span> から出てしまいます。）並進には成分ごとのふつうの<strong>線形補間</strong>。</p><p>これがねじ運動にどれだけ近いかを見る価値があります。SLERP はねじ運動の回転部分<em>そのもの</em>、同じ等角速度の弧です。違いは並進だけに宿ります:</p><p class="matline"><span class="m">lerp → 弦&nbsp;&nbsp;·&nbsp;&nbsp;ねじ → 弧</span></p><p>シーンの青緑の経路が分割レシピで、キーフレームを結ぶ直線分です。菫のほうが本物の <span class="m">SE(3)</span> 測地線 <span class="m">T(u) = T<sub>a</sub> exp(u · log(T<sub>a</sub><sup>−1</sup>T<sub>b</sub>))</span>。赤い矢印がその隔たりです。スライダーを動かしてください: 隔たりは区間内の<em>回転</em>とともに大きくなり、小さな角度では二次のオーダーです。ですから密で高レートの軌跡では無視できますが、半回転も跳ぶ区間ではそうはいきません。</p><div class="slrow"><label>t = <b id="se3iq">0.00</b></label><input type="range" id="se3isl" min="0" max="1000" value="0" step="1"></div><div class="ro">隔たり: <b id="se3igap">—</b> &nbsp;·&nbsp; <button class="act" id="se3iauto" type="button" style="padding:2px 10px;font-size:12px">自動</button></div><p style="margin-top:12px">心に留めておく価値のあることが二つ。分割レシピは<em>世界</em>座標系の取り方に対して不変です — 左から掛けても同じ曲線が出ます。しかし<em>body</em> 座標系の取り直しに対しては不変ではなく、ねじ版はその両方に対して不変です。そして、どちらもあくまで<em>内挿</em>です: 時間格子が狭義単調で、問い合わせが覆われた範囲の内側にあること — これは形式的な条件ではありません。位置姿勢の外挿は悪い考えだからです。</p>'}
      ]
    }
  };
})();
