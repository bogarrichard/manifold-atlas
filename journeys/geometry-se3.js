...
LIE.journeys['geometry-se3'] = (function () {
  ...
  const logQ = q => {
    ...
    if (u.w < 0) {
      u.set(-u.x, -u.y, -u.z, -u.w);
    } // shortest arc: |θ| ≤ π
    const s = Math.hypot(u.x, u.y, u.z),
      th = 2 * Math.atan2(s, u.w);
  ...
  function Vmul(phi, rho) {
    ...
    const c1 = phi.clone().cross(rho),
      c2 = phi.clone().cross(c1);
  ...
  const logSE3 = T => {
    const phi = logQ(T.q);
    return {rho: VinvMul(phi, T.p), phi};
  };
  const compose = (A, B) => ({
    p: A.p.clone().add(B.p.clone().applyQuaternion(A.q)),
    q: A.q.clone().multiply(B.q),
  });
  const invert = A => {
    const qi = A.q.clone().conjugate();
  ...
  function build(C, PAL) {
    ...
    function gizmo(len, r, op) {
      ...
      [
        [V3(len, 0, 0), COL.coral],
        [V3(0, len, 0), COL.teal],
        [V3(0, 0, len), COL.violet],
      ].forEach(([v, c]) => {
    ...
    function matrixPanel(w) {
      const CW = 710,
        ...
        X1 = 684,
        Y0 = 62,
        Y1 = 398; // X1+16 must clear CW
      const cw = (X1 - X0) / 4,
        rh = (Y1 - Y0) / 4;
      const cv = document.createElement('canvas');
      cv.width = CW * PANEL_DPR;
      cv.height = CH * PANEL_DPR;
      ...
      sp.userData.draw = (M, cols, alpha, vis, t) => {
        ...
        ctx.arcTo(PX1, PY0, PX1, PY1, PR);
        ctx.arcTo(PX1, PY1, PX0, PY1, PR);
        ctx.arcTo(PX0, PY1, PX0, PY0, PR);
        ctx.arcTo(PX0, PY0, PX1, PY0, PR);
        ctx.closePath();
        ctx.fill();
        ctx.globalAlpha = vis;
        ctx.textAlign = 'left';
        ...
        ctx.fillText('T =', 8, (Y0 + Y1) / 2);
        ctx.globalAlpha = vis * 0.5;
        ctx.strokeStyle = HX.ink;
        ctx.lineWidth = 5;
        const SER = 24;
        ctx.beginPath();
        ctx.moveTo(X0 - 16 + SER, Y0 - 16);
        ctx.lineTo(X0 - 16, Y0 - 16);
        ctx.lineTo(X0 - 16, Y1 + 16);
        ctx.lineTo(X0 - 16 + SER, Y1 + 16);
        ctx.moveTo(X1 + 16 - SER, Y0 - 16);
        ctx.lineTo(X1 + 16, Y0 - 16);
        ctx.lineTo(X1 + 16, Y1 + 16);
        ctx.lineTo(X1 + 16 - SER, Y1 + 16);
        ctx.stroke();
    ...
    const stations = [
      ...
      g => {
        ...
        const lw = makeLabel('W', HX.ink, 1.2);
        lw.position.set(-0.5, 1.7, 0);
        g.add(lw);
        const body = gizmo(1.15, 0.05);
        g.add(body);
        const lb = makeLabel('T', HX.amber, 1.2);
        g.add(lb);
        const tArr = dimArrow(COL.amber, 0.032, 0.55);
        g.add(tArr); // world origin → body origin
        const lt = makeLabel('t', HX.amber, 1.1);
        g.add(lt);
        const marks = [
          V3(2.6, 0.9, -1.4),
          V3(-2.2, 1.3, 1.9),
        ...
        return {
          tick(t) {
            const a = t * 0.28;
            const T = pose(
              V3(Math.cos(a) * 2.3, 0.55 + Math.sin(t * 0.4) * 0.25, Math.sin(a) * 2.3),
      ...
      g => {
        ...
        S.position.set(-1.0, -0.2, 0);
        g.add(S);
        const PHI = V3(0.18, 0.95, 0.28),
          TT = V3(2.0, 0.8, -0.7);
        ...
        const world = gizmo(1.15, 0.038, 0.24);
        world.position.copy(CTR);
        S.add(world); // W
        const body = gizmo(1.05, 0.042, 0.2);
        setPose(body, T);
        S.add(body); // where it is headed
        const lw = makeLabel('W', HX.ink, 1.1);
        lw.position.copy(CTR).add(V3(0.5, 1.3, 0));
        S.add(lw);
        const lbdy = makeLabel('B', HX.ink, 1.1);
        lbdy.position.copy(T.p).add(V3(0, 1.55, 0));
        S.add(lbdy);
        const tArr = dimArrow(COL.amber, 0.032, 0.75);
        S.add(tArr); // zero-length until t runs
        const AX = [V3(1, 0, 0), V3(0, 1, 0), V3(0, 0, 1)],
          CO = [COL.coral, COL.teal, COL.violet];
        const LEN = 1.05;
        // the three live axes, plus the arc each tip sweeps on its way into place
        const arms = CO.map(c => {
          const a = dimArrow(c, 0.05, 1);
          S.add(a);
          return a;
        });
        const swept = CO.map(c => {
          const l = polyline(24, c, 0.45);
          S.add(l);
          return l;
        });
        ...
        const lbl = makeLabel(' ', HX.coral, 2.7);
        lbl.position.copy(CTR).add(V3(0, 2.5, 0.9));
        S.add(lbl);
        ...
        const panel = matrixPanel(3.9);
        panel.position.set(2.6, 3.5, 0);
        g.add(panel);
        ...
        return {
          tick(t) {
            ...
            const dir = AX.map((e, i) =>
              e.clone().applyQuaternion(expQ(PHI.clone().multiplyScalar(u[i])))
            );
            for (let i = 0; i < 3; i++) {
              ...
              for (let k = 0; k < 24; k++) {
                ...
                pts.push(
                  org
                    .clone()
                    .addScaledVector(
                      AX[i].clone().applyQuaternion(expQ(PHI.clone().multiplyScalar(s))),
                      LEN
                    )
                );
              }
            ...
            if (ph !== cur) {
              cur = ph;
              if (ph >= 0) updateLabel(lbl, TX[ph], CX[ph]);
            }
            lbl.visible = ph >= 0;
      ...
      g => {
        ...
        const anchors = [
          ...
          const gz = gizmo(0.95, 0.036, ANCH);
          setPose(gz, T);
          g.add(gz);
          const l = makeLabel(name, HX.ink, 0.9);
          l.position.copy(T.p).add(V3(-0.42, 1.3, 0));
          g.add(l);
          return {p: T.p, gz};
        });
        const live = gizmo(0.95, 0.05, 1);
        g.add(live);
        // one 2-point line per axis component, rebuilt from `live`'s axes on every tick
        const legs = [
          polyline(2, COL.coral, 0.95),
          polyline(2, COL.teal, 0.95),
          polyline(2, COL.violet, 0.95),
        ];
        legs.forEach(l => g.add(l));
        const read = makeLabel(' ', HX.green, 3.9);
        g.add(read);
        const step = makeLabel(' ', HX.amber, 2.2);
        g.add(step);
        const HOP = [
          {A: TWC, B: TWB, s: 'B ← C', n: 'B'},
          {A: TWB, B: W, s: 'W ← B', n: 'W'},
        ];
        HOP.forEach(h => {
          h.xi = logSE3(compose(invert(h.A), h.B));
        });
        const at = (h, s) =>
          compose(
            h.A,
            expSE3(h.xi.rho.clone().multiplyScalar(s), h.xi.phi.clone().multiplyScalar(s))
          );
        const LEAD = 0.45,
          GROW = 1.5,
          HOLD = 1.1,
        ...
        return {
          tick(t) {
            ...
            live.children.forEach(a => {
              a.userData.cyl.material.opacity = o;
            });
            ...
            anchors.forEach(an => {
              ...
              an.gz.children.forEach(a => {
                a.userData.cyl.material.opacity = ANCH * f * vis;
              });
            });
            ...
            const txt =
              'p' + (name ? '(' + name + ')' : '') + ' = (' + shown.map(fmt).join(', ') + ')';
            if (txt !== lastR && t - lastT > 0.04) {
              lastR = txt;
              lastT = t;
              updateLabel(read, txt, HX.green);
            }
            ...
            if (k !== curS) {
              curS = k;
              if (k >= 0) updateLabel(step, HOP[k].s, HX.amber);
            }
          },
        };
      },
      ...
      g => {
        ...
        const trA = polyline(2, COL.teal, 0.8);
        g.add(trA);
        trA.userData.set([V3(0, 0, 0), TT.clone()]);
        const trB = polyline(42, COL.violet2, 0.8);
        g.add(trB);
        const pathB = [V3(0, 0, 0)];
        for (let i = 0; i <= 40; i++) {
          const s = i / 40;
          pathB.push(TT.clone().applyQuaternion(expQ(PHI.clone().multiplyScalar(s))));
        }
        trB.userData.set(pathB);
        g.add(dot(COL.teal, 0.14, endA.p));
        g.add(dot(COL.violet2, 0.14, endB.p));
        // B is drawn a touch smaller so the two frames stay apart while they still overlap
        const gA = gizmo(1.0, 0.05),
          gB = gizmo(0.8, 0.042);
        g.add(gA);
        g.add(gB);
        const lA = makeLabel('T(t) T(R)', HX.teal, 2.6);
        lA.position.copy(endA.p).add(V3(0.7, -0.8, 0));
        g.add(lA);
        const lB = makeLabel('T(R) T(t)', HX.violet2, 2.6);
        lB.position.copy(endB.p).add(V3(0, 0.9, 0));
        g.add(lB);
        const PER = 5.2;
        return {
          tick(t) {
            const tc = t % PER;
            const u1 = ease(clamp((tc - 0.4) / 1.6, 0, 1)),
              u2 = ease(clamp((tc - 2.4) / 1.6, 0, 1));
            setPose(gA, {
              p: TT.clone().multiplyScalar(u2),
      ...
      g => {
        ...
        g.add(
          new THREE.Mesh(
            new THREE.ConeGeometry(R0, 2 * HH, 4),
        ...
        const arrows = pts.map(() => {
          const a = dimArrow(COL.violet, 0.026, 0.8);
          g.add(a);
          return a;
        });
        const axArr = fatArrow(COL.coral, 0.05),
          uArr = fatArrow(COL.teal, 0.05);
        g.add(axArr);
        g.add(uArr);
        const lw = makeLabel('φ', HX.coral, 1.4),
          lu = makeLabel('ρ', HX.teal, 1.4);
        g.add(lw);
        g.add(lu);
        ...
        return {
          tick(t) {
            const w = V3(Math.sin(t * 0.33) * 0.55, 1.0, Math.cos(t * 0.29) * 0.45)
              .normalize()
              .multiplyScalar(1.6);
            pts.forEach((p, i) =>
              setArrow(arrows[i], p, w.clone().cross(p).add(U).multiplyScalar(0.55))
            );
      ...
      g => {
        const RHO = V3(2.2, 0.8, 0.2),
          PHI = V3(0.16, 1, 0.1).normalize().multiplyScalar(2.55);
        const path = [];
        for (let i = 0; i <= 90; i++) {
          const s = i / 90;
          path.push(expSE3(RHO.clone().multiplyScalar(s), PHI.clone().multiplyScalar(s)).p);
        }
        ...
        if (sc) {
          g.add(
            dashedLine(
              sc.c.clone().addScaledVector(sc.ax, -2.6),
              sc.c.clone().addScaledVector(sc.ax, 3.0),
              COL.amber,
              0.14
            )
          );
          const la = makeLabel('â', HX.amber, 1.0);
          la.position.copy(sc.c).addScaledVector(sc.ax, 3.35);
          g.add(la);
        }
        for (let k = 0; k <= 4; k++) {
          const s = k / 4,
            T = expSE3(RHO.clone().multiplyScalar(s), PHI.clone().multiplyScalar(s));
          const gh = gizmo(0.55, 0.028, 0.32);
          setPose(gh, T);
          g.add(gh);
        }
        const live = gizmo(0.95, 0.05);
        g.add(live);
        const ls = makeLabel('exp(ξ)', HX.ink, 2.0);
        ls.position.set(0, 2.6, 0);
        g.add(ls);
        const PER = 4.6;
        return {
          tick(t) {
            const u = clamp(((t % PER) - 0.5) / 3.2, 0, 1),
              s = ease(u);
      ...
      g => {
        const RHO = V3(2.5, 0.35, 0.0),
          AX = V3(0.12, 1, 0.16).normalize();
        const N = 14;
        const seg = [];
        for (let i = 0; i < N; i++) {
          const a = dimArrow(COL.violet, 0.026, 0.9);
          g.add(a);
          seg.push(a);
        }
        const arc = polyline(64, COL.violet2, 0.55);
        g.add(arc);
        const tArr = fatArrow(COL.amber, 0.05);
        g.add(tArr);
        const rArr = fatArrow(COL.teal, 0.05);
        g.add(rArr);
        const endD = dot(COL.amber, 0.1);
        g.add(endD);
        const lt = makeLabel('t = V(φ)ρ', HX.amber, 2.6),
          lr = makeLabel('ρ', HX.teal, 1.0);
        g.add(lt);
        g.add(lr);
        let theta = 0.0;
        function refresh() {
          ...
          for (let i = 0; i < 64; i++) {
            const s = i / 63;
            pts.push(Vmul(phi.clone().multiplyScalar(s), RHO.clone().multiplyScalar(s)));
          }
          ...
          for (let i = 0; i < N; i++) {
            ...
            setArrow(
              ...
              from,
              RHO.clone()
                .applyQuaternion(expQ(phi.clone().multiplyScalar(s)))
                .multiplyScalar(1 / N)
            );
          }
      ...
      g => {
        ...
        function frameAt(s) {
          const p = curve.getPointAt(clamp(s, 0, 1)),
            T = curve.getTangentAt(clamp(s, 0, 1)).normalize();
          let Nv = V3(0, 1, 0).cross(T);
          if (Nv.length() < 1e-3) Nv = V3(1, 0, 0).cross(T);
          Nv.normalize();
          const B = T.clone().cross(Nv);
          return {
            p,
            q: new THREE.Quaternion().setFromRotationMatrix(
              new THREE.Matrix4().makeBasis(Nv, B, T)
            ),
          };
        }
        const SS = [0.04, 0.24, 0.44, 0.64, 0.84];
        const KF = SS.map(s => frameAt(s));
        const ghosts = KF.map(T => {
          const gh = gizmo(0.5, 0.026, 0.3);
          setPose(gh, T);
          g.add(gh);
          return gh;
        });
        const a1 = gizmo(0.85, 0.045),
          a2 = gizmo(0.85, 0.045);
        g.add(a1);
        g.add(a2);
        ...
        inset.position.set(0.6, 1.9, 0);
        g.add(inset);
        inset.add(gizmo(0.7, 0.035, 0.45));
        const arc = polyline(48, COL.amber, 0.95);
        inset.add(arc);
        const relEnd = gizmo(0.8, 0.042);
        inset.add(relEnd);
        const read = makeLabel('θ = 0°', HX.amber, 3.4);
        read.position.set(0.6, 3.1, 0);
        g.add(read);
        const lrel = makeLabel('log(Ta^{-1}Tb)', HX.ink, 3.0);
        lrel.position.set(0.6, 3.8, 0);
        g.add(lrel);
        let cur = -1;
        function showPair(k) {
          cur = k;
          ...
          updateLabel(
            read,
            'θ = ' + th.toFixed(0) + '°   |ρ| = ' + xi.rho.length().toFixed(2),
            HX.amber
          );
        }
        showPair(0);
      ...
      g => {
        const RHO = V3(1.55, 0.4, 0.1),
          PHI = V3(0.1, 1, 0.12).normalize().multiplyScalar(2.2);
        const path = [];
        for (let i = 0; i <= 90; i++) {
          const s = i / 90;
          path.push(expSE3(RHO.clone().multiplyScalar(s), PHI.clone().multiplyScalar(s)).p);
        ...
        g.add(
          new THREE.Mesh(
            new THREE.TubeGeometry(new THREE.CatmullRomCurve3(path), 140, 0.03, 6, false),
        ...
        if (sc)
          g.add(
            dashedLine(
              sc.c.clone().addScaledVector(sc.ax, -2.2),
              sc.c.clone().addScaledVector(sc.ax, 2.8),
              COL.amber,
              0.14
            )
          );
        ...
        const start = gizmo(0.7, 0.035, 0.45);
        setPose(start, {p: V3(0, 0, 0), q: new THREE.Quaternion()});
        start.add(twistPack(0.4));
        g.add(start);
        const endT = expSE3(RHO, PHI);
        const end = gizmo(0.7, 0.035, 0.45);
        setPose(end, endT);
        end.add(twistPack(0.4));
        g.add(end);
        const live = gizmo(0.95, 0.05);
        live.add(twistPack(null));
        g.add(live);
        const lbl = makeLabel('Ad(exp ξ) ξ = ξ', HX.ink, 3.4);
        lbl.position.set(0, 2.5, 0);
        g.add(lbl);
        const PER = 5.0;
        return {
          tick(t) {
            const u = clamp(((t % PER) - 0.6) / 3.4, 0, 1),
              s = ease(u);
            setPose(live, expSE3(RHO.clone().multiplyScalar(s), PHI.clone().multiplyScalar(s)));
          },
        };
      ...
      g => {
        const KF = [
          ...
          {t: 1, T: pose(V3(-1.3, 0.25, -1.2), V3(0.05, 0.4, 0.22))},
          {t: 2, T: pose(V3(1.4, -0.45, 1.1), V3(-0.18, 1.35, -0.1))},
          {t: 3, T: pose(V3(3.5, 0.85, -0.7), V3(0.1, 2.45, 0.28))},
        ];
        KF.forEach(k => {
          const gh = gizmo(0.6, 0.03, 0.5);
          setPose(gh, k.T);
          g.add(gh);
        ...
        const seg = q => {
          let i = 0;
          while (i < KF.length - 2 && q > KF[i + 1].t) i++;
          return i;
        };
        function splitAt(q) {
          const i = seg(q),
        ...
        function screwAt(q) {
          ...
          return compose(
            A,
            expSE3(xi.rho.clone().multiplyScalar(u), xi.phi.clone().multiplyScalar(u))
          );
        }
        const chord = polyline(4, COL.teal, 0.85);
        g.add(chord);
        chord.userData.set(KF.map(k => k.T.p));
        const sPts = [];
        for (let i = 0; i < 120; i++) sPts.push(screwAt((i / 119) * 3).p);
        const screw = polyline(120, COL.violet2, 0.8);
        g.add(screw);
        screw.userData.set(sPts);
        const fA = gizmo(0.85, 0.045);
        g.add(fA);
        const fB = gizmo(0.7, 0.032, 0.55);
        g.add(fB);
        const gap = dimArrow(COL.red, 0.022, 0.9);
        g.add(gap);
        const l1 = makeLabel('lerp', HX.teal, 1.5);
        l1.position.set(-0.4, 2.2, 0);
        g.add(l1);
        const l2 = makeLabel('exp(u ξ)', HX.violet2, 2.2);
        l2.position.set(2.6, 2.2, 0);
        g.add(l2);
        let q = 0,
          auto = true;
        function place() {
          ...
          if (d.length() > 0.02) setArrow(gap, A.p, d);
          else gap.visible = false;
    ...
    function bindCard(i) {
      ...
      wireBubble('se3AdjOpInfo', 'se3AdjOpNote'); // card 9 · point vs. operator
      ...
      if (i === 6 && jacApi) {
        ...
        const th = document.getElementById('se3jth'),
          ro = document.getElementById('se3jrho'),
        ...
        if (sl) {
          sl.oninput = () => jacApi.set(parseFloat(sl.value));
          jacApi.set(parseFloat(sl.value));
        }
      }
      if (i === 9 && interpApi) {
        const sl = document.getElementById('se3isl'),
          qv = document.getElementById('se3iq'),
          gv = document.getElementById('se3igap'),
          au = document.getElementById('se3iauto');
        let lastQ = '',
          lastG = '';
