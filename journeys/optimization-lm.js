...
LIE.journeys['optimization-lm'] = (function () {
  ...
  const DY = [-0.72, -0.5, -0.35, -0.16, 0.14, 0.28, 0.55, 0.7, 0.98];
  const OUT_I = 7,
    OUT_Y = 2.6;
  ...
  function build(C, PAL) {
    ...
    function costCurve(g) {
      const pts = [];
      for (let i = 0; i <= 170; i++) {
        const x = -XMAX + (2 * XMAX * i) / 170;
        pts.push(V3(px(x), py(LOf(x)), 0));
      }
    ...
    const stations = [
      ...
      g => {
        ...
        const model = [];
        for (let i = 0; i <= 90; i++) {
          const d = -5.8 + (7.0 * i) / 90,
            v = r0 + J * d;
          model.push(V3(px(X0 + d), py(0.5 * v * v), 0));
        }
        line(g, model, COL.teal, 0.9);

        const at0 = dot(g, COL.coral, 0.11);
        at0.position.set(px(X0), py(LOf(X0)), 0);
        const promised = dot(g, COL.green, 0.1);
        promised.position.set(px(land), py(0), 0);
        const actual = dot(g, COL.coral, 0.12);
        actual.position.set(px(land), py(LOf(land)), 0);
        const gap = K.dashedLine(
          V3(px(land), py(0), 0),
          V3(px(land), py(LOf(land)), 0),
          COL.coral,
          0.1
        );
        g.add(gap);
        ...
        return {
          tick(t) {
            ...
            const shown = u > 0.99;
            actual.visible = shown;
            gap.visible = shown;
            lg.material.opacity = shown ? 0.95 : 0;
          },
        };
      },
      ...
      g => {
        ...
        const at0 = dot(g, COL.coral, 0.11);
        at0.position.set(px(X0), py(LOf(X0)), 0);
        const minDot = dot(g, COL.violet, 0.09);
        minDot.position.set(px(0), py(0), 0);
        const land = dot(g, COL.green, 0.12);
        const step = fatArrow(COL.green, 0.042);
        g.add(step);
        const lbl = makeLabel('λ = 0', HX.green, 3.6);
        lbl.position.set(0, 2.95, 0);
        g.add(lbl);
      ...
      g => {
        ...
        const inX = DX.filter((_, i) => i !== OUT_I),
          inY = DY.filter((_, i) => i !== OUT_I);
        ...
        const lbl = makeLabel('ρ(r) = ½ r²', HX.coral, 3.0);
