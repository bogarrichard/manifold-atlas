...
LIE.journeys['optimization-lm'] = (function(){
  ...
  return {
    ...
    cards: {
      ...
      ja: [
        {t:'ステップが大胆すぎるとき', b:'<p>Gauss–Newton のステップは<em>モデル</em>の放物線の底です。しかしモデルは局所的にしか正しくなく、ステップがその妥当な範囲に収まる保証はどこにもありません。</p><p>具体例、シーンにあるもの: <span class="m">r(x) = tanh x</span>、したがって <span class="m">L(x) = ½ tanh<sup>2</sup>x</span>。これは<em>飽和する</em>残差で、大きな <span class="m">x</span> ではほとんど変化しません。しかし線形化はそれを知りません: 残差が同じ傾きで下がり続けると信じています。</p><p><span class="m">x<sub>0</sub> = 1.5</span> から出発します。そこでは <span class="m">r<sub>0</sub> = 0.9051</span>、<span class="m">J = sech<sup>2</sup>(1.5) = 0.1807</span>、したがって</p><p class="matline"><span class="m">δ* = −r<sub>0</sub> / J = −5.01</span></p><p>モデルは、そこに着けばコストが<strong>ゼロ</strong>になると<em>約束</em>します — 青緑の放物線は実際そこで軸と交わります。ところが実際のコストは <strong>0.498</strong>、出発点（<span class="m">0.410</span>）より高い。<strong>このステップは事態を悪くしました。</strong></p><p>そして Gauss–Newton が悪いのではありません。線形化の誤差は <span class="m">O(‖δ‖<sup>2</sup>)</span> であり、私たちはたった今<em>5 単位</em>の長さを踏み出したのです。しかもこの失敗は体系的です: <button class="termbtn" id="lmStepInfo" type="button" aria-expanded="false" aria-controls="lmStepNote"><span class="m">J</span> が小さいところでは <span class="m">−g/H</span> が大きい</button> — モデルがいちばん信用ならないところで、ステップがいちばん巨大になります。</p><p>だから抑えるものが要ります。ステップの<em>向き</em>ではなく — そちらは問題ありません — その長さを。</p><div class="bubble" id="lmStepNote" role="dialog" aria-label="なぜステップが発散するのか" hidden><p>この例ではステップに閉じた形があります:</p><p class="matline"><span class="m">δ*(x) = − <span class="frac"><span>tanh x</span><span>sech<sup>2</sup>x</span></span> = − tanh x · cosh<sup>2</sup>x = − sinh x cosh x = −½ sinh 2x</span></p><p>つまりステップ長は <span class="m">x</span> について<strong>指数的に</strong>増えるのに、真の最小点は <span class="m">0</span> のままで、残りの道のりは線形にしか増えません。その比は限りなく大きくなります。</p><p>これはこの例に特有のことではありません。残差が飽和するところ — 角度、正規化された方向、逆深度 — ではどこでも <span class="m">J</span> は小さくなり、素の Gauss–Newton ステップは飛んでいきます。</p></div>'},
        {t:'減衰: H + λI', b:'<p>直し方は一行です。正規方程式に <span class="m">λ</span> を入れます:</p><p class="matline"><span class="m">(H + λI) δ = −g</span></p><p>両端を見ましょう。<span class="m">λ → 0</span>: ちょうど Gauss–Newton に戻り、放物線の底です。<span class="m">λ → ∞</span>: <span class="m">H</span> は無視できるほど小さくなり <span class="m">δ ≈ −g/λ</span> — <strong>勾配方向</strong>のごく小さな一歩です。</p><p>つまり <span class="m">λ</span> は二つの方法の間を調律します: 曲率を使う大胆な Gauss–Newton と、慎重だが頼れる勾配ステップの間を。名前の由来です: <em>Levenberg–Marquardt</em>。</p><p>一次元ではこの式で、シーンが動かしているのもこれです:</p><p class="matline"><span class="m">δ(λ) = − <span class="frac"><span>J r<sub>0</sub></span><span>J<sup>2</sup> + λ</span></span></span></p><p>同じ <span class="m">x<sub>0</sub> = 1.5</span>。<span class="m">λ = 0</span> では <span class="m">δ = −5.01</span>、前の駅の惨事です。<span class="m">λ ≈ 0.076</span> では <span class="m">δ = −1.51</span> — ちょうど最小点へ。<span class="m">λ</span> が大きければステップはゼロに近づきます: どこへも行きませんが、少なくとも悪くもしません。</p><p><strong>良い <span class="m">λ</span> はどうしてわかるのか。</strong>わかりません — 事前に選ぶものではないのです。ふつうの規則はこうです: ステップを取り、<em>真の</em>コストが本当に下がったか確かめる。下がったなら受け入れて <span class="m">λ</span> を<strong>下げる</strong>（モデルは信頼に足ると示された）。下がらなければステップを捨て、<span class="m">λ</span> を<strong>上げて</strong>やり直す。これが<em>信頼領域</em>の考え方で、<span class="m">λ</span> は線形化をどれだけ広い近傍で信じるかを間接的に決めています。</p><p>ひとつ、ただで手に入るものがあります: <span class="m">λ</span> が十分大きければ <button class="termbtn" id="lmLamInfo" type="button" aria-expanded="false" aria-controls="lmLamNote"><span class="m">H + λI</span> はつねに可逆</button>です。これは<em>定理</em>です。一方、<span class="m">λ</span> を上げ下げする段取りは<em>ヒューリスティック</em> — どちらがどちらか、知っておく価値があります。</p><div class="bubble" id="lmLamNote" role="dialog" aria-label="なぜ可逆なのか" hidden><p>前の衛星から: <span class="m">H = J<sup>⊤</sup>J</span> は半正定値なので、固有値はすべて <span class="m">μ<sub>i</sub> ≥ 0</span>。</p><p><span class="m">λI</span> を足すと、どの固有値もちょうど <span class="m">λ</span> だけずれます:</p><p class="matline"><span class="m">μ<sub>i</sub>(H + λI) = μ<sub>i</sub>(H) + λ ≥ λ > 0</span></p><p>よって <span class="m">H + λI</span> は正定値、したがって可逆です — <span class="m">H</span> が特異であっても。</p><p>素の Gauss–Newton が単に失敗する場面でも LM がステップを出せるのはこのためです。<strong>ただし注意:</strong> <span class="m">H</span> が特異だった理由が<em>情報の欠如</em>（ゲージ自由度）なら、<span class="m">λ</span> は情報を補いません — 同じくらい良い多数のステップから一つを選ぶだけです。症状を扱っていて、原因を扱ってはいません。</p></div>'},
        {t:'ロバストなカーネル', b:'<p>前の衛星の導出は、ただ一つの仮定に乗っていました: <strong>雑音はガウス</strong>。そうでない場合を問う頃合いです。</p><p>誤った対応付け、取り違えた特徴点は「大きなガウス雑音」ではありません。別の分布から来ており、実務では例外でもありません: 遮蔽、繰り返しテクスチャ、動く物体があれば、外れ値は<em>確実に</em>現れます。</p><p>そして二乗和はそれをとりわけ苦手とします。損失 <span class="m">ρ(r) = ½r<sup>2</sup></span> は<strong>二次で</strong>増えるので、10 倍の残差は<em>100 倍</em>効きます。シーンがそれを見せています: たった一つの珊瑚色の点が上へ登るにつれ、珊瑚色の直線 — 九点すべてに対する最小二乗解 — が素直について行きます。緑の直線は外れ値を除いた同じ当てはめで、びくともしません。</p><p>直し方は小細工ではなく<strong>別の雑音モデル</strong>です。裾が重い雑音なら、その負の対数は放物線ではありません。Huber のカーネル:</p><p class="matline"><span class="m">ρ(r) = ½r<sup>2</sup></span>&nbsp;&nbsp;（<span class="m">|r| ≤ k</span> のとき）</p><p class="matline"><span class="m">ρ(r) = k(|r| − ½k)</span>&nbsp;&nbsp;（それ以外）</p><p>小さい範囲では二次 — ガウス仮定が妥当な領域で、何も壊したくないからです。大きい範囲では<strong>線形</strong>なので、外れ値の<button class="termbtn" id="lmHuberInfo" type="button" aria-expanded="false" aria-controls="lmHuberNote">影響は有界に留まります</button>。</p><p>ここで避けたい言い回しの罠があります。これは<em>「精度をロバスト性と引き換えにする」</em>状況では<strong>ありません</strong>。裾ではガウス仮定がそもそも<strong>成り立たない</strong>ので、そこでの二乗和は精度が落ちるのではなく<em>目的関数として誤っている</em>のです。ロバストなカーネルは正しいモデルを使っています。</p><p><strong>この衛星が止まるところ。</strong>どのカーネルを、どの <span class="m">k</span> で — それは較正であって理論ではありません。続きは別の方向へ行きます: ここまで残差は名もない山でした。次の問いは<em>どの観測がどの変数に属するか</em> — それが問題に構造を与え、そもそも大規模に解けるようにするものです。それが<em>ファクタグラフ</em>の衛星です。</p><div class="bubble" id="lmHuberNote" role="dialog" aria-label="Huber の重みと IRLS" hidden><p>実務ではロバストなカーネルを別のソルバで解くのではなく、<strong>重み</strong>として適用します（IRLS — 反復再重み付け最小二乗）。重みは</p><p class="matline"><span class="m">w(r) = ρ′(r) / r</span></p><p>Huber なら</p><p class="matline"><span class="m">w(r) = 1</span>&nbsp;（<span class="m">|r| ≤ k</span>）,&nbsp;&nbsp;<span class="m">w(r) = k/|r|</span>&nbsp;（それ以外）</p><p>つまり大きな残差を、影響が <span class="m">k</span> を超えて増えなくなるちょうどの分だけ割り下げます。残差と Jacobian の両方に <span class="m">√w</span> を掛ければ Gauss–Newton の機構は<em>そのまま</em>使えます — だからロバストなカーネルは安いのです。</p><p><span class="m">k</span> の意味は「まだ信じられる」雑音の終わるところ。ふつうは雑音の標準偏差の小さな倍数で、白色化の後（前の衛星を参照）は無次元の数になります。</p><p>Cauchy のカーネルはさらに進みます: 重みが <span class="m">1/(1 + (r/k)<sup>2</sup>)</span> で、大きな <span class="m">r</span> ではゼロに向かいます — 外れ値の影響は有界どころか<em>消えます</em>。これが <em>redescending</em> な振る舞いで、シーンの緑の直線はその極限です。</p></div>'}
      ]
    }
  };
})();
