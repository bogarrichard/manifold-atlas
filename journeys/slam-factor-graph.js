  ...
  return {
    ...
    cards: {
      ...
      ja: [
        {
          t: '変数と因子',
          b: '<p>最適化の分野では、残差は名前のないひとかたまりでした。<span class="m">Σ<sub>i</sub> ½‖r<sub>i</sub>‖<sup>2</sup></span>、ただそれだけです。そろそろ<em>何が何に属しているのか</em>を見る頃です。</p><p><strong>因子グラフ</strong>（factor graph）は、二種類のノードを持つ<em>二部グラフ</em>です。</p><p><strong>変数ノード</strong>は、推定したいものです。位置姿勢、ランドマーク、IMU バイアス、速度など。シーンでは円で描かれ、紫が位置姿勢、緑がランドマークです。</p><p><strong>因子ノード</strong>は、分かっていることです。観測と拘束がこれにあたり、四角で描かれます。</p><p>辺の意味は<em>「この観測はこの変数に依存する」</em>です。そして各因子は、つながっている変数の現在の値から<strong>残差</strong>を返します。この観測の主張が、いまどれだけ満たされていないか、を表す量です。</p><p>二部構造になっているのは、様式の問題ではありません。二つの変数が直接つながることは決してなく、必ず因子を介します。推定量どうしの関係は、ある<em>観測</em>が両方について同時に語っている、という形以外にありえないからです。</p><p><button class="termbtn" id="fgTypesInfo" type="button" aria-expanded="false" aria-controls="fgTypesNote">代表的な因子の種類</button>は、いちばん単純なものから凝ったものまで、すべてこの同じ形に収まります。</p><div class="bubble" id="fgTypesNote" role="dialog" aria-label="因子の種類" hidden><p><strong>再投影観測。</strong>カメラ位置姿勢一つとランドマーク一つにつながります。残差は画素の誤差、つまりモデルの言う投影位置から実際に見えている位置を引いたものです。</p><p><strong>IMU 事前積分。</strong>連続する二つの位置姿勢につながり、ふつうは速度とバイアスも一緒に含みます。残差は、慣性観測が二つの状態のあいだの変化について語っている内容です。</p><p><strong>オドメトリと相対位置姿勢。</strong>連続する二つの位置姿勢につながります。シーンで紫の円のあいだにあるものです。</p><p><strong>ループ閉じ込み。</strong>時間的に<em>離れた</em>二つの位置姿勢につながります。構造はオドメトリと同一で、違いは回転が大きいことだけです。厳密な多様体の扱いがまさにここで必須になるのは、そのためです。</p><p><strong>事前分布。</strong>変数一つだけにつながります。辺が一本しかない<em>単項</em>因子で、シーンの緑の四角がこれです。</p><p>どれも同じことをしています。いくつかの変数から、残差と雑音モデルを作る。ソルバはそれ以上を知る必要がありません。</p></div>',
        },
        {
          t: 'グラフは MAP 推定の和の絵',
