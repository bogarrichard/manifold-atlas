  ...
  function build(C, PAL) {
    ...
    function sphere(g, color, r) {
      const d = new THREE.Mesh(
        new THREE.SphereGeometry(r || 0.16, 16, 12),
    ...
    function graph(parent, scale, ox, oy) {
      ...
      FACTORS.forEach(f => {
        ...
        if (f.v.length === 1) {
          fx = vPos(f.v[0])[0];
          fy = vPos(f.v[0])[1] - 1.15;
        } else {
          fx = (vPos(f.v[0])[0] + vPos(f.v[1])[0]) / 2;
          fy = (vPos(f.v[0])[1] + vPos(f.v[1])[1]) / 2;
        }
        const col = f.kind === 'prior' ? COL.green : COL.amber;
        const sq = square(G, col, 0.3);
    ...
    const stations = [
      ...
      g => {
        ...
        const lbl = makeLabel('x · l · factor', HX.ink, 3.4);
        ...
        return {
          tick(t) {
            gr.facts.forEach((sq, k) => {
              ...
              gr.edges[k].forEach(e => {
                e.material.opacity = 0.28 + 0.55 * w;
              });
            });
          },
        };
      ...
      g => {
        const gr = graph(g, 1.0, 0, -0.1);
        const lbl = makeLabel('Σ ½ r^{⊤} Σ^{−1} r', HX.amber, 4.0);
        lbl.position.set(0, 3.15, 0);
        g.add(lbl);
        ...
        return {
          tick(t) {
            ...
            if (k !== cur) {
              cur = k;
              const on = order[k];
              gr.facts.forEach((sq, j) => {
                sq.material.opacity = j === on ? 1.0 : 0.16;
              });
              gr.edges.forEach((es, j) =>
                es.forEach(e => {
                  e.material.opacity = j === on ? 0.95 : 0.08;
                })
              );
              const touched = FACTORS[on].v;
              gr.vars.forEach((s, j) => {
                s.material.opacity = touched.indexOf(j) >= 0 ? 1.0 : 0.22;
              });
            }
          },
        };
      ...
      g => {
        ...
        line(
          g,
          [V3(mx(0) - GAP * 0.62, my(-1) + 0.02, 0), V3(mx(NV - 1) + GAP * 0.62, my(-1) + 0.02, 0)],
          COL.grid1,
          0.5
        );
        line(
          g,
          [
            V3(mx(POSES.length) - GAP * 0.5, my(-1), 0),
            V3(mx(POSES.length) - GAP * 0.5, my(FACTORS.length), 0),
          ],
          COL.grid1,
          0.45
        );

        const lbl = makeLabel('J', HX.ink, 1.9);
        lbl.position.set(mx(3), Y0 + 0.7, 0);
        g.add(lbl);
        return {
          tick(t) {
            ...
            live.forEach(({m, r}) => {
              m.material.opacity = r === row ? 1.0 : 0.55;
            });
          },
        };
      },
