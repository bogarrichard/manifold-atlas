  ...
  function build(C, PAL) {
    ...
    const stations = [
      ...
      g => {
        const RX = 1.02,
          RMAX = 2.7;
        const bellY = r => 1.3 + 1.62 * Math.exp(-0.5 * r * r);
        const paraY = r => -2.25 + 0.52 * (0.5 * r * r);

        const bell = [],
          para = [];
        for (let i = 0; i <= 110; i++) {
          const r = -RMAX + (2 * RMAX * i) / 110;
          bell.push(V3(r * RX, bellY(r), 0));
          para.push(V3(r * RX, paraY(r), 0));
        }
        line(g, bell, COL.teal, 0.95);
        line(g, para, COL.amber, 0.95);
        line(g, [V3(-RMAX * RX, 1.3, 0), V3(RMAX * RX, 1.3, 0)], COL.grid1, 0.5);
        line(g, [V3(-RMAX * RX, -2.25, 0), V3(RMAX * RX, -2.25, 0)], COL.grid1, 0.5);
        line(g, [V3(0, -2.45, 0), V3(0, 3.05, 0)], COL.grid2, 0.45);

        const rb = dot(g, COL.teal, 0.1),
          rp = dot(g, COL.amber, 0.1);
        const link = line(g, [V3(0, 0, 0), V3(0, 0, 0)], COL.ink, 0.3);
        const lb = makeLabel('p(r) = e^{−r²/2}', HX.teal, 3.5);
        lb.position.set(0, 3.35, 0);
        g.add(lb);
        const lp = makeLabel('−log p = r²/2', HX.amber, 3.2);
        lp.position.set(0, -2.95, 0);
        g.add(lp);
        ...
        return {
          tick(t) {
            ...
            a.setXYZ(0, r * RX, bellY(r), 0);
            a.setXYZ(1, r * RX, paraY(r), 0);
            a.needsUpdate = true;
          },
        };
      },
      ...
      g => {
        ...
        const curve = [];
        for (let i = 0; i <= 160; i++) {
          const x = -XMAX + (2 * XMAX * i) / 160;
          curve.push(V3(px(x), py(LOf(x)), 0));
        }
        ...
        const lbl = makeLabel('r(δ) ≈ r₀ + J δ', HX.teal, 3.6);
        ...
        return {
          tick(t) {
            ...
            a.setXYZ(0, px(x0), Y0, 0);
            a.setXYZ(1, px(x0), py(LOf(x0)), 0);
            a.needsUpdate = true;
      ...
      g => {
        ...
        const minDot = dot(g, COL.green, 0.11);
        minDot.position.copy(goal);

        const gdBall = dot(g, COL.amber, 0.135);
        const gnBall = dot(g, COL.green, 0.135);
        const step = fatArrow(COL.green, 0.05);
        g.add(step);
        const lbl = makeLabel('H δ = −g', HX.green, 2.6);
        lbl.position.set(0, 4.35, 0);
        g.add(lbl);
        ...
        return {
          tick(t) {
            const tc = t % PER;
            if (tc < DW) {
              gdBall.position.copy(start);
              gnBall.position.copy(start);
              setArrow(step, start, V3(0, 0, 0));
            } else {
