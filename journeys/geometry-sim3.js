...
LIE.journeys['geometry-sim3'] = (function () {
  ...
  function build(C, PAL) {
    ...
    function body(g, L, op) {
      ...
      b.add(
        new THREE.Mesh(
          new THREE.BoxGeometry(L * 0.72, L * 0.72, L * 0.72),
    ...
    const stations = [
      ...
      g => {
        ...
        const lbl = makeLabel('S = [sR, t]', HX.ink, 3.0);
        lbl.position.set(0, 3.05, 0);
        g.add(lbl);
        const dof = makeLabel('7 DOF', HX.green, 1.9);
        dof.position.set(0, -2.5, 0);
        g.add(dof);
        ...
        return {
          tick(t) {
            ...
            const tv = V3(
              1.5 + 0.4 * Math.sin(t * 0.33),
              0.5 * Math.sin(t * 0.41),
              0.35 * Math.cos(t * 0.29)
            );
            b.scale.setScalar(s);
      ...
      g => {
        ...
        const curve = [];
        for (let i = 0; i <= 90; i++) {
          const sg = -SIG - 0.25 + ((2 * SIG + 0.5) * i) / 90;
          curve.push(V3(px(sg), py(Math.exp(sg)), 0));
        }
        line(g, curve, COL.amber, 0.95);
        const one = dot(g, COL.green, 0.085);
        one.position.set(px(0), py(1), 0); // the identity
        ...
        cube.position.set(3.3, py(1), 0);
        g.add(cube);

        const lbl = makeLabel('s = e^{σ}', HX.amber, 2.4);
        ...
        const lz = makeLabel('s ≤ 0', HX.coral, 1.9);
        lz.position.set(0.95, py(-0.85), 0);
        g.add(lz);

        return {
          tick(t) {
            const sg = SIG * Math.sin(t * 0.45),
              s = Math.exp(sg);
            const P = V3(px(sg), py(s), 0);
            rider.position.copy(P);
            const av = dropV.geometry.attributes.position;
            av.setXYZ(0, P.x, 0, 0);
            av.setXYZ(1, P.x, P.y, 0);
            av.needsUpdate = true;
            const ah = dropH.geometry.attributes.position;
            ah.setXYZ(0, 0, P.y, 0);
            ah.setXYZ(1, P.x, P.y, 0);
            ah.needsUpdate = true;
