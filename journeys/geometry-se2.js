...
LIE.journeys['geometry-se2'] = (function () {
  ...
  const CX = ((1 - CT) * TX - ST * TY) / DET,
    CY = (ST * TX + (1 - CT) * TY) / DET; // c = (I − R)⁻¹ t
  ...
  function build(C, PAL) {
    ...
    function frame(g) {
      // the scaled, recentred model space
    ...
    function dot(G, color, r) {
      const d = new THREE.Mesh(
        new THREE.SphereGeometry(r || 0.09, 14, 12),
    ...
    function axes(G) {
      dimArrow(G, COL.coral, 0.022, P(0, 0), V3(2.0, 0, 0), 0.3);
      dimArrow(G, COL.teal, 0.022, P(0, 0), V3(0, 2.0, 0), 0.3);
    }
    ...
    function arcAbout(cx, cy, r, a0, a1, seg) {
      ...
      for (let i = 0; i <= seg; i++) {
        ...
        p.push(P(cx + Math.cos(a) * r, cy + Math.sin(a) * r));
      }
      return p;
    }
    ...
    const stations = [
      ...
      g => {
        ...
        lt.position.copy(P(TX * 0.55, TY * 0.55)).add(V3(0.1, -0.34, 0));
        G.add(lt);
        const lth = makeLabel('θ', HX.coral, 1.2);
      ...
      g => {
        ...
        axes(G);
        dart(G, COL.ink, 0.22);
        const bA = dart(G, COL.amber),
          bV = dart(G, COL.violet);
        const endA = dot(G, COL.amber, 0.075);
        endA.position.copy(P(TX, TY));
        const endV = dot(G, COL.violet, 0.075);
        endV.position.copy(P(RT[0], RT[1]));
        const gap = K.dashedLine(P(TX, TY), P(RT[0], RT[1]), COL.green, 0.1);
        G.add(gap);
        const lg = makeLabel('(I − R) t', HX.green, 2.4);
        lg.position.copy(P((TX + RT[0]) / 2, (TY + RT[1]) / 2)).add(V3(0.15, 0.3, 0));
        G.add(lg);
        const lA = makeLabel('R p + t', HX.amber, 2.2);
        lA.position.copy(P(TX, TY)).add(V3(0.35, -0.42, 0));
        G.add(lA);
        const lV = makeLabel('R p + R t', HX.violet, 2.6);
        lV.position.copy(P(RT[0], RT[1])).add(V3(-0.5, 0.5, 0));
        G.add(lV);
      ...
      g => {
        ...
        const ghost = dart(G, COL.ink, 0.22);
        place(ghost, TX, TY, TH); // end pose
        ...
        const cDot = dot(G, COL.green, 0.105);
        cDot.position.copy(P(CX, CY));
        const lc = makeLabel('c', HX.green, 1.3);
        lc.position.copy(P(CX, CY)).add(V3(-0.36, 0.3, 0));
        G.add(lc);
        const b = dart(G, COL.amber);
