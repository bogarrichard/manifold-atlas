...
LIE.journeys['geometry-se2'] = (function(){
  ...
  return {
    ...
    cards: {
      hu: [
        {t:'Forgatás és eltolás egyben', b:'<p>Az <span class="m">SE(2)</span> a sík <em>merev</em> mozgásai: ami a hosszakat és a szögeket megtartja, és nem tükröz. Egy elemet két adat ír le — egy <span class="m">θ</span> szög és egy <span class="m">t</span> eltolásvektor —, összesen <strong>3 szabadsági fok</strong>.</p><p class="matline"><span class="m">T = (R(θ), t)</span>&nbsp;·&nbsp;<span class="m">p ⟼ R p + t</span></p><p>A jelenetben ez a két fél külön látszik: a test előbb elfordul a helyén <span class="m">θ</span>-val, aztán elcsúszik <span class="m">t</span>-vel.</p><p>Most fűzzünk össze kettőt. <button class="termbtn" id="se2HomInfo" type="button" aria-expanded="false" aria-controls="se2HomNote">Homogén alakban</button> ez sima mátrixszorzás, és ennyi jön ki:</p><p class="matline"><span class="m">T<sub>1</sub> T<sub>2</sub> = (R<sub>1</sub>R<sub>2</sub>, &nbsp;R<sub>1</sub>t<sub>2</sub> + t<sub>1</sub>)</span></p><p>Az egész állomás tartalma az <span class="m">R<sub>1</sub>t<sub>2</sub></span> tag. A második eltolást az első forgatás <strong>elforgatja</strong> — a két rész nem él egymás mellett függetlenül, hanem az egyik <em>hat</em> a másikra. Ezt hívják <em>fél-direkt szorzatnak</em>.</p><p>Ha ez a tag nem lenne ott, <span class="m">SE(2)</span> egyszerűen <span class="m">SO(2) × ℝ²</span> volna: egy szög és egy vektor, egymástól függetlenül, és minden felcserélhető. A következő állomás megmutatja, mennyire nem az.</p><div class="bubble" id="se2HomNote" role="dialog" aria-label="A homogen alak" hidden><p>A <span class="m">p ↦ Rp + t</span> hozzárendelés nem lineáris (a <span class="m">t</span> miatt), de eggyel nagyobb dimenzióban azzá tehető. Írjuk a pontot <span class="m">(p, 1)</span> alakban, a transzformációt pedig <span class="m">3×3</span>-asként:</p><p class="matline"><span class="m">T = ( R, t ; 0<sup>⊤</sup>, 1 )</span></p><p>soronként felsorolva. Ekkor</p><p class="matline"><span class="m">T (p, 1)<sup>⊤</sup> = (Rp + t, 1)<sup>⊤</sup></span></p><p>és két mozgás összefűzése <strong>mátrixszorzás</strong> lesz. Ez nem trükk, hanem a csoportszerkezet felmutatása: a szorzatmátrix bal felső blokkja <span class="m">R<sub>1</sub>R<sub>2</sub></span>, a jobb felső <span class="m">R<sub>1</sub>t<sub>2</sub> + t<sub>1</sub></span> — a fenti szabály egy sor számolás.</p><p>Ugyanez az alak megy tovább <span class="m">SE(3)</span>-ba és <span class="m">Sim(3)</span>-ba, csak nagyobb blokkokkal.</p></div>'},
        {t:'A sorrend számít', b:'<p>Vegyünk két nagyon egyszerű elemet: egy tiszta forgatást és egy tiszta eltolást.</p><p class="matline"><span class="m">R = (R(θ), 0)</span>&nbsp;·&nbsp;<span class="m">Tr = (I, t)</span></p><p>Fűzzük őket össze mindkét sorrendben. <button class="termbtn" id="se2OrderInfo" type="button" aria-expanded="false" aria-controls="se2OrderNote">Az előző szabályból</button>:</p><p class="matline"><span class="m">Tr · R = (R, t)</span>&nbsp;→&nbsp;<span class="m">p ↦ R p + t</span></p><p class="matline"><span class="m">R · Tr = (R, R t)</span>&nbsp;→&nbsp;<span class="m">p ↦ R(p + t) = R p + R t</span></p><p>Az <em>orientáció</em> mindkét esetben ugyanaz. A <em>hely</em> nem. A különbség pontosan:</p><p class="matline"><span class="m">t − R t = (I − R) t</span></p><p>A jelenetben <span class="m">θ = 1.25 rad</span> (≈ 72°) és <span class="m">t = (2.5, &nbsp;0.7)</span>. A borostyán test — előbb fordul, aztán csúszik — a <span class="m">(2.5, &nbsp;0.7)</span> pontban áll meg. A lila — előbb csúszik, aztán fordul — a <span class="m">(0.12, &nbsp;2.59)</span> pontban. Ugyanaz a két mozdulat, két nagyon különböző hely.</p><p>És most figyelj arra, honnan jön ez. <span class="m">SO(2)</span> <em>kommutatív</em> volt; a forgatásrész itt is az. A nem-kommutativitás nem a forgatásokból származik, hanem abból, hogy <strong>a forgatás megfogja az eltolást</strong>. Ez tétel, nem konvenció: amíg az <span class="m">R<sub>1</sub>t<sub>2</sub></span> tag ott van, addig a sorrend számít.</p><p>Jegyezd meg az <span class="m">(I − R)</span> operátort. A következő állomáson pontosan ezt fogjuk invertálni.</p><div class="bubble" id="se2OrderNote" role="dialog" aria-label="A ket szorzat" hidden><p>A szabály: <span class="m">T<sub>1</sub>T<sub>2</sub> = (R<sub>1</sub>R<sub>2</sub>, R<sub>1</sub>t<sub>2</sub> + t<sub>1</sub>)</span>.</p><p>Először <span class="m">T<sub>1</sub> = Tr = (I, t)</span> és <span class="m">T<sub>2</sub> = R = (R, 0)</span>:</p><p class="matline"><span class="m">(I·R, &nbsp;I·0 + t) = (R, t)</span></p><p>Most fordítva, <span class="m">T<sub>1</sub> = R</span> és <span class="m">T<sub>2</sub> = Tr</span>:</p><p class="matline"><span class="m">(R·I, &nbsp;R·t + 0) = (R, R t)</span></p><p>A bal oldali tényező forgatása mindig ráül a jobb oldali eltolására — ezért nem mindegy, melyik van bal oldalt.</p></div>'},
        {t:'Minden síkbeli mozgás egy forgatás — valahol', b:'<p>A <span class="m">T = (R(θ), t)</span> mozgásra eddig úgy néztünk, mint „elfordulás <em>plusz</em> elcsúszás”. Ez a felbontás azonban a mi választásunk volt — az origó választása. Kérdezzünk mást: van-e olyan pont, amit a mozgás <strong>helyben hagy</strong>?</p><p class="matline"><span class="m">R c + t = c</span>&nbsp;⟹&nbsp;<span class="m">(I − R) c = t</span>&nbsp;⟹&nbsp;<span class="m">c = (I − R)<sup>−1</sup> t</span></p><p>Ugyanaz az <span class="m">(I − R)</span>, ami az előző állomáson a két sorrend közti rést adta — most megoldunk vele egy egyenletet. És <button class="termbtn" id="se2FixInfo" type="button" aria-expanded="false" aria-controls="se2FixNote">invertálható is</button>, pontosan akkor, ha <span class="m">θ ≠ 0</span>.</p><p>Vagyis a mozgás nem forgatás <em>és</em> eltolás. <strong>Egyetlen forgatás a <span class="m">c</span> pont körül</strong> — az eltolás annak a látszata, hogy rossz pont körül néztük.</p><p>A számokkal: <span class="m">θ = 1.25</span>, <span class="m">t = (2.5, &nbsp;0.7)</span>, ebből <span class="m">c ≈ (0.76, &nbsp;2.08)</span>. A test a <span class="m">c</span> körüli, <span class="m">2.22</span> sugarú íven halad — ezt látod a jelenetben, és a végpont pontosan <span class="m">t</span>.</p><p>Mi történik <span class="m">θ = 0</span>-nál? Az <span class="m">(I − R)</span> szinguláris, <span class="m">c</span> „végtelenbe fut”: tiszta eltolás. Ez nem kivétel a szabály alól, hanem a határesete — a végtelen sugarú kör.</p><p><strong>És ez az <span class="m">exp</span> az <span class="m">SE(2)</span>-ben.</strong> Az egyparaméteres pálya, <span class="m">exp(s·ξ)</span>, pontosan ez az ív, nem az egyenes szakasz. <span class="m">SE(3)</span>-ban ugyanez a tétel egy fokkal gazdagabb: fix <em>pont</em> helyett fix <em>tengely</em> van, a test forog körülötte <em>és</em> halad mentén — ez a <strong>csavarmozgás</strong>. A levezetés az <span class="m">SE(3)</span> holdon.</p><div class="bubble" id="se2FixNote" role="dialog" aria-label="Mikor invertalhato" hidden><p>Írjuk ki:</p><p class="matline"><span class="m">I − R = ( 1 − cos θ, &nbsp;sin θ ; −sin θ, &nbsp;1 − cos θ )</span></p><p>soronként. A determináns:</p><p class="matline"><span class="m">det(I − R) = (1 − cos θ)² + sin²θ = 2(1 − cos θ)</span></p><p>Ez pontosan akkor nulla, ha <span class="m">cos θ = 1</span>, vagyis <span class="m">θ = 0</span>. Minden más szögnél az inverz létezik, tehát <strong>minden</strong> nem-triviális síkbeli merev mozgásnak van fixpontja.</p><p>Az is látszik, hogy kis <span class="m">θ</span>-nál a determináns <span class="m">≈ θ²</span>, tehát <span class="m">c</span> nagyon gyorsan nő — a majdnem-eltolás forgásközéppontja nagyon messze van.</p></div>'}
      ],
      en: [
        {t:'Rotation and Translation at Once', b:'<p><span class="m">SE(2)</span> is the set of <em>rigid</em> motions of the plane: what preserves lengths and angles and does not reflect. Two pieces of data describe one — an angle <span class="m">θ</span> and a translation <span class="m">t</span> — for <strong>3 degrees of freedom</strong>.</p><p class="matline"><span class="m">T = (R(θ), t)</span>&nbsp;·&nbsp;<span class="m">p ⟼ R p + t</span></p><p>The scene shows the two halves separately: the body first turns in place by <span class="m">θ</span>, then slides by <span class="m">t</span>.</p><p>Now compose two of them. <button class="termbtn" id="se2HomInfo" type="button" aria-expanded="false" aria-controls="se2HomNote">In homogeneous form</button> this is plain matrix multiplication, and out comes:</p><p class="matline"><span class="m">T<sub>1</sub> T<sub>2</sub> = (R<sub>1</sub>R<sub>2</sub>, &nbsp;R<sub>1</sub>t<sub>2</sub> + t<sub>1</sub>)</span></p><p>The whole content of this station is the <span class="m">R<sub>1</sub>t<sub>2</sub></span> term. The first rotation <strong>turns</strong> the second translation — the two parts do not sit independently side by side, one <em>acts on</em> the other. That is what a <em>semidirect product</em> means.</p><p>Without that term <span class="m">SE(2)</span> would simply be <span class="m">SO(2) × ℝ²</span>: an angle and a vector, independent, everything commuting. The next station shows how far from that we are.</p><div class="bubble" id="se2HomNote" role="dialog" aria-label="The homogeneous form" hidden><p>The map <span class="m">p ↦ Rp + t</span> is not linear (because of <span class="m">t</span>), but it becomes linear one dimension up. Write the point as <span class="m">(p, 1)</span> and the transformation as a <span class="m">3×3</span>:</p><p class="matline"><span class="m">T = ( R, t ; 0<sup>⊤</sup>, 1 )</span></p><p>listed row by row. Then</p><p class="matline"><span class="m">T (p, 1)<sup>⊤</sup> = (Rp + t, 1)<sup>⊤</sup></span></p><p>and composing two motions becomes <strong>matrix multiplication</strong>. This is not a trick but the group structure made visible: the product’s top-left block is <span class="m">R<sub>1</sub>R<sub>2</sub></span> and its top-right is <span class="m">R<sub>1</sub>t<sub>2</sub> + t<sub>1</sub></span> — the rule above is one line of arithmetic.</p><p>The same form carries into <span class="m">SE(3)</span> and <span class="m">Sim(3)</span>, with bigger blocks.</p></div>'},
        {t:'Order Matters', b:'<p>Take two very simple elements: a pure rotation and a pure translation.</p><p class="matline"><span class="m">R = (R(θ), 0)</span>&nbsp;·&nbsp;<span class="m">Tr = (I, t)</span></p><p>Compose them both ways round. <button class="termbtn" id="se2OrderInfo" type="button" aria-expanded="false" aria-controls="se2OrderNote">From the rule above</button>:</p><p class="matline"><span class="m">Tr · R = (R, t)</span>&nbsp;→&nbsp;<span class="m">p ↦ R p + t</span></p><p class="matline"><span class="m">R · Tr = (R, R t)</span>&nbsp;→&nbsp;<span class="m">p ↦ R(p + t) = R p + R t</span></p><p>The <em>orientation</em> is the same either way. The <em>place</em> is not. The difference is exactly:</p><p class="matline"><span class="m">t − R t = (I − R) t</span></p><p>In the scene <span class="m">θ = 1.25 rad</span> (≈ 72°) and <span class="m">t = (2.5, &nbsp;0.7)</span>. The amber body — turn first, then slide — stops at <span class="m">(2.5, &nbsp;0.7)</span>. The violet one — slide first, then turn — stops at <span class="m">(0.12, &nbsp;2.59)</span>. The same two moves, two very different places.</p><p>Now notice where this comes from. <span class="m">SO(2)</span> was <em>commutative</em>, and its rotation part still is. The non-commutativity does not come from the rotations; it comes from <strong>the rotation getting hold of the translation</strong>. That is a theorem, not a convention: as long as the <span class="m">R<sub>1</sub>t<sub>2</sub></span> term is there, order matters.</p><p>Remember the operator <span class="m">(I − R)</span>. The next station inverts exactly this.</p><div class="bubble" id="se2OrderNote" role="dialog" aria-label="The two products" hidden><p>The rule: <span class="m">T<sub>1</sub>T<sub>2</sub> = (R<sub>1</sub>R<sub>2</sub>, R<sub>1</sub>t<sub>2</sub> + t<sub>1</sub>)</span>.</p><p>First with <span class="m">T<sub>1</sub> = Tr = (I, t)</span> and <span class="m">T<sub>2</sub> = R = (R, 0)</span>:</p><p class="matline"><span class="m">(I·R, &nbsp;I·0 + t) = (R, t)</span></p><p>Now the other way, <span class="m">T<sub>1</sub> = R</span> and <span class="m">T<sub>2</sub> = Tr</span>:</p><p class="matline"><span class="m">(R·I, &nbsp;R·t + 0) = (R, R t)</span></p><p>The left factor’s rotation always lands on the right factor’s translation — which is why it matters which one is on the left.</p></div>'},
        {t:'Every Planar Motion Is a Rotation — Somewhere', b:'<p>So far we have read <span class="m">T = (R(θ), t)</span> as “a turn <em>plus</em> a slide”. But that split was our choice — the choice of origin. Ask something else instead: is there a point the motion <strong>leaves where it is</strong>?</p><p class="matline"><span class="m">R c + t = c</span>&nbsp;⟹&nbsp;<span class="m">(I − R) c = t</span>&nbsp;⟹&nbsp;<span class="m">c = (I − R)<sup>−1</sup> t</span></p><p>The same <span class="m">(I − R)</span> that gave the gap between the two orders one station ago — now we solve an equation with it. And it <button class="termbtn" id="se2FixInfo" type="button" aria-expanded="false" aria-controls="se2FixNote">is invertible</button>, exactly when <span class="m">θ ≠ 0</span>.</p><p>So the motion is not a rotation <em>and</em> a translation. It is <strong>a single rotation about the point <span class="m">c</span></strong> — the translation was an artefact of watching from the wrong centre.</p><p>With the numbers: <span class="m">θ = 1.25</span>, <span class="m">t = (2.5, &nbsp;0.7)</span>, giving <span class="m">c ≈ (0.76, &nbsp;2.08)</span>. The body rides an arc of radius <span class="m">2.22</span> about <span class="m">c</span> — that is the scene, and it ends exactly at <span class="m">t</span>.</p><p>What happens at <span class="m">θ = 0</span>? <span class="m">(I − R)</span> is singular and <span class="m">c</span> “runs off to infinity”: pure translation. Not an exception to the rule but its limiting case — the circle of infinite radius.</p><p><strong>And this is <span class="m">exp</span> in <span class="m">SE(2)</span>.</strong> The one-parameter orbit <span class="m">exp(s·ξ)</span> is precisely this arc, not the straight segment. In <span class="m">SE(3)</span> the same theorem is one notch richer: instead of a fixed <em>point</em> there is a fixed <em>axis</em>, and the body turns about it <em>while</em> advancing along it — a <strong>screw motion</strong>. That derivation belongs to the <span class="m">SE(3)</span> moon.</p><div class="bubble" id="se2FixNote" role="dialog" aria-label="When it is invertible" hidden><p>Written out:</p><p class="matline"><span class="m">I − R = ( 1 − cos θ, &nbsp;sin θ ; −sin θ, &nbsp;1 − cos θ )</span></p><p>row by row. The determinant:</p><p class="matline"><span class="m">det(I − R) = (1 − cos θ)² + sin²θ = 2(1 − cos θ)</span></p><p>which is zero exactly when <span class="m">cos θ = 1</span>, i.e. <span class="m">θ = 0</span>. At every other angle the inverse exists, so <strong>every</strong> non-trivial planar rigid motion has a fixed point.</p><p>It also shows that for small <span class="m">θ</span> the determinant is <span class="m">≈ θ²</span>, so <span class="m">c</span> grows very fast — the centre of rotation of a near-translation is very far away.</p></div>'}
      ],
      ja: [
        {t:'回転と並進を同時に', b:'<p><span class="m">SE(2)</span> は平面の<em>剛体</em>運動全体です: 長さと角を保ち、鏡映しないもの。一つの元は二つのデータ — 角 <span class="m">θ</span> と並進 <span class="m">t</span> — で決まり、<strong>自由度は 3</strong> です。</p><p class="matline"><span class="m">T = (R(θ), t)</span>&nbsp;·&nbsp;<span class="m">p ⟼ R p + t</span></p><p>シーンではこの二つの半分が別々に見えます: 物体はまずその場で <span class="m">θ</span> だけ回り、次に <span class="m">t</span> だけ滑ります。</p><p>では二つ合成してみましょう。<button class="termbtn" id="se2HomInfo" type="button" aria-expanded="false" aria-controls="se2HomNote">同次形</button>ならただの行列の積で、こう出ます:</p><p class="matline"><span class="m">T<sub>1</sub> T<sub>2</sub> = (R<sub>1</sub>R<sub>2</sub>, &nbsp;R<sub>1</sub>t<sub>2</sub> + t<sub>1</sub>)</span></p><p>この駅の中身はまるごと <span class="m">R<sub>1</sub>t<sub>2</sub></span> の項です。第一の回転が第二の並進を<strong>回してしまう</strong> — 二つの部分は独立に並んでいるのではなく、一方が他方に<em>作用</em>します。これが<em>半直積</em>の意味です。</p><p>この項がなければ <span class="m">SE(2)</span> はただの <span class="m">SO(2) × ℝ²</span>、つまり角とベクトルが独立で、すべてが可換になるはずでした。次の駅がその隔たりを見せます。</p><div class="bubble" id="se2HomNote" role="dialog" aria-label="同次形" hidden><p>写像 <span class="m">p ↦ Rp + t</span> は（<span class="m">t</span> のせいで）線形ではありませんが、次元を一つ上げれば線形になります。点を <span class="m">(p, 1)</span> と書き、変換を <span class="m">3×3</span> とします:</p><p class="matline"><span class="m">T = ( R, t ; 0<sup>⊤</sup>, 1 )</span></p><p>（行ごとの表記）。すると</p><p class="matline"><span class="m">T (p, 1)<sup>⊤</sup> = (Rp + t, 1)<sup>⊤</sup></span></p><p>となり、二つの運動の合成が<strong>行列の積</strong>になります。これは小手先の技ではなく、群構造を目に見える形にしたものです: 積の左上ブロックが <span class="m">R<sub>1</sub>R<sub>2</sub></span>、右上が <span class="m">R<sub>1</sub>t<sub>2</sub> + t<sub>1</sub></span> — 上の規則は計算一行です。</p><p>同じ形が <span class="m">SE(3)</span> や <span class="m">Sim(3)</span> へも、ブロックを大きくしただけで受け継がれます。</p></div>'},
        {t:'順序が効く', b:'<p>とても単純な二つの元を取ります: 純粋な回転と純粋な並進です。</p><p class="matline"><span class="m">R = (R(θ), 0)</span>&nbsp;·&nbsp;<span class="m">Tr = (I, t)</span></p><p>両方の順序で合成します。<button class="termbtn" id="se2OrderInfo" type="button" aria-expanded="false" aria-controls="se2OrderNote">上の規則から</button>:</p><p class="matline"><span class="m">Tr · R = (R, t)</span>&nbsp;→&nbsp;<span class="m">p ↦ R p + t</span></p><p class="matline"><span class="m">R · Tr = (R, R t)</span>&nbsp;→&nbsp;<span class="m">p ↦ R(p + t) = R p + R t</span></p><p>どちらでも<em>姿勢</em>は同じです。<em>場所</em>は違います。差はちょうど:</p><p class="matline"><span class="m">t − R t = (I − R) t</span></p><p>シーンでは <span class="m">θ = 1.25 rad</span>（≈ 72°）、<span class="m">t = (2.5, &nbsp;0.7)</span>。琥珀の物体 — 先に回り、後で滑る — は <span class="m">(2.5, &nbsp;0.7)</span> で止まります。菫の物体 — 先に滑り、後で回る — は <span class="m">(0.12, &nbsp;2.59)</span>。同じ二つの動作で、まるで違う場所です。</p><p>どこから来た差かに注意してください。<span class="m">SO(2)</span> は<em>可換</em>でしたし、ここでも回転部分は可換です。非可換性は回転から来るのではなく、<strong>回転が並進をつかまえる</strong>ことから来ます。これは約束ではなく定理です: <span class="m">R<sub>1</sub>t<sub>2</sub></span> の項がある限り、順序は効きます。</p><p>演算子 <span class="m">(I − R)</span> を覚えておいてください。次の駅で逆を取るのはまさにこれです。</p><div class="bubble" id="se2OrderNote" role="dialog" aria-label="二つの積" hidden><p>規則: <span class="m">T<sub>1</sub>T<sub>2</sub> = (R<sub>1</sub>R<sub>2</sub>, R<sub>1</sub>t<sub>2</sub> + t<sub>1</sub>)</span>。</p><p>まず <span class="m">T<sub>1</sub> = Tr = (I, t)</span>、<span class="m">T<sub>2</sub> = R = (R, 0)</span>:</p><p class="matline"><span class="m">(I·R, &nbsp;I·0 + t) = (R, t)</span></p><p>次に逆に、<span class="m">T<sub>1</sub> = R</span>、<span class="m">T<sub>2</sub> = Tr</span>:</p><p class="matline"><span class="m">(R·I, &nbsp;R·t + 0) = (R, R t)</span></p><p>左の因子の回転がつねに右の因子の並進に乗ります — だからどちらが左にあるかが効くのです。</p></div>'},
        {t:'平面のあらゆる運動は、どこかを中心とする回転', b:'<p>ここまで <span class="m">T = (R(θ), t)</span> を「回転 <em>＋</em> 並進」と読んできました。しかしこの分け方は私たちの選択 — 原点の選択でした。別の問いを立てましょう: この運動が<strong>その場に残す</strong>点はあるでしょうか。</p><p class="matline"><span class="m">R c + t = c</span>&nbsp;⟹&nbsp;<span class="m">(I − R) c = t</span>&nbsp;⟹&nbsp;<span class="m">c = (I − R)<sup>−1</sup> t</span></p><p>一つ前の駅で二つの順序の差を与えたのと同じ <span class="m">(I − R)</span> を、今度は方程式を解くのに使います。しかも <span class="m">θ ≠ 0</span> のとき、ちょうど<button class="termbtn" id="se2FixInfo" type="button" aria-expanded="false" aria-controls="se2FixNote">逆が存在します</button>。</p><p>つまりこの運動は回転<em>と</em>並進ではありません。<strong>点 <span class="m">c</span> を中心とする一つの回転</strong>です — 並進は、間違った中心から眺めていたことの見かけにすぎませんでした。</p><p>数で: <span class="m">θ = 1.25</span>、<span class="m">t = (2.5, &nbsp;0.7)</span> から <span class="m">c ≈ (0.76, &nbsp;2.08)</span>。物体は <span class="m">c</span> を中心とする半径 <span class="m">2.22</span> の弧を進みます — それがこのシーンで、終点はちょうど <span class="m">t</span> です。</p><p><span class="m">θ = 0</span> ではどうなるか。<span class="m">(I − R)</span> は特異になり、<span class="m">c</span> は「無限遠へ逃げ」ます: 純粋な並進です。規則の例外ではなく、その極限 — 半径無限大の円です。</p><p><strong>そしてこれが <span class="m">SE(2)</span> における <span class="m">exp</span> です。</strong>一径数の軌道 <span class="m">exp(s·ξ)</span> はまさにこの弧であって、直線分ではありません。<span class="m">SE(3)</span> では同じ定理が一段豊かになります: 不動<em>点</em>のかわりに不動<em>軸</em>があり、物体はそのまわりを回り<em>ながら</em>それに沿って進む — <strong>ねじれ運動</strong>です。その導出は <span class="m">SE(3)</span> の衛星のものです。</p><div class="bubble" id="se2FixNote" role="dialog" aria-label="いつ逆が存在するか" hidden><p>書き下すと:</p><p class="matline"><span class="m">I − R = ( 1 − cos θ, &nbsp;sin θ ; −sin θ, &nbsp;1 − cos θ )</span></p><p>（行ごと）。行列式は:</p><p class="matline"><span class="m">det(I − R) = (1 − cos θ)² + sin²θ = 2(1 − cos θ)</span></p><p>これがゼロになるのはちょうど <span class="m">cos θ = 1</span>、すなわち <span class="m">θ = 0</span> のときだけです。他のどの角でも逆は存在するので、<strong>自明でない平面剛体運動にはすべて</strong>不動点があります。</p><p>小さな <span class="m">θ</span> では行列式が <span class="m">≈ θ²</span> なので <span class="m">c</span> は非常に速く大きくなることも読み取れます — ほとんど並進に近い運動の回転中心は、はるか遠くにあるのです。</p></div>'}
      ]
    }
  };
})();
