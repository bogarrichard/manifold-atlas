...
LIE.journeys['geometry-se2'] = (function(){
  ...
  return {
    ...
    cards: {
      ...
      ja: [
        {t:'回転と並進を同時に', b:'<p><span class="m">SE(2)</span> は平面の<em>剛体</em>運動全体です: 長さと角を保ち、鏡映しないもの。一つの元は二つのデータ — 角 <span class="m">θ</span> と並進 <span class="m">t</span> — で決まり、<strong>自由度は 3</strong> です。</p><p class="matline"><span class="m">T = [R(θ), t]</span>&nbsp;·&nbsp;<span class="m">p ⟼ R p + t</span></p><p>シーンではこの二つの半分が別々に見えます: 物体はまずその場で <span class="m">θ</span> だけ回り、次に <span class="m">t</span> だけ滑ります。</p><p>では二つ合成してみましょう — 先に <span class="m">T<sub>1</sub></span>、次に <span class="m">T<sub>2</sub></span>。<button class="termbtn" id="se2HomInfo" type="button" aria-expanded="false" aria-controls="se2HomNote">同次形</button>ならただの行列の積で、こう出ます:</p><p class="matline"><span class="m">T<sub>2</sub> T<sub>1</sub> = [R<sub>2</sub>R<sub>1</sub>, &nbsp;R<sub>2</sub>t<sub>1</sub> + t<sub>2</sub>]</span></p><p>この駅の中身はまるごと <span class="m">R<sub>2</sub>t<sub>1</sub></span> の項です。<em>第二</em>の回転が<em>第一</em>の並進を<strong>回してしまう</strong> — 二つの部分は独立に並んでいるのではなく、一方が他方に<em>作用</em>します。これが<em>半直積</em>の意味です。</p><p>この項がなければ <span class="m">SE(2)</span> はただの <span class="m">SO(2) × ℝ<sup>2</sup></span>、つまり角とベクトルが独立で、すべてが可換になるはずでした。次の駅がその隔たりを見せます。</p><div class="bubble" id="se2HomNote" role="dialog" aria-label="同次形" hidden><p>写像 <span class="m">p ↦ Rp + t</span> は（<span class="m">t</span> のせいで）線形ではありませんが、次元を一つ上げれば線形になります。点に <span class="m">1</span> を付け足し、変換を <span class="m">3×3</span> とします:</p><p class="matline"><span class="m">T =</span><span class="matrix"><span class="mbracket"></span><span class="mgrid" style="grid-template-columns:repeat(2,auto)"><span>R</span><span>t</span><span>0<sup>⊤</sup></span><span>1</span></span><span class="mbracket right"></span></span></p><p>すると</p><p class="matline"><span class="m">T</span><span class="matrix"><span class="mbracket"></span><span class="mgrid" style="grid-template-columns:auto"><span>p</span><span>1</span></span><span class="mbracket right"></span></span><span class="m">=</span><span class="matrix"><span class="mbracket"></span><span class="mgrid" style="grid-template-columns:auto"><span>Rp + t</span><span>1</span></span><span class="mbracket right"></span></span></p><p>となり、二つの運動の合成が<strong>行列の積</strong>になります。これは小手先の技ではなく、群構造を目に見える形にしたものです: 積の左上ブロックが <span class="m">R<sub>2</sub>R<sub>1</sub></span>、右上が <span class="m">R<sub>2</sub>t<sub>1</sub> + t<sub>2</sub></span> — 上の規則は計算一行です。</p><p>同じ形が <span class="m">SE(3)</span> や <span class="m">Sim(3)</span> へも、ブロックを大きくしただけで受け継がれます。</p></div>'},
        {t:'順序が効く', b:'<p>とても単純な二つの元を取ります: 純粋な回転と純粋な並進です。</p><p class="matline"><span class="m">R = [R(θ), 0]</span>&nbsp;·&nbsp;<span class="m">Tr = [I, t]</span></p><p>両方の順序で合成します。<button class="termbtn" id="se2OrderInfo" type="button" aria-expanded="false" aria-controls="se2OrderNote">上の規則から</button>:</p><p class="matline"><span class="m">Tr · R = [R, t]</span>&nbsp;→&nbsp;<span class="m">p ↦ R p + t</span></p><p class="matline"><span class="m">R · Tr = [R, R t]</span>&nbsp;→&nbsp;<span class="m">p ↦ R(p + t) = R p + R t</span></p><p>どちらでも<em>姿勢</em>は同じです。<em>場所</em>は違います。差はちょうど:</p><p class="matline"><span class="m">t − R t = (I − R) t</span></p><p>シーンでは <span class="m">θ = 1.25 rad</span>（≈ 72°）、<span class="m">t = (2.5, &nbsp;0.7)</span>。琥珀の物体 — 先に回り、後で滑る — は <span class="m">(2.5, &nbsp;0.7)</span> で止まります。菫の物体 — 先に滑り、後で回る — は <span class="m">(0.12, &nbsp;2.59)</span>。同じ二つの動作で、まるで違う場所です。</p><p>どこから来た差かに注意してください。<span class="m">SO(2)</span> は<em>可換</em>でしたし、ここでも回転部分は可換です。非可換性は回転から来るのではなく、<strong>回転が並進をつかまえる</strong>ことから来ます。これは約束ではなく定理です: <span class="m">R<sub>2</sub>t<sub>1</sub></span> の項がある限り、順序は効きます。</p><p>演算子 <span class="m">(I − R)</span> を覚えておいてください。次の駅で逆を取るのはまさにこれです。</p><div class="bubble" id="se2OrderNote" role="dialog" aria-label="二つの積" hidden><p>規則（先に効くのは <span class="m">T<sub>1</sub></span>）: <span class="m">T<sub>2</sub>T<sub>1</sub> = [R<sub>2</sub>R<sub>1</sub>, R<sub>2</sub>t<sub>1</sub> + t<sub>2</sub>]</span>。</p><p>先に回転、次に並進 — <span class="m">T<sub>1</sub> = R = [R, 0]</span>、<span class="m">T<sub>2</sub> = Tr = [I, t]</span>:</p><p class="matline"><span class="m">[I·R, &nbsp;I·0 + t] = [R, t]</span></p><p>次は逆に — <span class="m">T<sub>1</sub> = Tr</span>、<span class="m">T<sub>2</sub> = R</span>:</p><p class="matline"><span class="m">[R·I, &nbsp;R·t + 0] = [R, R t]</span></p><p>左の因子の回転がつねに右の因子の並進に乗ります — だからどちらが左にあるかが効くのです。</p></div>'},
        {t:'平面のあらゆる運動は、どこかを中心とする回転', b:'<p>ここまで <span class="m">T = [R(θ), t]</span> を「回転 <em>＋</em> 並進」と読んできました。しかしこの分け方は私たちの選択 — 原点の選択でした。別の問いを立てましょう: この運動が<strong>その場に残す</strong>点はあるでしょうか。</p><p class="matline"><span class="m">R c + t = c</span>&nbsp;⟹&nbsp;<span class="m">(I − R) c = t</span>&nbsp;⟹&nbsp;<span class="m">c = (I − R)<sup>−1</sup> t</span></p><p>一つ前の駅で二つの順序の差を与えたのと同じ <span class="m">(I − R)</span> を、今度は方程式を解くのに使います。しかも <span class="m">θ ≠ 0</span> のとき、ちょうど<button class="termbtn" id="se2FixInfo" type="button" aria-expanded="false" aria-controls="se2FixNote">逆が存在します</button>。</p><p>つまりこの運動は回転<em>と</em>並進ではありません。<strong>点 <span class="m">c</span> を中心とする一つの回転</strong>です — 並進は、間違った中心から眺めていたことの見かけにすぎませんでした。</p><p>数で: <span class="m">θ = 1.25</span>、<span class="m">t = (2.5, &nbsp;0.7)</span> から <span class="m">c ≈ (0.76, &nbsp;2.08)</span>。物体は <span class="m">c</span> を中心とする半径 <span class="m">2.22</span> の弧を進みます — それがこのシーンで、終点はちょうど <span class="m">t</span> です。</p><p><span class="m">θ = 0</span> ではどうなるか。<span class="m">(I − R)</span> は特異になり、<span class="m">c</span> は「無限遠へ逃げ」ます: 純粋な並進です。規則の例外ではなく、その極限 — 半径無限大の円です。</p><p><strong>そしてこれが <span class="m">SE(2)</span> における <span class="m">exp</span> です。</strong>一径数の軌道 <span class="m">exp(s·ξ)</span> はまさにこの弧であって、直線分ではありません。<span class="m">SE(3)</span> では同じ定理が一段豊かになります: 不動<em>点</em>のかわりに不動<em>軸</em>があり、物体はそのまわりを回り<em>ながら</em>それに沿って進む — <strong>ねじれ運動</strong>です。その導出は <span class="m">SE(3)</span> の衛星のものです。</p><div class="bubble" id="se2FixNote" role="dialog" aria-label="いつ逆が存在するか" hidden><p>書き下すと:</p><p class="matline"><span class="m">I − R =</span><span class="matrix"><span class="mbracket"></span><span class="mgrid" style="grid-template-columns:repeat(2,auto)"><span>1 − cos θ</span><span>sin θ</span><span>−sin θ</span><span>1 − cos θ</span></span><span class="mbracket right"></span></span></p><p>行列式は:</p><p class="matline"><span class="m">det(I − R) = (1 − cos θ)<sup>2</sup> + sin<sup>2</sup>θ = 2(1 − cos θ)</span></p><p>これがゼロになるのはちょうど <span class="m">cos θ = 1</span>、すなわち <span class="m">θ = 0</span> のときだけです。他のどの角でも逆は存在するので、<strong>自明でない平面剛体運動にはすべて</strong>不動点があります。</p><p>小さな <span class="m">θ</span> では行列式が <span class="m">≈ θ<sup>2</sup></span> なので <span class="m">c</span> は非常に速く大きくなることも読み取れます — ほとんど並進に近い運動の回転中心は、はるか遠くにあるのです。</p></div>'}
      ]
    }
  };
})();
