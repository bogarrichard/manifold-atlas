  ...
  return {
    ...
    cards: {
      ...
      ja: [
        ...
        {t:'なぜ e なのか', b:'<p>前のカードの脚注は <span class="m">e</span> を二度使いました — 一度はべき級数として、一度は <span class="m">(1 + v/n)<sup>n</sup></span> の極限として。この極限が <span class="m">⊞</span> になる前に、その数そのものがどこから来るのかを見ておく価値があります。</p><p>任意の底の指数関数を定義どおりに微分し、<span class="m">a<sup>x</sup></span> をくくり出します:</p><p class="matline"><span class="m"><span class="frac"><span>d</span><span>dx</span></span> a<sup>x</sup> = a<sup>x</sup> · lim<sub>h→0</sub> <span class="frac"><span>a<sup>h</sup> − 1</span><span>h</span></span></span></p><p>括弧の中は <strong><span class="m">x</span> に依存しません</strong> — 底だけで決まる一つの数です。これを <span class="m">C<sub>a</sub></span> と呼びましょう。つまり<em>どの</em>指数関数も、係数一つの違いを除いて自分自身の微分になっています:</p><p class="matline"><span class="m">d/dx a<sup>x</sup> = C<sub>a</sub> · a<sup>x</sup></span></p><p>ではその数はいくつでしょうか。<span class="m">h → 0</span> のとき分子も分母も 0 に向かいます: <span class="m">0/0</span>。問題は 0 に向かうかどうかではなく、<button class="termbtn" id="gfCaInfo" type="button" aria-expanded="false" aria-controls="gfCaNote">互いにどれだけの速さで向かうか</button>です — これが競走です。</p><p class="matline"><span class="m">a = 2 &nbsp;→&nbsp; C ≈ 0.69</span></p><p class="matline"><span class="m">a = 3 &nbsp;→&nbsp; C ≈ 1.10</span></p><p>一方は分子が分母より遅く減り、他方は速く減ります。しかも <span class="m">C<sub>a</sub></span> は底について連続に増えます — したがって <strong>2 と 3 のあいだに、競走が引き分けになる底がちょうど一つ存在します。</strong> それが <span class="m">e</span> です。選択ではなく、交点です。</p><p>シーンではそれが割線の傾きです。<span class="m">h</span> が縮むにつれ、青緑は <span class="m">0.69</span> へ、菫は <span class="m">1.10</span> へ寄り、琥珀だけが傾きちょうど <span class="m">1</span> の緑の破線に重なります。</p><p>引き分けが何を意味するかを書き、両辺を <span class="m">1/h</span> 乗します:</p><p class="matline"><span class="m">e<sup>h</sup> ≈ 1 + h &nbsp;⟹&nbsp; e ≈ (1 + h)<sup>1/h</sup></span></p><p class="matline"><span class="m">e = lim<sub>n→∞</sub> (1 + 1/n)<sup>n</sup> ≈ 2.71828</span></p><p><strong>つまり複利は喩えではなく、定義そのものです。</strong> だからこそ同じ形がどの階層にもそのまま現れます: 円の上では <span class="m">(1 + iα/n)<sup>n</sup></span>、次の駅では <span class="m">(I + X/n)<sup>n</sup></span>。極限は一つ、対象が違うだけです。<button class="termbtn" id="gfSerInfo" type="button" aria-expanded="false" aria-controls="gfSerNote">べき級数も同じことを言います</button> — 別の形で。</p><div class="bubble" id="gfCaNote" role="dialog" aria-label="極限の閉じた形" hidden><p><span class="m">t = a<sup>h</sup> − 1</span> と置くと、極限は閉じた形になります:</p><p class="matline"><span class="m">C<sub>a</sub> = lim<sub>t→0</sub> <span class="frac"><span>t</span><span>log<sub>a</sub>(1 + t)</span></span> = <span class="frac"><span>1</span><span>log<sub>a</sub> K</span></span></span></p><p>ここで <span class="m">K = lim<sub>t→0</sub>(1 + t)<sup>1/t</sup></span> は底を含まない普遍定数です。すると <span class="m">C<sub>a</sub> = 1</span> という条件は文字どおり <span class="m">a = K</span> を意味します。</p><p>つまり「どの底で微分の係数が 1 になるか」と「<span class="m">(1 + 1/n)<sup>n</sup></span> の極限は何か」は<em>同じ問い</em>を二通りに述べたものです。おなじみの形も従います: <span class="m">C<sub>a</sub> = ln a</span>。</p></div><div class="bubble" id="gfSerNote" role="dialog" aria-label="級数は自分自身を再生する" hidden><p><span class="m">e<sup>x</sup></span> の級数を項ごとに微分すると、定数項は消え、残りの項は <span class="m">k!</span> のおかげで<strong>一つずつ左へずれます</strong>:</p><p class="matline"><span class="m">e<sup>x</sup> = 1 + x + <span class="frac"><span>x<sup>2</sup></span><span>2!</span></span> + <span class="frac"><span>x<sup>3</sup></span><span>3!</span></span> + …</span></p><p class="matline"><span class="m">→ 0 + 1 + <span class="frac"><span>2x</span><span>2!</span></span> + <span class="frac"><span>3x<sup>2</sup></span><span>3!</span></span> + … = e<sup>x</sup></span></p><p>級数の構造がそっくりそのまま自分を再生します。だから <span class="m">ẏ = y</span> は成り立つだけでなく、級数の形に<em>見えて</em>います。</p><p>そしてこの形は行列へもそのまま移ります: <span class="m">x</span> のあったところに <span class="m">A</span> と書けば、同じ計算が <span class="m">exp(A)</span> を与えます。</p></div>'},
        {t:'⊞ の三つの公理', b:'<p>さきほどは測地線が直線だったので、ステップは <span class="m">exp<sub>p</sub>(v) = p + v</span> でした。曲がった空間ではステップを <span class="m">⊞</span> と書きます。では <span class="m">⊞</span> は<em>何でありうる</em>のでしょうか。実はほとんど選択肢がありません。三つの無害な要求が、たった一つの式だけを生き残らせます。</p><p class="matline"><span class="m">p ⊞ 0 = p</span></p><p class="matline"><span class="m">(p ⊞ t<sub>1</sub>v) ⊞ t<sub>2</sub>v = p ⊞ (t<sub>1</sub> + t<sub>2</sub>)v</span></p><p class="matline"><span class="m"><span class="frac"><span>d</span><span>dt</span></span> (p ⊞ tv) |<sub>t=0</sub> = v</span></p><p>順に: ゼロのステップは何も動かさない; 同じ向きの二つのステップは一つの大きなステップである; そして <span class="m">v</span> は本当に出発時の速度である。どれにも指数関数は入っていません — どれにも<em>式</em>が入っていません。三つとも、運動が滑らかで自分自身と整合的であることだけを要求しています。</p><p>多様体の点は足せません。しかし群の元は点に<em>変換として作用します</em>: <span class="m">tv</span> のステップを行う変換を <span class="m">g(t)</span> としましょう。ステップを <span class="m">n</span> 等分すると、公理 2 によって同じ場所に着き、変換は<button class="termbtn" id="gfMulInfo" type="button" aria-expanded="false" aria-controls="gfMulNote">掛け合わされます</button>:</p><p class="matline"><span class="m">p ⊞ v = [ g(1/n) ]<sup>n</sup> · p</span></p><p><span class="m">g(0)</span> は何でしょうか。静止 — すなわち群の単位元、行列で書けば <span class="m">I</span> です。<strong>ここが <span class="m">I</span> の出どころです</strong>。いつも何もないところから現れるように見えるあの <span class="m">I</span> です。<span class="m">1/n</span> は無限小なので、<span class="m">g</span> を 0 のまわりで展開します:</p><p class="matline"><span class="m">g(1/n) = I + X/n + O(1/n<sup>2</sup>)</span></p><p>ここで <span class="m">X = g′(0)</span> は単位元における速度 — まさにリー代数の元です。代入すると:</p><p class="matline"><span class="m">p ⊞ v = lim<sub>n→∞</sub> (I + X/n)<sup>n</sup> · p = exp(X) · p</span></p><p>前のカードと同じ極限で、<span class="m">1/n</span> が <span class="m">X/n</span> になっただけです。シーンの琥珀の折れ線はまさにこれです: <em>頂点ごとに <span class="m">(I + X/n)</span> の掛け算が一回</em>、正規化はしません。<span class="m">n = 1</span> では一回の大きなステップが球からはみ出し、<span class="m">n</span> が増えると折れ線は緑の測地線に重なっていきます。</p><p><strong>つまり ⊞ は定義ではなく定理です。</strong> <span class="m">exp</span> であるのは、便利だからでも、たまたま拘束を保つからでもなく、滑らかさと合成可能性が二つそろうと<em>他を許さない</em>からです。はみ出しが極限でなぜ消えるのかは、<span class="m">SO(2)</span> の月が次数ごとに計算します。</p><p>そして同じ構成が四つの階層で、いつも同じ微分方程式から走ります:</p><p class="matline"><span class="m">ℝ: &nbsp; ẏ = c·y &nbsp;→&nbsp; e<sup>c</sup></span></p><p class="matline"><span class="m">S<sup>1</sup> ⊂ ℂ: &nbsp; ẏ = i·y &nbsp;→&nbsp; cos θ + i sin θ</span></p><p class="matline"><span class="m">リー群: &nbsp; γ̇ = X·γ &nbsp;→&nbsp; exp(X)</span></p><p class="matline"><span class="m">多様体: &nbsp; ∇<sub>γ̇</sub> γ̇ = 0 &nbsp;→&nbsp; γ(1)</span></p><p>どの行も同じことを言っています: <em>変化の速さは今いる位置に比例する</em>。行から行へ変わるのは、掛け算が何を意味するかだけです。</p><div class="bubble" id="gfMulNote" role="dialog" aria-label="べきはどこから来るか" hidden><p>公理 2 により、<span class="m">v/n</span> のステップ <span class="m">n</span> 回は <span class="m">v</span> のステップ一回と同じです:</p><p class="matline"><span class="m">p ⊞ v = ( … (p ⊞ v/n) ⊞ … ⊞ v/n )</span></p><p>各 <span class="m">⊞</span> は裏で変換を一つ適用しており、変換を次々に適用することは変換を<strong>掛け合わせる</strong>ことです。同じステップが <span class="m">n</span> 回なら、同じ因子が <span class="m">n</span> 回です:</p><p class="matline"><span class="m">g(1/n) · g(1/n) · … · g(1/n) = [ g(1/n) ]<sup>n</sup></span></p><p>ここでべきが生まれます。べき乗されるのは<em>点</em>ではなく — 点どうしの積は無意味です — 変換のほうです。指数は合成の帳簿づけなのです。</p></div>'}
      ]
    }
  };
})();
