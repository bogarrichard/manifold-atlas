...
LIE.journeys['geometry-flat'] = (function () {
  ...
  const GX0 = -1.3,
    GX1 = 1.1; // the plotted x range
  const HMAX = 1.05,
    HMIN = 0.06; // the secant offset h sweeps this
  ...
  const WU = (function () {
    const w = V3(1, 0.25, 0);
    return w.sub(PU.clone().multiplyScalar(w.dot(PU))).normalize();
  })();
  ...
  function build(C, PAL) {
    ...
    const stations = [
      ...
      g => {
        ...
        [
          [V3(2.2, 0, 0), COL.coral],
          [V3(0, 2.2, 0), COL.teal],
          [V3(0, 0, 2.2), COL.violet],
        ].forEach(([v, c]) => {
          const a = fatArrow(c, 0.05);
          setArrow(a, V3(0, 0, 0), v);
          triad.add(a);
        });
        g.add(triad);
        const pts = new THREE.Group();
        [V3(1.6, 0.8, -1.2), V3(-2.0, 1.4, 0.9), V3(0.6, 2.0, 1.8), V3(-1.2, 0.5, -2.1)].forEach(
          p => {
            const d = new THREE.Mesh(
        ...
        g.add(pts);
        const lbl = makeLabel('ℝ^{n}', HX.ink, 2.0);
        lbl.position.set(0, 3.0, 0);
        g.add(lbl);
        return {
          tick(t) {
            triad.rotation.y = t * 0.25;
          },
        };
      },
      ...
      g => {
        ...
        const items = bases.map(b => {
          const a = fatArrow(COL.teal, 0.05);
          g.add(a);
        ...
        const lbl = makeLabel('Tₚℝ^{n} ≅ ℝ^{n}', HX.teal, 3.6);
        lbl.position.set(0, 2.9, 0);
        g.add(lbl);
        return {
          tick(t) {
      ...
      g => {
        ...
        [
          [V3(2.0, 0, 0), COL.coral],
          [V3(0, 2.0, 0), COL.teal],
          [V3(0, 0, 2.0), COL.violet],
        ].forEach(([v, c]) => {
        ...
        g.add(
          new THREE.Mesh(
            ...
            new THREE.MeshBasicMaterial({color: COL.ink, transparent: true, opacity: 0.55})
          )
        ); // origin
        ...
        g.add(ball);
        const lp = makeLabel('p', HX.violet, 2.2);
        lp.position.copy(P0).add(V3(-0.55, 0.45, 0));
        g.add(lp);
        const lv = makeLabel('v', HX.coral, 2.2);
        lv.position
          .copy(P0)
          .add(VV.clone().multiplyScalar(0.5))
          .add(V3(0.4, 0.45, 0));
        g.add(lv);
        const lq = makeLabel('p + v', HX.amber, 2.6);
        lq.position.copy(Q0).add(V3(0, 0.6, 0));
        g.add(lq);
        const T0 = 1.1,
          TT = 1.7,
          T1 = 1.3,
          PER = T0 + TT + T1; // dwell p · travel · dwell p+v
        return {
          tick(t) {
            const tc = t % PER;
            const u = tc < T0 ? 0 : tc < T0 + TT ? ease((tc - T0) / TT) : 1;
            ball.position.lerpVectors(P0, Q0, u);
            lq.material.opacity = 0.12 + 0.88 * u;
            lq.visible = u > 0.02;
          },
        };
      },
      ...
      g => {
        ...
        BASES.forEach((a, i) => {
          const pts = [];
          for (let k = 0; k <= 96; k++) {
            const x = GX0 + ((GX1 - GX0) * k) / 96;
            pts.push(GP(x, Math.pow(a, x)));
          }
          line(g, pts, CC[i], 0.9);
        });
        // every a^x passes through (0,1) — the one point all three share
        const p1 = GP(0, 1);
        dot(g, COL.ink, 0.075).position.copy(p1);
        // the slope-1 reference: the line the winning secant has to land on
        const rd = V3(GSX, GSY, 0).normalize().multiplyScalar(2.15);
        g.add(K.dashedLine(p1.clone().sub(rd), p1.clone().add(rd), COL.green, 0.1));
        const lref = makeLabel('1', HX.green, 0.6);
        lref.position.copy(p1.clone().add(rd)).add(V3(0.2, 0.18, 0));
        g.add(lref);

        const secs = BASES.map((a, i) => ({
          a,
          line: seg(g, CC[i], 0.9),
          chord: seg(g, CC[i], 0.28),
          rider: dot(g, CC[i], 0.07),
          lbl: makeLabel('', HX.ink, 1.45),
        }));
        secs.forEach((s, i) => {
          s.lbl.position.set(2.05, 1.55 - i * 0.46, 0);
          g.add(s.lbl);
        });
        const lh = makeLabel('', HX.ink, 2.5);
        lh.position.set(-0.35, 2.25, 0);
        g.add(lh);
      ...
      g => {
        ...
        setArrow(arr, P, WU.clone().multiplyScalar(THETA * SPH_R));
        g.add(arr);
        ...
        for (let k = 0; k <= 72; k++)
          arc.push(
            PU.clone()
              .applyAxisAngle(AXIS5, (THETA * k) / 72)
              .multiplyScalar(SPH_R)
          );
        ...
        const pos = new Float32Array((MAXN + 1) * 3),
          geo = new THREE.BufferGeometry();
        geo.setAttribute('position', new THREE.BufferAttribute(pos, 3));
        ...
        const lp = makeLabel('p', HX.violet, 0.7);
        lp.position.copy(P).add(V3(-0.42, 0.34, 0));
        g.add(lp);
        const lq = makeLabel('exp(X) p', HX.green, 2.0);
        lq.position.copy(qEnd).add(V3(0.5, 0.5, 0));
        g.add(lq);
        const ln = makeLabel('', HX.amber, 2.4);
        ln.position.set(0, 3.15, 0);
        g.add(ln);
    ...
    function bindCard(i) {
      ...
      wireBubble('gfCurveInfo', 'gfCurveNote'); // card 4 · why the polygon curves
    }
