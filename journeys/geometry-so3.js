  ...
  return {
    ...
    cards: {
      ...
      en: [
        ...
        {t:'Gimbal Lock Is a Theorem, Not a Nuisance', b:'<p>Euler angles stack three rotations inside one another — the three rings in the scene. Watch what happens as the middle one, <span class="m">β</span>, approaches 90°: the outer and the inner ring’s axes <strong>coincide</strong>. Three dials, but only two independent directions left. A degree of freedom has vanished.</p><p>The usual reaction is that this is clumsiness on the part of Euler angles, and that a smarter parameterisation repairs it. <strong>It does not.</strong> This is the one impossibility theorem in the subject:</p><p class="matline"><span class="m">there is no global, singularity-free 3-parameter coordinate for SO(3)</span></p><p>Such a coordinate would by definition be a <button class="termbtn" id="so3gTopInfo" type="button" aria-expanded="false" aria-controls="so3gTopNote">diffeomorphism</button> between <span class="m">SO(3)</span> and an open piece of <span class="m">ℝ<sup>3</sup></span>. But diffeomorphisms preserve every topological invariant, and <em>two</em> of them break here, independently:</p><p class="matline"><span class="m">SO(3) is compact</span></p><p class="matline"><span class="m">open subsets of ℝ<sup>3</sup> are not</span></p><p class="matline"><span class="m">π<sub>1</sub>(SO(3)) = ℤ/2</span></p><p class="matline"><span class="m">π<sub>1</sub>(ℝ<sup>3</sup>) = 0</span></p><p>The first alone is enough. So <em>any</em> 3-parameter representation — Euler angles, the Rodrigues vector, axis-angle packed as a 3-vector — <strong>must break somewhere</strong>. The choice is not bad; three flat numbers are too few.</p><p>What looks like an escape, and what it actually buys. The <strong>quaternion</strong> steps up one dimension: <span class="m">S<sup>3</sup></span> is smooth and singularity-free, trading a <em>constraint</em> (<span class="m">‖q‖ = 1</span>) for <em>smoothness</em> — but <span class="m">S<sup>3</sup></span> is still compact and still curved, so we are no closer to flat. An <strong>atlas</strong> — several local charts with overlaps — does work, but then we have abandoned the ambition of a global flat coordinate rather than met it.</p><p>The practical answer is the combination of the two: store the rotation <em>redundantly</em> (matrix or quaternion), but take the <em>increment</em> as a 3-vector in the local tangent space. Exactly how that works — and what <span class="m">SO(3)</span>’s true, <span class="m">ℤ/2</span>-shaped form looks like — is what the <em>Riemannian gradient</em> moon shows.</p><div class="bubble" id="so3gTopNote" role="dialog" aria-label="The two invariants" hidden><p>A <em>diffeomorphism</em> is a one-to-one correspondence that is smooth in both directions. If two spaces admit one, they are topologically the same space drawn differently.</p><p><strong>Compactness.</strong> <span class="m">SO(3)</span> is closed and bounded (its matrix entries lie in <span class="m">[−1, 1]</span>): every infinite sequence has a convergent subsequence. From an open subset of <span class="m">ℝ<sup>3</sup></span> you can instead “escape” towards the boundary. A smooth correspondence cannot bridge that.</p><p><strong>Simple connectedness.</strong> <span class="m">π<sub>1</sub></span> counts how many essentially different loops a space has. In <span class="m">ℝ<sup>3</sup></span> every loop contracts to a point, so <span class="m">π<sub>1</sub> = 0</span>. In <span class="m">SO(3)</span> the loop of a 360° rotation <strong>does not</strong> contract — while the 720° one does. That is the <span class="m">ℤ/2</span>, and it is what the belt trick demonstrates.</p><p>Either one alone kills the global flat coordinate; that both hold only underlines it.</p></div>'}
      ],
      ja: [
        {t:'どの回転にも軸がある', b:'<p><span class="m">R ∈ SO(3)</span> は<em>実際には</em>何をしているのか。答えは Euler の定理で、驚くほど強い主張です: <strong>どの</strong>回転にも軸がある — その場に残す直線がある、ということです。</p><p>「たいていの回転には」でも「行儀のよい回転には」でもありません。すべてに、です。しかも<button class="termbtn" id="so3gEulerInfo" type="button" aria-expanded="false" aria-controls="so3gEulerNote">証明は三行</button>。</p><p>シーンがそれを見せています: 緑の直線は静止し、他のどの点もそのまわりに円を描きます。だからどの回転もこう書けます:</p><p class="matline"><span class="m">R = R(a, θ)</span></p><p class="matline"><span class="m">‖a‖ = 1</span></p><p>単位ベクトル <span class="m">a</span> と角 <span class="m">θ</span>。ここから自由度を数えましょう: <span class="m">a</span> は単位球面上の点で <strong>2</strong>、<span class="m">θ</span> は数一つで <strong>1</strong>。合わせて <strong>3</strong> — 拘束 <span class="m">R<sup>⊤</sup>R = I</span> から出てくるのと同じ 3 に、まったく別の道から着きました。</p><p>そしてこれを先に置く理由: 軸と角は<em>一つの</em>ベクトルに掛け合わせられます。</p><p class="matline"><span class="m">ω = a θ ∈ ℝ<sup>3</sup></span></p><p>これはふつうの 3 次元ベクトルです: 足せる、スケールできる、微分できる。これこそ <span class="m">exp</span> が群へ戻す接ベクトルです。<span class="m">ω</span> の住む空間は平坦で、<span class="m">R</span> の住む空間はそうではない — この二つの間の往復が、機械のすべてです。</p><p>ひとつ引っかかりがあります: <span class="m">exp</span> はベクトルを受け取りません。その級数 — <span class="m">I + X + ½X<sup>2</sup> + …</span> — は自分自身と掛けられる <span class="m">X</span> を要求しますが、<span class="m">ω</span> は三つの数の列です。<button class="termbtn" id="so3gHatInfo" type="button" aria-expanded="false" aria-controls="so3gHatNote">ハット</button>がその隙間を埋めます: 三つの数を、外積を再現する行列に並べ替えるのです。</p><p class="matline"><span class="m">ω<sup>∧</sup> =</span><span class="matrix"><span class="mbracket"></span><span class="mgrid"><span>0</span><span>&minus;ω<sub>3</sub></span><span>ω<sub>2</sub></span><span>ω<sub>3</sub></span><span>0</span><span>&minus;ω<sub>1</sub></span><span>&minus;ω<sub>2</sub></span><span>ω<sub>1</sub></span><span>0</span></span><span class="mbracket right"></span></span></p><p class="matline"><span class="m">ω<sup>∧</sup> v = ω × v</span></p><p>これで <span class="m">R = exp(ω<sup>∧</sup>)</span> が意味を持ちます。そしてこれらの行列がつくる平坦な空間 — <span class="m">SO(3)</span> 自身とは違い、足し算とスケールについて閉じています — が Lie 代数 <span class="m">so(3)</span> です。ハットは線形なので <span class="m">ω<sup>∧</sup> = (aθ)<sup>∧</sup> = θ a<sup>∧</sup></span>: <span class="m">SO(2)</span> の衛星が三つの符号化の表に書いた <span class="m">exp(â θ)</span> は、まさにこの式のことでした。<span class="m">SE(3)</span> はこの演算子をそのまま、4×4 の回転ブロックとして再利用します。</p><div class="bubble" id="so3gHatNote" role="dialog" aria-label="なぜ歪対称か" hidden><p>歪対称であることは<strong>強制</strong>であって、選択ではありません。拘束 <span class="m">R<sup>⊤</sup>R = I</span> を運動に沿って微分します:</p><p class="matline"><span class="m">Ṙ<sup>⊤</sup>R + R<sup>⊤</sup>Ṙ = 0 ⟹ (R<sup>⊤</sup>Ṙ)<sup>⊤</sup> = −R<sup>⊤</sup>Ṙ</span></p><p>つまり許される速度は、単位元で書けばすべて歪対称行列です。その自由な成分を数えてみましょう: 対角は 0、下三角は上三角の符号違いなので、残るのは <strong>3</strong>。上の軸と角から数えた 3 と同じ、拘束が残した 3 とも同じ — 同じ数への三本目の道です。</p><p>逆の写像は行列をベクトルへ戻すもので、<span class="m">∨</span> と書きます:</p><p class="matline"><span class="m">(ω<sup>∧</sup>)<sup>∨</sup> = ω</span></p></div><div class="bubble" id="so3gEulerNote" role="dialog" aria-label="Euler の定理の証明" hidden><p><span class="m">1</span> が <span class="m">R</span> の固有値であること、すなわち <span class="m">det(R − I) = 0</span> を示します。<span class="m">R R<sup>⊤</sup> = I</span> と <span class="m">det R = 1</span> を使って:</p><p class="matline"><span class="m">det(R − I) = det(R − R R<sup>⊤</sup>) = det R · det(I − R<sup>⊤</sup>)</span></p><p class="matline"><span class="m">= det((I − R)<sup>⊤</sup>) = det(I − R) = (−1)<sup>3</sup> det(R − I)</span></p><p>したがって <span class="m">det(R − I) = −det(R − I)</span>、ゆえに <span class="m">det(R − I) = 0</span>。<span class="m">R a = a</span> となる非零の <span class="m">a</span> が存在します: それが軸です。</p><p><strong>奇数</strong>次元がどこで効いているかに注目してください: <span class="m">(−1)<sup>3</sup></span> の因子です。偶数次元では証明が壊れますが、それは正当です — 平面回転（<span class="m">SO(2)</span>）には不動の<em>方向</em>はなく、不動点があるだけだからです。</p></div>'},
        ...
        {t:'ジンバルロックは定理であって不便ではない', b:'<p>Euler 角は三つの回転を入れ子にします — シーンの三つの環です。真ん中の <span class="m">β</span> が 90° に近づくとどうなるか見てください: 外側の環と内側の環の軸が<strong>一致します</strong>。ダイヤルは三つあるのに、独立な向きは二つしか残っていません。自由度が一つ消えたのです。</p><p>よくある反応は、これは Euler 角の不器用さであって、賢いパラメータ化なら直る、というものです。<strong>直りません。</strong>これはこの主題における唯一の不可能性定理です:</p><p class="matline"><span class="m">SO(3) に大域的で特異点のない 3 パラメータ座標は存在しない</span></p><p>そのような座標は定義上、<span class="m">SO(3)</span> と <span class="m">ℝ<sup>3</sup></span> の開集合との間の<button class="termbtn" id="so3gTopInfo" type="button" aria-expanded="false" aria-controls="so3gTopNote">微分同相</button>になります。しかし微分同相はあらゆる位相不変量を保ち、ここでは<em>二つ</em>が独立に壊れます:</p><p class="matline"><span class="m">SO(3) はコンパクト</span></p><p class="matline"><span class="m">ℝ<sup>3</sup> の開集合はそうでない</span></p><p class="matline"><span class="m">π<sub>1</sub>(SO(3)) = ℤ/2</span></p><p class="matline"><span class="m">π<sub>1</sub>(ℝ<sup>3</sup>) = 0</span></p><p>最初の一つだけで十分です。つまり<em>どんな</em> 3 パラメータ表現も — Euler 角も、Rodrigues ベクトルも、3 次元ベクトルに詰めた axis-angle も — <strong>どこかで必ず壊れます</strong>。選び方が悪いのではなく、平坦な三つの数では足りないのです。</p><p>逃げ道に見えるもの、そして実際に得られるもの。<strong>四元数</strong>は次元を一つ上げます: <span class="m">S<sup>3</sup></span> は滑らかで特異点がなく、<em>拘束</em>（<span class="m">‖q‖ = 1</span>）を<em>滑らかさ</em>と交換します — しかし <span class="m">S<sup>3</sup></span> は依然コンパクトで曲がっており、平坦には一歩も近づいていません。<strong>アトラス</strong>（重なりをもつ複数の局所チャート）は機能しますが、それは「大域的な平坦座標」という望みを達成したのではなく、放棄したということです。</p><p>実務上の答えは両者の組み合わせです: 回転は<em>冗長に</em>保存し（行列か四元数）、<em>増分</em>は局所接空間の 3 次元ベクトルとして取る。それが具体的にどう働くか — そして <span class="m">SO(3)</span> の本当の、<span class="m">ℤ/2</span> の形がどう見えるか — は<em>リーマン勾配</em>の衛星が見せます。</p><div class="bubble" id="so3gTopNote" role="dialog" aria-label="二つの不変量" hidden><p><em>微分同相</em>とは、両方向に滑らかな一対一対応のことです。二つの空間の間にそれがあれば、位相的には同じ空間を別の描き方で見ているだけです。</p><p><strong>コンパクト性。</strong><span class="m">SO(3)</span> は閉かつ有界です（行列成分が <span class="m">[−1, 1]</span> に入る）: どの無限列にも収束部分列があります。ところが <span class="m">ℝ<sup>3</sup></span> の開集合からは境界へ「逃げ出す」ことができます。滑らかな対応ではその差は埋まりません。</p><p><strong>単連結性。</strong><span class="m">π<sub>1</sub></span> は本質的に異なるループが何種類あるかを測ります。<span class="m">ℝ<sup>3</sup></span> ではどのループも一点に縮むので <span class="m">π<sub>1</sub> = 0</span>。<span class="m">SO(3)</span> では 360° 回転のループは<strong>縮みません</strong> — 720° のループは縮みます。これが <span class="m">ℤ/2</span> であり、ベルトの手品が示しているものです。</p><p>どちらか一方だけでも大域的な平坦座標は否定されます。両方成り立つことは、それを念押ししているにすぎません。</p></div>'}
      ]
    }
  };
})();
