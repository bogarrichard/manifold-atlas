...
LIE.journeys['slam-pipeline'] = (function () {
  ...
  const NPOSE = 11,
    SWEEP = (300 * Math.PI) / 180; // 11 poses, 30° apart
  ...
  function build(C, PAL) {
    ...
    const stations = [
      ...
      g => {
        const R = 2.3;
        const at = i => {
          const a = Math.PI / 2 - (SWEEP * i) / (NPOSE - 1);
          return {p: V3(Math.cos(a) * R, Math.sin(a) * R, 0), a};
        };
        const ring = [];
        for (let i = 0; i <= 120; i++) {
          const a = Math.PI / 2 - (SWEEP * i) / 120;
        ...
        const loop = K.dashedLine(at(NPOSE - 1).p, at(0).p, COL.coral, 0.13);
        g.add(loop);
        const lSmall = makeLabel('30°', HX.green, 1.5);
        lSmall.position.copy(at(1).p).add(V3(0.62, 0.3, 0));
        g.add(lSmall);
        const lBig = makeLabel('300°', HX.coral, 1.9);
        lBig.position.set(0.35, -0.15, 0);
        g.add(lBig);
        ...
        return {
          tick(t) {
            ...
            if (tc < T0) {
              ...
              hops.forEach((h, j) => {
                h.material.opacity = j === k ? 0.95 : 0.3;
              });
      ...
      g => {
        const EYE = V3(-3.2, 0, 0),
          IMG_X = -1.6,
          NEAR_X = 0.2,
          FAR_X = 3.6;
        const MS = [-0.3, -0.13, 0.06, 0.22, 0.37];
        ...
        line(
          g,
          [
            EYE.clone().add(V3(-0.34, 0.3, 0)),
            EYE.clone().add(V3(0.3, 0.52, 0)),
            EYE.clone().add(V3(0.3, -0.52, 0)),
            EYE.clone().add(V3(-0.34, -0.3, 0)),
            EYE.clone().add(V3(-0.34, 0.3, 0)),
          ],
        ...
        MS.forEach(m => {
          ...
          const img = dot(g, COL.coral, 0.085);
          img.position.set(IMG_X, yAt(IMG_X), 0);
          const gh = dot(g, COL.teal, 0.1);
          gh.position.set(NEAR_X, yAt(NEAR_X), 0);
          ghosts.push(gh);
          const mv = dot(g, COL.amber, 0.115);
          movers.push({mv, yAt});
        });
        const lbl = makeLabel('s · [R, t]', HX.amber, 2.6);
        lbl.position.set(0.4, 2.6, 0);
        g.add(lbl);
      ...
      g => {
        const XS = 1.62,
          YS = 1.55,
          Y0 = -1.65; // θ ∈ [0, π] → x ∈ [0, 5.1]
        const px = th => -2.55 + th * XS,
          py = v => Y0 + v * YS;
        ...
        const seg = (a, b, color, op) => {
          ...
          for (let i = 0; i <= 70; i++) {
            const th = a + ((b - a) * i) / 70;
            pts.push(V3(px(th), py(jlDev(th)), 0));
          }
        ...
        const lbl = makeLabel('‖J − I‖', HX.ink, 2.6);
        lbl.position.set(0, 2.75, 0);
        g.add(lbl);
        const read = makeLabel('5° · 0.06', HX.amber, 2.8);
        lbl.position.set(0, 2.75, 0);
        read.position.set(1.55, 2.15, 0);
        g.add(read);
