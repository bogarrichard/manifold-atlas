...
LIE.journeys['optimization-gd'] = (function () {
  ...
  function build(C, PAL) {
    ...
    function bowl(g) {
      ...
      minDot.position.set(0, H(0, 0) + 0.02, 0);
      g.add(minDot);
      return minDot;
    }
    ...
    const stations = [
      ...
      g => {
        ...
        g.add(ball);
        const lbl = makeLabel('L(w)', HX.ink, 2.0);
        lbl.position.set(0, 3.3, 0);
        g.add(lbl);
        return {
          tick(t) {
            const a = t * 0.5,
      ...
      g => {
        ...
        ball.position.set(p.x, H(p.x, p.z) + 0.15, p.z);
        g.add(ball);
        const up = fatArrow(COL.coral, 0.05),
          dn = fatArrow(COL.teal, 0.055);
        g.add(up);
        g.add(dn);
        const l1 = makeLabel('∇L = Q w', HX.coral, 2.4),
          l2 = makeLabel('−α∇L', HX.teal, 2.2);
        g.add(l1);
        g.add(l2);
        ...
        return {
          tick() {
            ...
            l1.position
              .copy(bp)
              .add(gdir.clone().multiplyScalar(1.9))
              .add(V3(0, 0.35, 0));
            l2.position
              .copy(bp)
              .add(gdir.clone().multiplyScalar(-1.7))
              .add(V3(0, 0.35, 0));
          },
        };
      },
      ...
      g => {
        ...
        g.add(arr);
        const lbl = makeLabel('w − α∇L', HX.coral, 2.8);
        lbl.position.set(0, 3.3, 0);
        g.add(lbl);
        return {
          tick(t) {
            ...
            if (k >= N - 1) {
              ball.position.copy(path[N - 1]);
              arr.visible = false;
              return;
            }
            const f = ease(clamp(cyc - k, 0, 1));
            ball.position.lerpVectors(path[k], path[k + 1], f);
            const d = V3(path[k + 1].x - path[k].x, 0, path[k + 1].z - path[k].z),
              L = d.length();
            if (L > 0.02) setArrow(arr, ball.position, d.normalize().multiplyScalar(0.6));
  ...
  return {
    ...
    cards: {
      hu: [
        {
          t: 'A völgy',
          b: '<p>Minimalizáljuk a</p><p class="matline"><span class="m">L(w) = <span class="frac"><span>1</span><span>2</span></span> w<sup>⊤</sup>Q w</span></p><p class="matline"><span class="m">Q =</span><span class="matrix"><span class="mbracket"></span><span class="mgrid" style="grid-template-columns:repeat(2,auto)"><span>0.64</span><span>0</span><span>0</span><span>0.64</span></span><span class="mbracket right"></span></span></p><p><button class="termbtn" id="ogQuadInfo" type="button" aria-expanded="false" aria-controls="ogQuadNote">kvadratikus</button> völgyet a lapos <span class="m">w ∈ ℝ<sup>2</sup></span> fölött. A sárga gömb a jelenlegi <span class="m">w</span>, a zöld pont a minimum (<span class="m">w<sub>*</sub> = 0</span>). Ugyanez a legkisebb négyzetek völgye is, ahol <span class="m">Q = A<sup>⊤</sup>A</span>. Sokaság nincs — minden a lapos <span class="m">ℝ<sup>2</sup></span>-ben történik.</p><div class="bubble" id="ogQuadNote" role="dialog" aria-label="Miert negyzetes alak" hidden><p>Skalár eset: egy változóra a völgy <span class="m">L(w) = <span class="frac"><span>1</span><span>2</span></span> q w<sup>2</sup></span>, a deriváltja <span class="m">q w</span>.</p><p>Több változóra a <span class="m">w<sup>2</sup></span> helyére a <span class="m">w<sup>⊤</sup>Q w</span> lép — ez a <em>négyzetes alak</em>:</p><p class="matline"><span class="m">w<sup>⊤</sup>Q w = Σ<sub>i,j</sub> w<sub>i</sub> Q<sub>ij</sub> w<sub>j</sub></span></p><p>A mi <span class="m">Q = 0.64·I</span> mátrixunkkal csak az átlós tagok maradnak, <span class="m">0.64 (w<sub>1</sub><sup>2</sup> + w<sub>2</sub><sup>2</sup>)</span>. A derivált a skalár <span class="m">q w</span> mátrixos párja: <span class="m">∇L = Q w</span>.</p></div>',
        },
        {
          t: 'A gradiens',
          b: '<p>A gradiens a kvadratikus alakból közvetlenül adódik:</p><p class="matline"><span class="m">∇L(w) = Q w = 0.64 · w</span></p><p>Ez a legmeredekebb <em>emelkedő</em> iránya (narancssárga nyíl). Lefelé a mínusza visz, és ez a lépés: <span class="m">−α∇L</span> (zöld). Az <span class="m">α</span> a tanulási ráta — mekkorát lépünk a negatív gradiens mentén. Általános <button class="termbtn" id="ogLsqInfo" type="button" aria-expanded="false" aria-controls="ogLsqNote">legkisebb négyzeteknél</button> <span class="m">∇L = A<sup>⊤</sup>(Aw − b)</span> — ugyanez az alak.</p><div class="bubble" id="ogLsqNote" role="dialog" aria-label="Legkisebb negyzetek gradiense" hidden><p>A legkisebb négyzetek a <span class="m">r = Aw − b</span> reziduum hosszának négyzetét minimalizálják:</p><p class="matline"><span class="m">L(w) = <span class="frac"><span>1</span><span>2</span></span> ‖Aw − b‖<sup>2</sup> = <span class="frac"><span>1</span><span>2</span></span> (Aw − b)<sup>⊤</sup>(Aw − b)</span></p><p>Kifejtve, a <span class="m">(Aw)<sup>⊤</sup>b = w<sup>⊤</sup>A<sup>⊤</sup>b</span> azonossággal:</p><p class="matline"><span class="m">L = <span class="frac"><span>1</span><span>2</span></span> w<sup>⊤</sup>A<sup>⊤</sup>A w − w<sup>⊤</sup>A<sup>⊤</sup>b + <span class="frac"><span>1</span><span>2</span></span> b<sup>⊤</sup>b</span></p><p>A gradiens (épp a fenti négyzetes alak, <span class="m">Q = A<sup>⊤</sup>A</span>):</p><p class="matline"><span class="m">∇L = A<sup>⊤</sup>A w − A<sup>⊤</sup>b = A<sup>⊤</sup>(Aw − b)</span></p></div>',
        },
        {
          t: 'Az iteráció',
          b: '<p>A frissítést behelyettesítve zárt alak marad:</p><p class="matline"><span class="m">w<sub>k+1</sub> = w<sub>k</sub> − α∇L(w<sub>k</sub>) = (I − αQ) w<sub>k</sub> = 0.82 · w<sub>k</sub></span></p><p>itt <span class="m">α = 0.28</span>. A hiba tehát mértani sorként fogy, <span class="m">w<sub>k</sub> = 0.82<sup>k</sup> · w<sub>0</sub></span> — a gömb, amit látsz, pontosan ezt járja be, <span class="m">w<sub>0</sub> = (2.7, 1.9)</span>-ből:</p><p class="matline"><span class="m">(2.70, 1.90) → (2.22, 1.56) → (1.82, 1.28) → … → 0</span></p><p>Ez a lapos motor. A <em>Riemann-gradiens</em> pontosan ezt emeli a görbült <span class="m">SO(3)</span>-ra: érintősík + vetítés + <span class="m">exp</span>.</p>',
        },
      ],
      en: [
        {
          t: 'The Valley',
          b: '<p>We minimize the</p><p class="matline"><span class="m">L(w) = <span class="frac"><span>1</span><span>2</span></span> w<sup>⊤</sup>Q w</span></p><p class="matline"><span class="m">Q =</span><span class="matrix"><span class="mbracket"></span><span class="mgrid" style="grid-template-columns:repeat(2,auto)"><span>0.64</span><span>0</span><span>0</span><span>0.64</span></span><span class="mbracket right"></span></span></p><p><button class="termbtn" id="ogQuadInfo" type="button" aria-expanded="false" aria-controls="ogQuadNote">quadratic</button> valley over flat <span class="m">w ∈ ℝ<sup>2</sup></span>. The yellow sphere is the current <span class="m">w</span>, the green dot the minimum (<span class="m">w<sub>*</sub> = 0</span>). This is also the least-squares valley, where <span class="m">Q = A<sup>⊤</sup>A</span>. No manifold — it all happens in flat <span class="m">ℝ<sup>2</sup></span>.</p><div class="bubble" id="ogQuadNote" role="dialog" aria-label="Why a quadratic form" hidden><p>Scalar case: for one variable the valley is <span class="m">L(w) = <span class="frac"><span>1</span><span>2</span></span> q w<sup>2</sup></span>, with derivative <span class="m">q w</span>.</p><p>For several variables, <span class="m">w<sup>2</sup></span> becomes <span class="m">w<sup>⊤</sup>Q w</span> — the <em>quadratic form</em>:</p><p class="matline"><span class="m">w<sup>⊤</sup>Q w = Σ<sub>i,j</sub> w<sub>i</sub> Q<sub>ij</sub> w<sub>j</sub></span></p><p>With our <span class="m">Q = 0.64·I</span> only the diagonal terms survive, <span class="m">0.64 (w<sub>1</sub><sup>2</sup> + w<sub>2</sub><sup>2</sup>)</span>. The derivative is the matrix analogue of the scalar <span class="m">q w</span>: <span class="m">∇L = Q w</span>.</p></div>',
        },
        {
          t: 'The Gradient',
          b: '<p>The gradient follows directly from the quadratic form:</p><p class="matline"><span class="m">∇L(w) = Q w = 0.64 · w</span></p><p>This is the direction of steepest <em>ascent</em> (orange arrow). Downhill is its negative, and that is the step: <span class="m">−α∇L</span> (green). <span class="m">α</span> is the learning rate — how far we move along the negative gradient. For general <button class="termbtn" id="ogLsqInfo" type="button" aria-expanded="false" aria-controls="ogLsqNote">least squares</button> <span class="m">∇L = A<sup>⊤</sup>(Aw − b)</span> — the same shape.</p><div class="bubble" id="ogLsqNote" role="dialog" aria-label="Least-squares gradient" hidden><p>Least squares minimizes the squared length of the residual <span class="m">r = Aw − b</span>:</p><p class="matline"><span class="m">L(w) = <span class="frac"><span>1</span><span>2</span></span> ‖Aw − b‖<sup>2</sup> = <span class="frac"><span>1</span><span>2</span></span> (Aw − b)<sup>⊤</sup>(Aw − b)</span></p><p>Expanding, using the identity <span class="m">(Aw)<sup>⊤</sup>b = w<sup>⊤</sup>A<sup>⊤</sup>b</span>:</p><p class="matline"><span class="m">L = <span class="frac"><span>1</span><span>2</span></span> w<sup>⊤</sup>A<sup>⊤</sup>A w − w<sup>⊤</sup>A<sup>⊤</sup>b + <span class="frac"><span>1</span><span>2</span></span> b<sup>⊤</sup>b</span></p><p>The gradient (exactly the quadratic form above, with <span class="m">Q = A<sup>⊤</sup>A</span>):</p><p class="matline"><span class="m">∇L = A<sup>⊤</sup>A w − A<sup>⊤</sup>b = A<sup>⊤</sup>(Aw − b)</span></p></div>',
        },
        {
          t: 'The Iteration',
          b: '<p>Substituting the update leaves a closed form:</p><p class="matline"><span class="m">w<sub>k+1</sub> = w<sub>k</sub> − α∇L(w<sub>k</sub>) = (I − αQ) w<sub>k</sub> = 0.82 · w<sub>k</sub></span></p><p>with <span class="m">α = 0.28</span>. The error therefore decays as a geometric series, <span class="m">w<sub>k</sub> = 0.82<sup>k</sup> · w<sub>0</sub></span> — the sphere you see walks exactly this, starting from <span class="m">w<sub>0</sub> = (2.7, 1.9)</span>:</p><p class="matline"><span class="m">(2.70, 1.90) → (2.22, 1.56) → (1.82, 1.28) → … → 0</span></p><p>This is the flat engine. The <em>Riemannian gradient</em> lifts exactly this onto the curved <span class="m">SO(3)</span>: tangent plane + projection + <span class="m">exp</span>.</p>',
        },
      ],
      ja: [
        {
          t: '谷',
          b: '<p>最小化するのは、平坦な <span class="m">w ∈ ℝ<sup>2</sup></span> の上に置かれた<button class="termbtn" id="ogQuadInfo" type="button" aria-expanded="false" aria-controls="ogQuadNote">二次</button>の谷です。</p><p class="matline"><span class="m">L(w) = <span class="frac"><span>1</span><span>2</span></span> w<sup>⊤</sup>Q w</span></p><p class="matline"><span class="m">Q =</span><span class="matrix"><span class="mbracket"></span><span class="mgrid" style="grid-template-columns:repeat(2,auto)"><span>0.64</span><span>0</span><span>0</span><span>0.64</span></span><span class="mbracket right"></span></span></p><p>黄色の球が現在の <span class="m">w</span>、緑の点が最小値（<span class="m">w<sub>*</sub> = 0</span>）です。これは最小二乗法の谷でもあり、そのときは <span class="m">Q = A<sup>⊤</sup>A</span> になります。多様体はどこにも出てきません。すべては平坦な <span class="m">ℝ<sup>2</sup></span> の中の話です。</p><div class="bubble" id="ogQuadNote" role="dialog" aria-label="なぜ二次形式なのか" hidden><p>まずスカラーの場合から見ましょう。変数が一つなら谷は <span class="m">L(w) = <span class="frac"><span>1</span><span>2</span></span> q w<sup>2</sup></span> で、その微分は <span class="m">q w</span> です。</p><p>変数が増えると、<span class="m">w<sup>2</sup></span> の位置に <span class="m">w<sup>⊤</sup>Q w</span> が入ります。これが<em>二次形式</em>です。</p><p class="matline"><span class="m">w<sup>⊤</sup>Q w = Σ<sub>i,j</sub> w<sub>i</sub> Q<sub>ij</sub> w<sub>j</sub></span></p><p>いまの <span class="m">Q = 0.64·I</span> では対角の項だけが残り、<span class="m">0.64 (w<sub>1</sub><sup>2</sup> + w<sub>2</sub><sup>2</sup>)</span> になります。微分はスカラーの <span class="m">q w</span> をそのまま行列にしたもので、<span class="m">∇L = Q w</span> です。</p></div>',
        },
        {
          t: '勾配',
          b: '<p>勾配は、二次形式からそのまま求まります。</p><p class="matline"><span class="m">∇L(w) = Q w = 0.64 · w</span></p><p>これが最も急な<em>上り</em>の方向で、オレンジ色の矢印がそれです。下りはその符号を反転した向きで、そこから一歩ぶんが <span class="m">−α∇L</span>（緑）になります。<span class="m">α</span> は学習率、つまり負の勾配に沿ってどれだけ進むかを決める数です。一般の<button class="termbtn" id="ogLsqInfo" type="button" aria-expanded="false" aria-controls="ogLsqNote">最小二乗</button>でも <span class="m">∇L = A<sup>⊤</sup>(Aw − b)</span> となり、形はまったく同じです。</p><div class="bubble" id="ogLsqNote" role="dialog" aria-label="最小二乗の勾配" hidden><p>最小二乗は、残差 <span class="m">r = Aw − b</span> の長さの二乗を最小にします。</p><p class="matline"><span class="m">L(w) = <span class="frac"><span>1</span><span>2</span></span> ‖Aw − b‖<sup>2</sup> = <span class="frac"><span>1</span><span>2</span></span> (Aw − b)<sup>⊤</sup>(Aw − b)</span></p><p><span class="m">(Aw)<sup>⊤</sup>b = w<sup>⊤</sup>A<sup>⊤</sup>b</span> を使って展開すると、こうなります。</p><p class="matline"><span class="m">L = <span class="frac"><span>1</span><span>2</span></span> w<sup>⊤</sup>A<sup>⊤</sup>A w − w<sup>⊤</sup>A<sup>⊤</sup>b + <span class="frac"><span>1</span><span>2</span></span> b<sup>⊤</sup>b</span></p><p>勾配は、上の二次形式で <span class="m">Q = A<sup>⊤</sup>A</span> と置いたものそのものです。</p><p class="matline"><span class="m">∇L = A<sup>⊤</sup>A w − A<sup>⊤</sup>b = A<sup>⊤</sup>(Aw − b)</span></p></div>',
        },
        {
          t: '反復',
          b: '<p>更新式を代入すると、そのまま解ける形が残ります。</p><p class="matline"><span class="m">w<sub>k+1</sub> = w<sub>k</sub> − α∇L(w<sub>k</sub>) = (I − αQ) w<sub>k</sub> = 0.82 · w<sub>k</sub></span></p><p>ここで <span class="m">α = 0.28</span> です。誤差は等比数列として減っていき、<span class="m">w<sub>k</sub> = 0.82<sup>k</sup> · w<sub>0</sub></span> となります。画面の球は <span class="m">w<sub>0</sub> = (2.7, 1.9)</span> から、まさにこの数列をたどります。</p><p class="matline"><span class="m">(2.70, 1.90) → (2.22, 1.56) → (1.82, 1.28) → … → 0</span></p><p>これが平坦な世界での原動機です。<em>リーマン勾配</em>は、この仕組みをそっくりそのまま曲がった <span class="m">SO(3)</span> の上へ持ち上げます。接平面を張り、勾配を射影し、<span class="m">exp</span> で戻す。それだけです。</p>',
        },
      ],
    },
  };
